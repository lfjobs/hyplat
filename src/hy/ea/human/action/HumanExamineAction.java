...
/**
 * 待人事审核管理：HumanExamineAction
 * @author zl
 *
 */
public class HumanExamineAction {
	@Resource
	...
	{
		ActionContext.getContext().getSession().put("tablesearch",cashierBillsVO);
		return getHumanList();
	}
		...
		if (search != null && search.equals("search")) {
			...
			if (cashierBillsVO != null) {
				...
				if (null != cashierBillsVO.getConsultStatus()
					...
					sql += " and t.consultStatus = '03'";
				} else {
					sql += " and t.consultStatus = '02'";
				}
			}
		}else{
			sql += " and t.consultStatus = '02'";
	...
	public String updateHuman() {
		...
		String hql = "update CashierBills set consultStatus = '"
				+ cashierBills.getConsultStatus() + "',personnel= '"
				+ sta.getStaffName()
		...
		if (cashierBills.getConsultStatus().equals("10")) {
			logbook = logBookService.saveCLogBook(organizationID,
					"驳回待人事审核(凭证号:" + cb.getJournalNum() + ")", account);
		} else {
			logbook = logBookService.saveCLogBook(organizationID,
					"人事审核通过:(凭证号：" + cb.getJournalNum() + ")", account);
		}
		beans.add(logbook);
	...
	public String showExcel() {
		...
		CLogBook logBook =logBookService.saveCLogBook(organizationID,"导出人事审核管理", account);
