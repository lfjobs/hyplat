...
public class CarInformationAction {
	...
	private DetachedCriteria getList() {
		...
		if(null != vehicleBrand && !"".equals(vehicleBrand)){
			if("其它".equals(vehicleBrand)){
				dc.add(Restrictions.isNull("vehicleBrand"));

			}else{
				dc.add(Restrictions.like("vehicleBrand", vehicleBrand.trim(),MatchMode.ANYWHERE));
			}
		}
  ...
  //车辆信息保存
    ...
    public String saveCarInformation()
    {
		...
		//02为教练车，增加教练信息
		if(carinfo.getState1().equals("02")){

            TbJpCar tbJpCar= (TbJpCar) baseBeanService.getBeanByHqlAndParams(" from TbJpCar where carId = ?",new Object[]{carInformation.getCarID()});
            if(tbJpCar == null) {
				tbJpCar=new TbJpCar();
                tbJpCar.setCarzjid(serverService.getServerID("carzjid"));
                tbJpCar.setCarId(carInformation.getCarID());
                tbJpCar.setPerdritype(tcar.getPerdritype());
                tbJpCar.setSkillLevel(tcar.getSkillLevel());
                tbJpCar.setFuel(tcar.getFuel());
                tbJpCar.setPlateColor(tcar.getPlateColor());
                tbJpCar.setManufacture(tcar.getManufacture());
                tbJpCar.setModel(tcar.getModel());
                tbJpCar.setBuydate(tcar.getBuydate());
                tbJpCar.setZkl(tcar.getZkl());
                tbJpCar.setEquipStatus(tcar.getEquipStatus());
                tbJpCar.setRemark(tcar.getRemark());
                tbJpCar.setFranum(carinfo.getCarFrameNum());
                tbJpCar.setEngnum(carinfo.getEngineNum());
                tbJpCar.setPlateNumber(carinfo.getCarNum());
                tbJpCar.setCreatedate(new Date());
                tbJpCar.setEngnum(carinfo.getEngineNum());
            }else{
                tbJpCar.setPerdritype(tcar.getPerdritype());
                tbJpCar.setSkillLevel(tcar.getSkillLevel());
    ...
    public TbJpCar getTcar() {
        return tcar;
    }

    public void setTcar(TbJpCar tcar) {
        this.tcar = tcar;
    }
}
