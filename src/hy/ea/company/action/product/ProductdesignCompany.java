 ...
 * 	产品包装设计公司汇总
 */
public class ProductdesignCompany {
	@Resource
	...
	public String showExcel(){
		...
		String hql = "from ProductPackaging pp where exists " +
				"( select cp.companyID from Company cp where pp.companyID = cp.companyID and exists " +
				"(select co.organizationID  from COrganization  co where co.organizationID = pp.organizationID and exists " +
				"(select hs.staffID from Staff hs where hs.staffID=pp.staffID and exists " +
				"(select gm.goodsID from GoodsManage gm where gm.goodsID=pp.goodsID and pp.companyID = ?))))"; 
