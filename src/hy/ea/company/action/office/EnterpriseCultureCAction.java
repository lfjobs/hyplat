...
@Controller
@Scope("prototype")
public class EnterpriseCultureCAction {
	@Resource
	...
	private DetachedCriteria getList() {
		...
		if (search != null && search.equals("search")) {
			...
			if(enterpriseCulture.getCulture() != null
					&& !"".equals(enterpriseCulture.getCulture().trim())){
				dc.add(Restrictions.like("culture", enterpriseCulture.getCulture(), MatchMode.ANYWHERE));
				
			}if(enterpriseCulture.getCultureContent() != null
					&& !"".equals(enterpriseCulture.getCultureContent().trim())){
				dc.add(Restrictions.like("cultureContent", enterpriseCulture.getCultureContent(), MatchMode.ANYWHERE));
			}
		} 
		return dc;
	}
	...
	public String showExcel(){
		...
		String companyID = account.getCompanyID();
		excelStream = excelService.showExcel(EnterpriseCulture.columnHeadings(), baseBeanService.getListByDC(getList()));
		CLogBook logbook=logBookService.saveCLogBook(companyID,"导出文件管理公司汇总列表", account);
