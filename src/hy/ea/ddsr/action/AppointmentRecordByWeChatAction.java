...
public class AppointmentRecordByWeChatAction {
	...
	public String getListCoachReservationRecordContentPersonal() {
		if (generateListReservationRecord()) {
			...
			if (dssrsubject != null) {
				if (dssrsubject.getSubjType() != null
						&& !"".equals(dssrsubject.getSubjType())) {
					dc.add(Restrictions.in(
							dc.getAlias() + "." + "rereSubjects",
							new Object[] { dssrsubject.getSubjType(),
									Byte.parseByte("90") }));
				}
			}
			/*
			 * if(dssrstudent!=null){
			 * if(dssrstudent.getStudKey()!=null&&!"".equals
			 * (dssrstudent.getStudKey().trim())){
			 * dc.add(Restrictions.eq("dssrstudent.studKey",
			 * dssrstudent.getStudKey().trim())); } }
			 */
			ddsrreservationrecordList = baseBeanService.getListByDC(dc);
		}
		return "coachreservationrecordcontentOfWeChatOfList";
	}
	 ...
	 * @return
	 */
	public String getListRecordContentPersonal() {
		String alias = "ddsrreservationrecord"; // 查的table名
		DetachedCriteria dc = DetachedCriteria.forClass(
				Ddsrreservationrecord.class, alias);
		Dssrstudent dssrstudent=(Dssrstudent) session.get("dssrstudent");
		dc.add(Restrictions.eq(alias + "." + "rereCompanyid",
				dssrstudent.getStudCompanyid()));
		
		dc.createAlias("ddsrcoach", "ddsrcoach", DetachedCriteria.LEFT_JOIN);
		dc.createAlias("ddsrcoach.dtHrStaff", "dtHrStaff",
				DetachedCriteria.LEFT_JOIN);
		dc.createAlias("reDssrstudentDdsrresrecords",
				"reDssrstudentDdsrresrecords", DetachedCriteria.LEFT_JOIN);
		dc.createAlias("reDssrstudentDdsrresrecords.dssrstudent",
				"dssrstudent", DetachedCriteria.LEFT_JOIN);
		dc.createAlias("reDssrstudentDdsrresrecords.dssrstudent.dtHrStaff",
				"dtHrStaff1", DetachedCriteria.LEFT_JOIN);
		
		dc.add(Restrictions.eq("dssrstudent.studKey", dssrstudent.getStudKey()));
		
		Calendar calendar = Calendar.getInstance();
		Calendar calendar1 = Calendar.getInstance();
		calendar1.add(Calendar.DATE, 7);
		dc.add(Restrictions.between(alias + "." + "rereAppdate", calendar.getTime(), calendar1.getTime()));
		
		dc.addOrder(Order.asc(alias + "." + "rereStadate"));
		ddsrreservationrecordList = baseBeanService.getListByDC(dc);
		return "recordcontentOfWeChatOfList";
	}
	...
	public DetachedCriteria getListCoachReservationRecordContentPersonalByDc() {
		 ...
		 * pList.add(Projections.property(alias + "." +
		 * "rereKey").as("rereKey")); pList.add(Projections.property(alias + "."
		 * + "rereCompanyid").as("rereCompanyid"));
		 * pList.add(Projections.property(alias + "." +
		 * "rereAppdate").as("rereAppdate"));
		 * pList.add(Projections.property(alias + "." +
		 * "rereClass").as("rereClass")); pList.add(Projections.property(alias +
		 * "." + "rereStadate").as("rereStadate"));
		 * pList.add(Projections.property(alias + "." +
		 * "rereEnddate").as("rereEnddate"));
		 * pList.add(Projections.property(alias + "." +
		 * "rerePeoplesum").as("rerePeoplesum"));
		 * pList.add(Projections.property(alias + "." +
		 * "rereSubjects").as("rereSubjects"));
		 * pList.add(Projections.property(alias + "." +
		 * "ddsrcoach"+"."+"dtHrStaff.staffName").as(alias));
		 * pList.add(Projections.property(alias + "." +
		 * "reDssrstudentDdsrresrecords").as("reDssrstudentDdsrresrecords"));
		 * dc.setProjection(pList);
		 * dc.setResultTransformer(Transformers.aliasToBean
		 * (Ddsrreservationrecord.class));
		 */
		Dssrstudent dssrstudent=(Dssrstudent) session.get("dssrstudent");
		dc.add(Restrictions.eq(alias + "." + "rereCompanyid",
				dssrstudent.getStudCompanyid()));
		...
		dc.createAlias("reDssrstudentDdsrresrecords.dssrstudent.dtHrStaff",
				"dtHrStaff1", DetachedCriteria.LEFT_JOIN);
		if (ddsrcoach != null && ddsrcoach.getCoacKey() != null
				&& !"".equals(ddsrcoach.getCoacKey().trim())) {
			dc.add(Restrictions.eq("ddsrcoach.coacKey", ddsrcoach.getCoacKey()
					.trim()));
		}
		if (ddsrreservationrecord != null
				&& ddsrreservationrecord.getRereAppdate() != null
				&& !"".equals(ddsrreservationrecord.getRereAppdate())) {
			dc.add(Restrictions.eq(alias + "." + "rereAppdate",
					ddsrreservationrecord.getRereAppdate()));
		}
		dc.addOrder(Order.asc(alias + "." + "rereStadate"));
		return dc;
	}
	...
	public boolean generateListReservationRecord() {
		 ...
		 * ddsrreservationrecord
		 * .setRereAppdate(DateUtil.parseDate("yyyy-MM-dd","2013-12-14"));
		 */
		if (ddsrcoach != null && ddsrcoach.getCoacKey() != null
				&& !"".equals(ddsrcoach.getCoacKey().trim())) {
			Ddsrcoach ddsrcoach_a = (Ddsrcoach) baseBeanService.getBeanByKey(
					Ddsrcoach.class, ddsrcoach.getCoacKey());
			if (ddsrreservationrecord != null
					&& ddsrreservationrecord.getRereAppdate() != null
					&& !"".equals(ddsrreservationrecord.getRereAppdate())) {
				int count = baseBeanService
						.getConutByDC(getListCoachReservationRecordContentPersonalByDc());
				/**
				 * 如果没有就生成预约记录
				 */
				if (count == 0) {
					 ...
					 */
					// List<BaseBean>
					// beans_subject=getListReDdsrcoachDssrsubjectPersonal();
					if (beans_worktime != null && beans_worktime.size() != 0) {
						...
						// ReDdsrcoachDssrsubject
						// subject=(ReDdsrcoachDssrsubject)bean_subject;
						for (BaseBean bean_worktime : beans_worktime) {
							Ddsrworktime worktime = (Ddsrworktime) bean_worktime;
							Date startdate = DateUtil.parseDate("HH:mm",
									worktime.getWotiStrdate());
							Date enddate = DateUtil.parseDate("HH:mm",
									worktime.getWotiEnddate());
							int hour = DateUtil.getHourOfDate(enddate)
									- DateUtil.getHourOfDate(startdate);
							for (int i = 0; i < hour; i++) {
								Ddsrreservationrecord ddsrrr = new Ddsrreservationrecord();
								ddsrrr.setDdsrcoach(ddsrcoach_a);
								ddsrrr.setRereCompanyid(ddsrcoach_a
										.getCoacCompanyid());
								ddsrrr.setRereAppdate(ddsrreservationrecord
										.getRereAppdate());
								ddsrrr.setRereClass(worktime.getWotiClass());
								try {
									Calendar calendar = DateUtil
											.toCalendarFromUtilDate(startdate);
									calendar.set(Calendar.HOUR_OF_DAY,
											startdate.getHours() + i);
									ddsrrr.setRereStadate(DateUtil.toStrDateFromUtilDateByFormat(
	...
	public DetachedCriteria getListDdsrworktimePersonalByDc() {
		...
		if (session.get("unionCompanyID") != null) {// 区别于 驾校联盟网站传参获得数据
													// 从session中获得companyID;
			dc.add(Restrictions.eq("wotiCompanyid",
					session.get("unionCompanyID")));
		} else if (account != null && null != account.getCompanyID()
				&& !"".equals(account.getCompanyID())) {
			dc.add(Restrictions.eq("wotiCompanyid", account.getCompanyID()));
		}
		dc.add(Restrictions.eq("ddsrcoach.coacKey", ddsrcoach.getCoacKey()
				.trim()));
		dc.add(Restrictions.le("wotiStrdaydate",
				ddsrreservationrecord.getRereAppdate()));
		dc.add(Restrictions.ge("wotiEnddaydate",
				ddsrreservationrecord.getRereAppdate()));
	...
	public List<BaseBean> getListReDdsrcoachDssrsubjectPersonal() {
		List<BaseBean> ddsrSubjectList = baseBeanService
				.getListByDC(getListReDdsrcoachDssrsubjectPersonalByDc());
	...
	public DetachedCriteria getListReDdsrcoachDssrsubjectPersonalByDc() {
		...
		DetachedCriteria dc = DetachedCriteria
				.forClass(ReDdsrcoachDssrsubject.class);
		dc.createAlias("ddsrcoach", "ddsrcoach");
		dc.add(Restrictions.eq("ddsrcoach.coacCompanyid",
	...
	public String deleteReservationRecordContentPersonal() {
		...
		if (ddsrreservationrecord != null) {
			if (ddsrreservationrecord.getRereKey() != null
					&& !"".equals(ddsrreservationrecord.getRereKey())) {

				DetachedCriteria dc = DetachedCriteria
						.forClass(Ddsrreservationrecord.class);
				dc.createAlias("reDssrstudentDdsrresrecords",
						"reDssrstudentDdsrresrecords",
						DetachedCriteria.LEFT_JOIN);
				dc.add(Restrictions.eq("rereKey",
						ddsrreservationrecord.getRereKey()));
				ddsrreservationrecordList = baseBeanService.getListByDC(dc);
				Ddsrreservationrecord bean = (Ddsrreservationrecord) ddsrreservationrecordList
						.get(0);
				...
				String dateStr = "";
				try {
					dateStr = DateUtil.toStrDateFromUtilDateByFormat(
				...
				for (BaseBean baseBean : setBeans) {
					...
					Object[] objs = toAddOrSubtractDssrstudentByRemhour(sd, 1,
							1);
				...
				baseBeanService
						.saveBeansListAndexecuteHqlsByParams(beans, hqls,
								new Object[] { reDssrstudentDdsrresrecord
										.getStureKey() });

				map.put("status", true);
				addRemindContent(bean.getDdsrcoach(), sd,
						ddsrreservationrecordList, 0);
				/*
				 * if(sendMessage(bean.getDdsrcoach(),sd,ddsrreservationrecordList
				 * ,0)){ map.put("message", "操作成功！！短信已发送"); }else{
	...
	public String addReservationRecordContentPersonal() {
		...
		Map<String, Object> map = new HashMap<String, Object>();
		boolean status = false;
		if (rereKeyString != null && !"".equals(rereKeyString)
				&& dssrstudent != null && dssrstudent.getStudKey() != null
				&& !"".equals(dssrstudent.getStudKey())) {
			List<String> array = Arrays.asList(rereKeyString.split(","));
			DetachedCriteria dc = DetachedCriteria
					.forClass(ReDssrstudentDdsrresrecord.class);
			...
			if (reDssrstDdsList == null || reDssrstDdsList.size() == 0) {
				 ...
				 * 预约学员
				 */
				Dssrstudent sd = (Dssrstudent) baseBeanService.getBeanByKey(
						Dssrstudent.class, dssrstudent.getStudKey());
				...
				List<BaseBean> reDssrstDdsList2 = baseBeanService
						.getListByDC(dc1);

				/**
				 * 预约教练
				 */
				Ddsrcoach dca = (Ddsrcoach) baseBeanService.getBeanByKey(
						Ddsrcoach.class,
						((Ddsrreservationrecord) reDssrstDdsList2.get(0))
								.getDdsrcoach().getCoacKey());
				/**
				 * 判断日期是否合法
				 */
				Date t1 = new Date();
				...
				if (((Ddsrreservationrecord) reDssrstDdsList2.get(0))
						.getRereAppdate().compareTo(date) != 1) {
					map.put("status", status);
				 ...
				 * @return reDssrstDdsList2
				 */
				DetachedCriteria dc1_a = DetachedCriteria
						.forClass(Ddsrreservationrecord.class);
				dc1_a.createAlias("reDssrstudentDdsrresrecords",
						"reDssrstudentDdsrresrecords",
						DetachedCriteria.LEFT_JOIN);
				dc1_a.createAlias("reDssrstudentDdsrresrecords.dssrstudent",
						"dssrstudent", DetachedCriteria.LEFT_JOIN);
				dc1_a.add(Restrictions.eq("rereAppdate",
						((Ddsrreservationrecord) reDssrstDdsList2.get(0))
								.getRereAppdate()));
				dc1_a.add(Restrictions.eq("dssrstudent.studKey", dssrstudent
						.getStudKey().trim()));
				List<BaseBean> reDssrstDdsList3 = baseBeanService
						.getListByDC(dc1_a);
				/**
				 * 
				 * 判断是否超过4个学时
				 ...
				 */
				Object[] objs = validataPeriodOfTimeByStudent(reDssrstDdsList2,
						reDssrstDdsList3);
				 ...
				 * 更新学员学时 addOrdel
				 */
				Object[] objs_a = toAddOrSubtractDssrstudentByRemhour(sd,
						array.size(), 0);
				 ...
				 * map.put("message", "操作成功！！短信已发送"); }else{ map.put("message",
				 * "操作成功！！短信未发送"); }
				 */
				map.put("message", "操作成功！！短信未发送");
			}
	 ...
	 * 新增 webservices 提醒内容
	 */
	private void addRemindContent(Ddsrcoach dca, Dssrstudent dsd,
			List<BaseBean> reDdList, int i) {
		...
		for (BaseBean baseBean : reDdList) {
			 ...
			 */
			sb.append(" 科目 : "
					+ ("10".equals(dd.getRereSubjects().toString()) ? "科一"
							: "20".equals(dd.getRereSubjects()) ? "科二" : "30"
									.equals(dd.getRereSubjects()) ? "科三" : "科四")
					+ ";");
			try {
				sb.append(" 日期 : "
						+ DateUtil.toStrDateFromUtilDateByFormat(
								dd.getRereAppdate(), "yyyy-MM-dd") + ";");
				rm.setReceiveDate(dd.getRereAppdate());// 接收时间
			} catch (ParseException e) {
			}
			sb.append(" 时间段 : " + dd.getRereStadate() + "~"
					+ dd.getRereEnddate() + ",");
	 ...
	 * 发送短信
	 */
	private boolean sendMessage(Ddsrcoach dca, Dssrstudent dsd,
			List<BaseBean> reDdList, int i) {
	 ...
	 * */
	private Object[] validataPeriodOfTimeByStudent(List<BaseBean> listNo,
			List<BaseBean> listYes) {
		...
		if (listYes == null || listYes.size() == 0) {
			...
			for (int i = 0; i < listNo.size(); i++) {
				Ddsrreservationrecord dd_no = (Ddsrreservationrecord) listNo
						.get(i);
				Date strDate_no = DateUtil.parseDate("HH:mm",
						dd_no.getRereStadate());
				Date endDate_no = DateUtil.parseDate("HH:mm",
						dd_no.getRereEnddate());
				for (int j = 0; j < listYes.size(); j++) {
					Ddsrreservationrecord dd_yes = (Ddsrreservationrecord) listYes
							.get(j);
					Date strDate_yes = DateUtil.parseDate("HH:mm",
							dd_yes.getRereStadate());
					Date endDate_yes = DateUtil.parseDate("HH:mm",
							dd_yes.getRereEnddate());
					if (strDate_yes.equals(strDate_no)
							|| (strDate_yes.after(strDate_no) && strDate_yes
									.before(endDate_no))
							|| (endDate_yes.after(strDate_no) && endDate_yes
									.before(endDate_no))) {
						result_a[0] = true;
						result_a[1] = "即将预约时间段 (" + dd_no.getRereStadate()
								+ "~" + dd_no.getRereEnddate() + ") 与 已预约时间段("
								+ dd_yes.getRereStadate() + "~"
								+ dd_yes.getRereEnddate() + ") 相交!!";
						return result_a;
					}
				}
			}
	 ...
	 */
	private Object[] toAddOrSubtractDssrstudentByRemhour(Dssrstudent sd,
			int sum, int b) {
		Object[] result_a = new Object[2];
		/**
		 * add
	...
	public String getListReservationRecordSummary() {
		pageForm = baseBeanService.getPageFormBySQL(
				...
				getReservationRecordSummaryBySql().get(0).toString(),
				"select count(1) from("
						+ getReservationRecordSummaryBySql().get(0).toString()
						+ ")", (Object[]) getReservationRecordSummaryBySql()
						.get(1));
	...
	public String getListReservationRecordSummaryPrint() {
		ddsrreservationrecordsummaryList = baseBeanService
				.getListBeanBySqlAndParams(getReservationRecordSummaryBySql()
						.get(0).toString(),
	...
	public List<Object> getReservationRecordSummaryBySql() {
		...
		sqlFromStatic
				.append(" left join ddsrcoach dsc on dsc.coac_key=dsrr.coac_key ");
		sqlFromStatic
				.append(" left join dt_hr_staff hs on hs.staffkey=dsc.coac_key ");
		sqlFromStatic
				.append(" left join Re_Dssrstudent_Ddsrresrecord rddr on rddr.rere_key=dsrr.rere_key ");
		sqlFromStatic
				.append(" left join dssrstudent dss on dss.stud_key=rddr.stud_key ");
		sqlFromStatic
				.append(" left join dt_hr_staff hs1 on hs1.staffkey=dss.stud_key ");
		sql.append(sqlFromStatic);

		StringBuffer sqlWhereStatic = new StringBuffer(" where ");
		sqlWhereStatic.append(" 1=1 ");
		if (account != null && null != account.getCompanyID()
				&& !"".equals(account.getCompanyID())) {
		...
		if (search != null && "search".equals(search)) {
			...
			Ddsrreservationrecord ddsrreservationrecord = (Ddsrreservationrecord) session
					.get("tablesearch2");
			if (ddsrcoach != null) {
				if (ddsrcoach.getDtHrStaff() != null) {
					if (ddsrcoach.getDtHrStaff().getStaffName() != null
							&& !"".equals(ddsrcoach.getDtHrStaff()
									.getStaffName().trim())) {
						sqlWhereStatic.append(" and hs.STAFFNAME like ? ");
						param.add("%"
								+ ddsrcoach.getDtHrStaff().getStaffName()
										.trim() + "%");
					}
				}
			}
			if (dssrstudent != null) {
				if (dssrstudent.getDtHrStaff() != null) {
					if (dssrstudent.getDtHrStaff().getStaffName() != null
							&& !"".equals(dssrstudent.getDtHrStaff()
									.getStaffName().trim())) {
						sqlWhereStatic.append(" and hs1.STAFFNAME like ? ");
						param.add("%"
								+ dssrstudent.getDtHrStaff().getStaffName()
										.trim() + "%");
					}
				}
			}
			if (ddsrreservationrecord != null) {
				if (ddsrreservationrecord.getSearchStaDate() != null
						&& !"".equals(ddsrreservationrecord.getSearchStaDate())
						&& ddsrreservationrecord.getSearchEndDate() != null
						&& !"".equals(ddsrreservationrecord.getSearchEndDate())) {
					sqlWhereStatic
							.append(" and dsrr.rere_appdate between ? and ? ");
			...
			Calendar calendarStaDate = DateUtil
					.toCalendarFromUtilDate(new Date());
			Calendar calendarEndDate = DateUtil
					.toCalendarFromUtilDate(new Date());
			try {
				searchStaDate = DateUtil.parseDate(
						"yyyy-MM-dd",
						DateUtil.toStrDateFromUtilDateByFormat(
								calendarStaDate.getTime(), "yyyy-MM-dd"));
				calendarEndDate.add(Calendar.DAY_OF_MONTH, 6);
				searchEndDate = DateUtil.parseDate(
						"yyyy-MM-dd",
						DateUtil.toStrDateFromUtilDateByFormat(
	...
	public static void main(String[] args) {
		/*
		 * Date date=DateUtil.parseDate("HH:mm", "05:30"); Date
		 * date1=DateUtil.parseDate("HH:mm","07:30");
