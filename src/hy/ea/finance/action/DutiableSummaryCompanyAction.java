...
@Controller
@Scope("prototype")
public class DutiableSummaryCompanyAction {
	...
	public String getDutiableList() {
		...
		/*
		 * String go = ""; if (taxstatusDu.equals("01")) { go = "list"; } else
		 * if (taxstatusDu.equals("00")) { go = "taxlist"; }
		 */
		return "list";
	}
	...
	public List<Object> getList() {
		...
		// 判断页面传过来的税务状态是否为空
		if (taxstatusDu != null && !taxstatusDu.equals("")
				&& !taxstatusDu.equals("null")) {
			// 不为空则保存在session里
			session.put("taxstatus", taxstatusDu);
		} else {
			// 为空则从session里取之前存好的值
			taxstatusDu = session.get("taxstatus").toString();
		}
		sql += " and t.taxstatus = ?";
		parms.add(taxstatusDu);
