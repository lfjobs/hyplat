...
public class AuditAction {
	...
	public List<Object> getList() {
		...
		if (taxstatusDu != null && taxstatusDu.equals("taxstatusDu")) {
			sql += " and (t.taxstatus = ? or (t.taxstatus=? and t.financial is not null))";
			parms.add("04");
			parms.add("10");
		} else {
			sql += " and t.taxstatus = ?";
	...
	public String updateResponsible() {
		...
		String hql = "update CashierBills set taxstatus = ?,financial= ? where companyID = ?  and cashierBillsID = ? ";
		...
		if (cashierBills.getTaxstatus().equals("10")) {
			logbook = logBookService.saveCLogBook(organizationID,
					"财务审核驳回作废(凭证号:" + cb.getJournalNum() + ")", account);
		} else {
			logbook = logBookService.saveCLogBook(organizationID, "审核通过:(凭证号："
