...
public class CompetentAction {
	...
	public List<Object> getList() {
		...
		if (taxstatusDu != null && taxstatusDu.equals("taxstatusDu")) {
			sql += " and (t.taxstatus between ? and ? or (t.taxstatus=? and t.competent is not null and t.manager is null))";
			parms.add("02");
			parms.add("04");
			parms.add("10");
		} else {
	...
	public String updateResponsible() {
		...
		String hql = "update CashierBills set taxstatus = ?,competent= ? where companyID = ?  and cashierBillsID = ? ";
		...
		CashierBills cb = (CashierBills) baseBeanService.getBeanByHqlAndParams(
				hql1, new Object[]{account.getCompanyID(), cashierBills.getCashierBillsID()});
		List<BaseBean> beans = new ArrayList<BaseBean>();
		CLogBook logbook = null;
		if (cashierBills.getTaxstatus().equals("10")) {
			logbook = logBookService.saveCLogBook(organizationID,
					"主管审核驳回作废(凭证号:" + cb.getJournalNum() + ")", account);
		} else {
			logbook = logBookService.saveCLogBook(organizationID,
					"转经理审核单:(凭证号：" + cb.getJournalNum() + ")", account);
		}
		beans.add(logbook);
