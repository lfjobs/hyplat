...
@Controller
@Scope("prototype")
public class CDepotPersonAction { 
	@Resource
	...
	private DetachedCriteria getList(){
		...
		if (search != null && search.equals("search")) {
			...
			if (depotPerson.getResponsible()!=null && !"".equals(depotPerson.getResponsible())) {
				dc.add(Restrictions.like("responsible", depotPerson.getResponsible(), MatchMode.ANYWHERE));
			}	
		} 		 
		return dc;
	}
	...
	public String showExcel(){
		excelStream = excelService.showExcel(DepotPerson.columnHeadings(), baseBeanService.getListByDC(getList()));
		Map<String, Object> session = ActionContext.getContext().getSession();
		String companyID=(String) session.get("companyID");
		CAccount account = (CAccount) session.get("account");		
		CLogBook logBook = logBookService.saveCLogBook(companyID,"导出库房责任人", account);
    ...
    public String saveDepotPerson()
    {
		...
		if (depotPersonmap!=null) {
			...
			for (DepotPerson depotPersons : depotPersonmap.values()) {	
				...
				if (depotPersons.getDepotPersonID()== null|| "".equals(depotPersons.getDepotPersonID())) {
					depotPersons.setDepotPersonID(serverService.getServerID("depotPerson"));
					parameter = "添加库房责任人(责任人:"+depotPersons.getResponsible()+")";
				}else{
				 String hql2="from DepotPerson where companyID=?  and depotPersonID=?";
				 DepotPerson network=(DepotPerson) baseBeanService.getBeanByHqlAndParams(hql2, new Object[]{companyID,depotPersons.getDepotPersonID()});
			     parameter = "修改库房责任人(责任人:"+network.getResponsible()+")";
			
				}
				depotPersons.setCompanyID(companyID);
				parermiters.add(parameter);
				beans.add(depotPersons);
	 ...
	 public String delDepotPerson()
	    {
			...
			CAccount account = (CAccount) session.get("account");			
			String companyID=(String) session.get("companyID");
			Object[] params = {companyID,depotPerson.getDepotPersonID()};
		    String hql2="from DepotPerson where companyID=?  and depotPersonID=?";
		    DepotPerson network=(DepotPerson) baseBeanService.getBeanByHqlAndParams(hql2, params);
		    CLogBook logBook = logBookService.saveCLogBook(companyID, "删除库房责任人(责任人:"+network.getResponsible()+")", account);
		    beans = new ArrayList<BaseBean>();
	    	beans.add(logBook);
