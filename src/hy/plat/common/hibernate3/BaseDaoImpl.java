...
import hy.plat.common.util.MyBeanUtils;
...
public abstract class BaseDaoImpl<T extends Serializable> implements BaseDao<T> {
	...
	@SuppressWarnings("unchecked")
	public List<T> findByList(Finder finder){
	...
	@SuppressWarnings("rawtypes")
	public Pagination find(String sql,Object[] params, int pageNo, int pageSize) {
		int totalCount = countQueryResult(sql, params);
		Pagination p = new Pagination(pageNo, pageSize, totalCount);
		if (totalCount < 1) {
			p.setList(new ArrayList());
			return p;
	 ...
	 * @return
	 */
	protected int countQueryResult(String sql, Object[] params) {
		Query query = getSession().createSQLQuery(sql);
		if (params != null && params.length > 0) {
			int i = 0;
			for (Object param : params) {
