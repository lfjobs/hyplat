...
<head>
    ...
    <script type="text/javascript">
        ...
        var depPostIDN = depPostID.split("_")[0];
        var postName = depPostID.split("_")[1];
    </script>
</head>
...
<div id="personalInformation">
    <form method="post" id="myForm"> <!-- 提交表单的目标页面为processForm.jsp -->
        ...
        岗位名称：<input type="text" name="jobName" style="height: 12px" required/><br/>
    ...
    $(document).ready(function () {
            ...
            if ($(this).val() === 'full-time') {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $.each(data.personTypes, function (index, personType) {
                ...
                $.ajax({
                    url: '职务级别接口url', // 替换为你的后端接口URL
                    method: 'POST',
                    success: function (data) {
                        // 填充职务级别下拉框
                        var jobLevelSelect = $('select[name="jobLevel"]');
                        jobLevelSelect.empty(); // 清空之前的选项
                        $.each(data.jobLevels, function (index, jobLevel) {
        ...
        $('#myForm').submit(function (event) {
            ...
            var formData = {
                ...
                personType: $('select[name="personType"]').val(),
                jobLevel: $('select[name="jobLevel"]').val()
            };

            // 发送 JSON 数据到后端
            $.ajax({
                url: '确认分配接口url', // 替换为你的后端接口URL
                method: 'POST',
