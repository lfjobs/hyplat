...
<div class="content">
    <s:form id="f2" action="ea_savePlanCostBudgetSheet.jspa" namespace="/ea/scBudget" method="POST">
...
<%--                        </li>--%>
                        <input type="text" id="billsType" name="cashierBills.billsType" value="计划" style="display: none;"/><%--所选项目类型--%>
                    </ul>
                </section>
...
<%--下拉控制js文件--%>
<script type="text/javascript" src="<%=basePath%>js/ea/plan/costBudgetBillAdd.js"></script>
<%--调用安卓js文件--%>
...
<script>
    ...
    function toBarCodeInfo(goodsBillsID) {
        window.location.href = basePath + "ea/scBudget/ea_toPlanCostBudgetItemUpdate.jspa?keyNum=" + goodsBillsID + "&editFlag=add&showFlag=${showFlag}&departmentID=${departmentID}&cashierBillsId=${cashierBillsId}&menuId="+menuId;
    }
    ...
    function toRemoveGoodsBills() {
        ...
        if(id){
            var url = "ea_removePlanGoodsBillOfAdd.jspa?keyNum=" + id+"&editFlag=add&menuId="+menuId;
