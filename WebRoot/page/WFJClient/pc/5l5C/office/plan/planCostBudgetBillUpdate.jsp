...
<div class="content">
    <s:form id="f2" action="ea_updatePlanCostBudgetSheet.jspa" namespace="/ea/scBudget" method="POST">
...
<%--下拉控制js文件--%>
<script type="text/javascript" src="<%=basePath%>js/ea/plan/costBudgetBillUpdate.js"></script>
<%--调用安卓js文件--%>
...
<script>
    ...
    function toBarCodeInfo(goodsBillsID) {
        window.location.href = basePath + "ea/scBudget/ea_toPlanCostBudgetItemUpdate.jspa?keyNum=" + goodsBillsID + "&editFlag=edit&showFlag=${showFlag}&departmentID=${departmentID}&cashierBillsId=${cashierBillsId}&menuId="+menuId;
    }
    ...
    function toRemoveGoodsBills(goodsBillsId) {
        ...
        if(id){
            document.location.replace(basePath + "ea/scBudget/ea_removePlanGoodsBillOfUpdate.jspa?keyNum=" + id+"&editFlag=edit&cashierBillsId="+cashierBillsId+"&menuId="+menuId);
        }
    ...
    function toBack() {
        if(menuId=="receive"){
            window.location.href = basePath + "ea/scBudget/ea_toPlanCostBudgetBillReceiveList.jspa?menuId=" + menuId;
        }else{
            window.location.href = basePath + "ea/scBudget/ea_toPlanCostBudgetBillList.jspa?menuId=" + menuId;
        }
    }
