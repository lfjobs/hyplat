    ...
    <section class="sec-list">
        ...
        <div class="div-sec-data"
            ...
            <div class="data-scroll-container" style="max-height: calc(100vh - 200px); width: max-content;">
                ...
                <!-- 数据列表 -->
                <div class="data-list div-dataProcess" style="overflow: auto;font-size: 15px;">
                    <!-- 数据项 -->
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (getParameterByName('type') == 'isProcess') {
            ...
            if (options.length > 0) {
                ...
                options[2].style.color = '#16baaa';
                options[2].style.borderBottom = '2px solid #16baaa';
                options[2].style.fontweight = 'bold';
            }
            ...
            if (options.length > 0) {
                // options[0].style.backgroundColor = '#4a8e4a';
                options[3].style.color = '#16baaa';
                options[3].style.borderBottom = '2px solid #16baaa';
                options[3].style.fontweight = 'bold';
            }
    ...
    function getEntryStaffData() {
        ...
        $.ajax({
            ...
            success: function cbf(data) {
                ...
                let codeList;
                try {
                    codeList = typeof data === 'string' ? JSON.parse(data) : data;
                } catch (e) {
                    console.error('数据解析失败:', e);
                    $(".div-dataProcess").html("数据解析失败");
                    return;
                }
                let dataList = Array.isArray(codeList) ? codeList : (codeList.list || []);
                const codeListNew = [];
                if (type == "isProcess") {
                    const processedList = dataList.filter(item => {
                        return item[6] === '1';
                    });
                    codeListNew.push(processedList);
                } else if (type == "noProcess") {
                    const unprocessedList = dataList.filter(item => {
                        return item[6] !== '1';
                    });
                    codeListNew.push(unprocessedList);
                }
                if (!codeListNew[0] || codeListNew[0].length === 0) {
                    $(".div-dataProcess").html("暂无数据");
                    $(".div-dataProcess").css({
                        "display": "flex",
                        "align-items": "center",
                    ...
                    const list = codeListNew[0];
                    pageCount = (typeof codeList === 'object' && codeList.pageCount) ? codeList.pageCount : 1;
                    ...
                    if (pageNumber > 1) {
                        $(".div-dataProcess").append(htmlstr.join(""));
                    } else {
                        $(".div-dataProcess").html(htmlstr.join(""));
                    }
                    $(".div-dataProcess").css({"display": "block"});
                }
                scrollBool = true;
            },
