...
<div class="content" style="width: 100%;">
    <s:form id="updateTrilateralData" action="ea_UpdateTrilateralData.jspa" namespace="/ea/trilateral" method="POST"
            enctype="multipart/form-data" onsubmit="return validateForm()">
    <div class="div-name">
        <p style="display: none">
            <label><span style="color: #f74c32">*</span>id</label>
            <input required type="text" placeholder="请填写姓名" name="trilateralData.id" id="id"
                   value="${trilateralData.id}"/>
        ...
        <p style="display: none">
            ...
            <input style="width: 100%;" type="text" placeholder="请填写公司名称"
                   ...
                   id="companyId" value="${trilateralData.companyId}"/>
        </p>
        <p>
            <label>人员编号</label>
            <input style="width: 100%" type="text" placeholder="请填写人员编号" name="trilateralData.personnelID"
                   id="personnelID" value="${trilateralData.personnelID}"/>
        </p>
        <p>
        ...
        <p>
            ...
            <input style="width: 100%" required type="text" placeholder="请填写手机号" name="trilateralData.phone"
                   id="phone" value="${trilateralData.phone}"/>
        </p>
        <p>
        ...
        <p>
            ...
            <input style="width: 100%" required type="text" placeholder="请填写密码" name="trilateralData.pwd"
                   id="pwd" value="${trilateralData.pwd}"/>
        </p>
        <p>
            ...
            <input style="width: 100%" required type="text" placeholder="微信 小红书 抖音...."
                   ...
                   id="uploadProject" value="${trilateralData.uploadProject}"/>
        </p>
        <p>
            <label>事由</label>
        ...
        <p>
            ...
            <select style="padding: 8px; box-sizing: border-box;"
                ...
                <option value="0" ${trilateralData.distributionState eq '0' ? 'selected' : ''}>未分配</option>
                <option value="1" ${trilateralData.distributionState eq '1' ? 'selected' : ''}>已分配</option>
            </select>
        </p>
            <%--        <p>--%>
            <%--            <label>服务跟踪</label>--%>
        ...
        <p>
            ...
            <input style="width: 100%" type="text" placeholder="请填写私信内容" name="trilateralData.privateMessage"
                   id="privateMessage" value="${trilateralData.privateMessage}"/>
        </p>
        ...
        <p>
            ...
            <select style="padding: 8px; box-sizing: border-box;"
                    ...
                    id="audit1" disabled>
                <option value="0" ${trilateralData.audit1 eq '0' ? 'selected' : ''}>未审核</option>
                <option value="1" ${trilateralData.audit1 eq '1' ? 'selected' : ''}>已审核</option>
            </select>
        </p>
        <p>
            <label>托管</label>
            <select style="padding: 8px; box-sizing: border-box;"
                ...
                <option value="1" ${trilateralData.trusteeship eq '1' ? 'selected' : ''}>是</option>
                <option value="2" ${trilateralData.trusteeship eq '2' ? 'selected' : ''}>否</option>
            </select>
        </p>
        <p>
            <label>上传</label>
            <select style="padding: 8px; box-sizing: border-box;"
                ...
                <option value="1" ${trilateralData.isUpload eq '1' ? 'selected' : ''}>是</option>
                <option value="2" ${trilateralData.isUpload eq '2' ? 'selected' : ''}>否</option>
            </select>
        </p>
        ...
        <div class="div-img clearfix">
            <div class="div-left">
                ...
                <input type="text"
                       name="trilateralData.attachmentAddress"
                       value="${trilateralData.attachmentAddress}"
                       id="attachmentAddress"
                       disabled
                       placeholder="支持doc/docx,xls/xlsx"/>
            </div>
        ...
        <p>
            <label style="color: #0a84ff" onclick="viewAttachment()">查看附件</label>
        </p>
        <div class="div-name1 div-baseData div-contract ">
            <label>发布的小视频</label>
            <div id="videos">
            </div>
        </div>
    ...
    <div class="auditResult" style="display: none;">
        <label>审核结果&nbsp&nbsp:</label>
    </div>
...
<script>
    ...
    var pageNumber = 1, totalPages = 1, loading = false, pageSize = 10;
    var localVideoList = [];
    var tab = 1;
    const staffID = "${trilateralData ne null and trilateralData.staffId ne null ? trilateralData.staffId : ''}"; // 统一使用 staffID 变量
    const videoStaffId = "${trilateralData ne null and trilateralData.staffId ne null ? trilateralData.staffId : ''}";
    var isScrollHandling = false;  // 防止滚动处理重复执行
    var scrollTimer;
    ...
    $(function () {
        ...
        if (getParameterByName("type") === "search") {
            $(".submit-button-container").hide();
            $(".auditResult").show();
            auditResult();
        }
        if (getParameterByName("type") === "audit1") {
            $(".submit-button-container input[type='submit']").attr("type", "text").val("审核").css("padding", "12");
        }
    });
    ...
    function initCss() {
        ...
        if (getParameterByName("type") === "search") {
            $(".div-name").height($(window).height() - $(".submit-button-container").height() - 140);
        }
    ...
    function getDictDataListByType() {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                // 设置默认选中值
                var savedValue = "${trilateralData.registrationPlatformName}";
                if (savedValue) {
                    $("#registrationPlatformName").val(savedValue);
                }
            },
    ...
    function loadData() {
        // 添加多重检查防止重复执行
        if (loading || pageNumber > totalPages || isScrollHandling) {
            console.log("Load blocked: loading=" + loading + ", page=" + pageNumber + ", total=" + totalPages + ", handling=" + isScrollHandling);
            return;
        }
            ...
            success: function (data) {
                ...
                if (list.length === 0) {
                    if ($("#videos").children().length === 0) {
                        $("#videos").html("<div style='margin: 6px;color:#828b88' class='no-videos'>尚未发布视频</div>");
                    }
                } else {
                    var innerHtml = [];
                    $(".last").remove();
                    ...
                    for (let i = 0; i < list.length; i++) {
                        ...
                        if (tab === 1) {
                            // 发布的视频
                            innerHtml.push("<div id='rvideo-" + video.videoId + "' class='relative basis-1/3 w-full p-1' style='aspect-ratio: 3/4'>");
                            innerHtml.push("<a href='" + basePath + "ea/dsp/ea_getVideoViewListPage.jspa?videoId=" + video.videoId + "&videoStaffId=" + staffID + "&viewType=1&videoIndex=" + ((pageNumber - 1) * pageSize) + "&pageSize=" + pageNumber * pageSize + "'>");
                            innerHtml.push("<img style='width: 100%;aspect-ratio: 3/4' src='" + video.coverUrl + "' onerror=\"this.src='" + basePath + "images/ea/collage/dsp/pic_03.png'\"/>");
                            innerHtml.push("</a>");

                            innerHtml.push("<span class='absolute flex flex-row space-x-1 text-center bottom-3 left-2 bg-white-100 text-sm text-white w-full p-1'>");
                            innerHtml.push("<img style='width: 12px; height: 12px' src='" + basePath + "images/ea/collage/dsp/img-13.png'/>");
                            innerHtml.push("<span style='line-height: 12px'>" + video.likedCount + "</span>");
                            innerHtml.push("</span>");
                ...
                // 使用文档片段提高性能并避免潜在的DOM问题
                var fragment = document.createDocumentFragment();
                var tempDiv = document.createElement('div');
                tempDiv.innerHTML = innerHtml.join('');

                while (tempDiv.firstChild) {
                    fragment.appendChild(tempDiv.firstChild);
                }

                $("#videos")[0].appendChild(fragment);
                // 更新页面信息
    ...
    // 修复后的代码
    var scrollTimer;
    var isScrollHandling = false;  // 添加标志位防止重复处理

    $(window).off('scroll').on('scroll', function () {
        clearTimeout(scrollTimer);
        scrollTimer = setTimeout(function () {
            if (!isScrollHandling && !loading && pageNumber <= totalPages) {
                var scrollTop = $(window).scrollTop();
                var windowHeight = $(window).height();
                var documentHeight = $(document).height();

                if (scrollTop + windowHeight >= documentHeight - 100) {
                    isScrollHandling = true;  // 设置处理标志
                    loadData();
                }
            }
        }, 300);  // 增加延迟时间以减少触发频率
    ...
    function viewAttachment() {
        ...
        $.ajax({
            url: basePath + "ea/trilateral/sajax_ea_getPath.jspa",
            type: "post",
            dataType: "json",
            data: {
                "trilateralData.attachmentAddress": $("#attachmentAddress").val()
            },
            success: function cbf(data) {
                if (data !== '') {
                    // 确保返回的是服务器路径而不是本地文件系统路径
                    var fullPath = basePath + data;  // 使用basePath构建完整URL

                    // 根据文件类型决定如何打开
                    var fileExtension = data.toLowerCase().split('.').pop();
                    var supportedPreviewTypes = ['pdf', 'jpg', 'jpeg', 'png', 'gif'];
    ...
    $(function () {
        // 检测按钮类型是否为 text
        $(document).on("click", ".submit-button-container input[type='text']", function () {
            layer.confirm('请选择审核结果：', {
                btn: ['同意', '不同意'],
                title: '审核确认:'
            }, function (index) {
                openAuditPrompt("同意", index);
            }, function (index) {
                openAuditPrompt("不同意", index);
            });
        });
        ...
        /**
         * 打开审核意见输入框
         * @param {string} auditStatus - 审核状态（同意/不同意）
         * @param {number} confirmIndex - 确认框的索引
         */
        function openAuditPrompt(auditStatus, confirmIndex) {
            layer.prompt({
                formType: 2, // 输入框类型：2 表示多行文本
                title: '请输入' + auditStatus + '审核意见',
                area: ['400px', '200px'], // 自定义宽高
                maxlength: 200 // 限制最大字符数
                ...
                // 提交审核意见到后台
                $.ajax({
                    url: basePath + "ea/trilateral/sajax_ea_submitAuditOpinion.jspa",
                    type: "POST",
                    data: {
                        "id": "${trilateralData.id}",
                        "auditOpinion": value,
                        "auditStatus": auditStatus // 传递审核状态
                    },
                    dataType: "json",
                    success: function (response) {
                        layer.close(promptIndex); // 关闭输入框
                        layer.close(confirmIndex); // 关闭确认框
                        layer.msg(response + '！状态：${auditStatus}');
                        setTimeout(function () {
                            document.location.href = basePath + "page/WFJClient/pc/5l5C/office/config/trilateralDataAudit.jsp";
                        }, 2000); // 2000毫秒 = 2秒
                    },
                    error: function () {
                        layer.msg("审核意见提交失败，请重试！");
                    }
                });
            })
        }
    ...
    function auditResult() {
        if (getParameterByName("type") === "search") {
            $.ajax({
                url: basePath + "ea/trilateral/sajax_ea_auditResult.jspa",
                type: "POST",
                data: {
                    "id": "${trilateralData.id}"
                },
                success: function (data) {
                    ...
                    if (list && list.length > 0) {
                        // 构建审核结果内容
                        let resultHtml = []; // 声明为数组
                        list.forEach(item => {
                            resultHtml.push("<li>");
                            resultHtml.push("审核人: " + item.auditStaffName);
                            let auditStatusText = "待审核";
                            ...
                            resultHtml.push("&nbsp&nbsp&nbsp意见: " + auditStatusText);
                            resultHtml.push("&nbsp&nbsp&nbsp详情: " + item.auditOpinion);
                            resultHtml.push("&nbsp&nbsp&nbsp时间: " + item.updateTime);
                            resultHtml.push("</li>");
                        });

                        // 将数组转换为字符串并插入到页面中
                        resultHtml = '<label style="color: #e76161">审核结果：</label><ul style="height: 200px">' + resultHtml.join('') + '</ul>';
                        $(".auditResult").html(resultHtml).css({
                            'min-height': '150px',
                            'height': 'auto',
                            'overflow-y': 'auto',
                            'display': 'block'
                        ...
                        // 如果没有数据，显示提示信息
                        $(".auditResult").html('<label>暂无审核结果</label>').show();
                    }
                },
                error: function () {
                    layer.msg("审核意见提交失败，请重试！");
