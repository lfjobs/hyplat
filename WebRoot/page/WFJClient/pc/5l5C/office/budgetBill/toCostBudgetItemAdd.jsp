...
<header>
    <ul class="clearfix">
        ...
        <li>
            ${(billsType=='收入' || billsType=='支出') ? '初始项目':billsType}明细添加
        </li>
    </ul>
</header>
<div class="content">
    <s:form id="f2" action="ea_toAddCostBudgetBill.jspa" namespace="/ea/scBudget" method="POST">
...
<%--下拉控制js文件--%>
<script type="text/javascript" src="<%=basePath%>js/ea/budgetBill/toCostBudgetItemAdd.js"></script>
<%--调用安卓js文件--%>
...
<script>
    ...
    function toAdd() {
        ...
        //     return false;
        // }
        var url = basePath + "ea/scBudget/ea_toAddCostBudgetBill.jspa?billsType=" + billsType+"&head="+head;
        if(editFlag == "edit"){
            url = basePath + "ea/scBudget/ea_toUpdateCostBudgetBill.jspa?cashierBillsId="+cashierBillsId + "&billsType=" + billsType+"&head="+head
        }
        $("#f2").attr("action",url);
                        ...
                        wx.scanQRCode({
                            ...
                            success: function (res) {
                                ...
                                if(result.indexOf("http")!=-1){
                                    ...
                                    var url = "ea_scanningCostBudgetInfo.jspa?editFlag="+editFlag+"&cashierBillsId="+cashierBillsId+"&head="+head;
    ...
    function toBarCodeInfo(goodsBillsID) {
        window.location.href = basePath + "ea/scBudget/ea_toCostBudgetItemToUpdate.jspa?keyNum=" + goodsBillsID + "&editFlag=addPage&showFlag=${showFlag}&departmentID=${departmentID}&cashierBillsId=${cashierBillsId}";
    }
    ...
    function toRemoveGoodsBills() {
        ...
        if(id){
            var url = "ea_removeGoodsBillOfToAdd.jspa?keyNum=" + id+"&editFlag=add";
    ...
    function toBack() {
        if(editFlag == "edit"){
            window.location.href = basePath + "ea/scBudget/ea_toUpdateCostBudgetBill.jspa?commitFlag='no'&cashierBillsId="+cashierBillsId+"&billsType="+billsType+"&head="+head;
       }else{
            window.location.href = basePath + "ea/scBudget/ea_toAddCostBudgetBill.jspa?commitFlag='no'&billsType="+billsType+"&head="+head;
        }
    }
