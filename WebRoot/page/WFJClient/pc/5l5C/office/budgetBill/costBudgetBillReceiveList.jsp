            ...
            <c:forEach items="${pageForm.list}" var = "entity" varStatus="v">
                <li class="clearfix<c:if test="${v.index+1 ne fn:length(pageForm.list)}"> ttsw_last</c:if>">
                    <section id="sec-checked">
                        <aside class="aside_no" checkCasId="${entity.cashierBillsID}" cashierBillsStatus="${entity.status}">
                        ...
                        <p class="un">单据状态：
                            <span>
                                ...
                                <c:if test="${empty entity.paystatus}">
                                    ...
                                    <c:if test="${entity.status eq '15'}">已入库</c:if>
                                    <c:if test="${entity.status eq '16'}">已出库</c:if>
                                </c:if>
                            </span>
                        </p>
                    </div>
...
<%--            <span class="close">&times;</span>--%>
            <p style="font-size: 16px;">确认审批或驳回？</p>
            <div style="position: relative;width:280px;left:60px;top:30px;">
                <button onclick="buttonClicked('AGREE')" style="width:80px;height: 30px;">同意</button>
                <button onclick="buttonClicked('REJECT')" style="width:80px;height: 30px;">驳回</button>
                <button onclick="hideDialog()" style="width:80px;height: 30px;">取消</button>
                <input type="hidden" id="approvalId" value="" />
            </div>
        </div>
    </div>
</body>
...
<%--异步上拉加载js文件--%>
<script type="text/javascript" src="<%=basePath%>js/ea/budgetBill/costBudgetReceiveList.js"></script>
<script>
    //跳转详情
    function toDetail(id){
        var url = "ea/scBudget/ea_toCostBudgetDetail.jspa";
        var parameter = "?showFlag="+showFlag+"&menuId="+menuId+"&cashierBillsId="+id+"&billsType="+billsType+"&head="+head;
    ...
    function toUpdate(){
        ...
        if(id != "" && id != null){
            ...
            var url = "ea/scBudget/ea_toUpdateCostBudgetBill.jspa";
            var parameter = "?showFlag="+showFlag+"&departmentID="+departmentId+"&cashierBillsId="+id+"&menuId="+menuId+"&billsType="+billsType+"&head="+head;
            if(showFlag){//单独部门传部门名称过去
                parameter += "&departmentName="+departmentName;
            }
            window.location.href = basePath + url + parameter;
    ...
    function buttonClicked(option) {
        var url = basePath + "ea/scBudget/sajax_ea_approval.jspa";
        $.ajax({
            url: url,
            type: "post",
            async: false,
            dataType: "json",
            data: {
                "approvalResult" : option,
                "cashierBillsId" : $("#approvalId").val()
            },
            success: function(data) {
                ...
                modal.style.display = "none";
                window.location.href = basePath + "ea/scBudget/ea_toCostBudgetBillReceiveList.jspa?menuId=receive&billsType="+billsType+"&head="+head;
            },
            error: function () {
                alert("审批失败！");
            }
    ...
    function delCostSheet(){
        ...
        if(id != "" && id != null){
            ...
            if (r == true) {
                var url = "ea/scBudget/ea_delCostBudgetSheet.jspa?cashierBillsId="+id+"&menuId="+menuId+"&billsType="+billsType+"&head="+head;
                window.location.href = basePath + url;
            }
        }else{
            alert("请选择要删除的数据");
