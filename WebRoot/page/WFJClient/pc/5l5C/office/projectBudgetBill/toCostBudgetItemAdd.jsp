...
<header>
    <ul class="clearfix">
        ...
        <li>
            ${(billsType=='收入' || billsType=='支出') ? billsType.concat('单'):billsType}明细添加
        </li>
    </ul>
</header>
<div class="content">
    <s:form id="f2" action="ea_toAddProjectBill.jspa" namespace="/ea/scBudget" method="POST">
...
<%--下拉控制js文件--%>
<script type="text/javascript" src="<%=basePath%>js/ea/projectBudgetBill/toCostBudgetItemAdd.js"></script>
<%--调用安卓js文件--%>
...
<script>
    ...
    //点击审核提交
    function toAdd() {
        var url = basePath + "ea/scBudget/ea_toAddProjectBill.jspa?billsType=" + billsType;
        if(editFlag == "edit"){
            url = basePath + "ea/scBudget/ea_toUpdateProjectBill.jspa?cashierBillsId="+cashierBillsId + "&billsType=" + billsType;
        }
        $("#f2").attr("action",url);
        $("#f2").submit();
    }
                        ...
                        wx.scanQRCode({
                            ...
                            success: function (res) {
                                ...
                                if(result.indexOf("http")!=-1){
                                    ...
                                    var url = "ea_scanningProjectInfo.jspa?editFlag="+editFlag+"&cashierBillsId="+cashierBillsId;
    ...
    function toBarCodeInfo(goodsBillsID) {
        window.location.href = basePath + "ea/scBudget/ea_toProjectItemToUpdate.jspa?keyNum=" + goodsBillsID + "&editFlag=addPage&showFlag=${showFlag}&departmentID=${departmentID}&cashierBillsId=${cashierBillsId}";
    }
    ...
    function toRemoveGoodsBills() {
        ...
        if(id){
            var url = "ea_removeProjectGoodsBillOfToAdd.jspa?keyNum=" + id+"&editFlag=add";
