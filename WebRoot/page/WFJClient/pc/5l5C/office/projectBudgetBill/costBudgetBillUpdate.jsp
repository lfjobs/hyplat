...
<script>
    ...
    var cashierBillsBillsType = "${cashierBills.billsType}";
...
<header>
    <ul class="clearfix">
        ...
        <li id="title">
            ${(billsType=='收入' || billsType=='支出') ? billsType.concat('单'):billsType}修改
        </li>
    </ul>
</header>
<div class="content">
    <s:form id="f2" action="ea_updateProjectSheet.jspa" namespace="/ea/scBudget" method="POST">
        <input type="hidden" id="cashierBillsId" name="cashierBills.cashierBillsID" value="${cashierBills.cashierBillsID}"/><%--已添加预算单id（修改用）--%>
        <input type="hidden" id="ttsw_hid_delGoods2" name="delGoodsBillsIds"
               value="${delGoodsBillsIds}"/><%--修改页面删除已保存的货物id数组--%>
        <section class="sec_con1">
            <p>
                ...
                <input type="text" id="cashierBillsKey" name="cashierBills.cashierBillsKey"
                       value="${cashierBills.cashierBillsKey}"
                       style="display: none;"/><%--主键id--%>
            </p>
            ...
            <div class="clearfix">
                <p>
                    ...
                    <input type="text" id="ttsw_companyNmae" name="cashierBills.companyName" class="gs_name"
                           value="${cashierBills.companyName}" placeholder="" required readonly/>
                </p>
                    <%--所选分部门则显示不可修改--%>
                <c:if test="${showFlag eq true}">
                    <p>
                        ...
                        <input type="text" name="cashierBills.departmentName" id="ttsw_dep_y_name"
                               value="${cashierBills.departmentName}" placeholder="请选择部门" required readonly/>
                        <input type="text" name="cashierBills.departmentID" id="ttsw_dep_y_id"
                               value="${cashierBills.departmentID}" readonly style="display: none;"/><%--所选部门id--%>
                    </p>
                </c:if>
                    <%--所选为总列表，则显示下来自己选择--%>
                <c:if test="${showFlag eq false}">
                    <p class="clearfix">
                        ...
                        <input type="text" id="ttsw_dep_n_name" name="cashierBills.departmentName"
                               value="${empty costBaseBean.departmentName ? cashierBills.departmentName:costBaseBean.departmentName}" required readonly/>
<%--                        <input type="text" class="csbm_xiala-1" id="ttsw_dep_n_name" name="cashierBills.departmentName"--%>
<%--                               value="${empty costBaseBean.departmentName ? cashierBills.departmentName:costBaseBean.departmentName}"--%>
<%--                               placeholder="请选择部门" required readonly/>--%>
<%--                        <input type="text" class="csbm_id_xiala-1" id="ttsw_dep_n_id" name="cashierBills.departmentID"--%>
<%--                               value="${empty costBaseBean.departmentID ? cashierBills.departmentID:costBaseBean.departmentID}"--%>
<%--                               readonly style="display: none;"/>&lt;%&ndash;所选部门id&ndash;%&gt;--%>
<%--                        <img src="<%=basePath %>images/scMobile/payBudget/addBudget/wupinguanli_img_13.png"/>--%>
            ...
            <div class="clearfix">
                <p class="clearfix">
                    ...
                    <input type="text" name="cashierBills.tradeName" id="hyfl"
                           value="${empty costBaseBean.tradeName ? cashierBills.tradeName:costBaseBean.tradeName}"
                           placeholder="请选择行业分类" required readonly/>
                    <input type="hidden" name="cashierBills.tradeId" id="hyflId"
                           value="${empty costBaseBean.tradeId ? cashierBills.tradeId:costBaseBean.tradeId}"
                           required readonly/>
                    <img src="<%=basePath %>images/scMobile/payBudget/addBudget/wupinguanli_img_13.png"/>
                </p>
                <p class="clearfix">
                    <label for=""><span>*</span>项目分类： </label>
                    <input type="text" name="cashierBills.xmtypename" id="xiangmu"
                           value="${empty costBaseBean.xmTypeName ? cashierBills.xmtypename:costBaseBean.xmTypeName}"
                           placeholder="请选择项目分类" required readonly/>
                    <input type="hidden" name="cashierBills.xmtype" id="xiangmuId"
                           value="${empty costBaseBean.xmType ? cashierBills.xmtype:costBaseBean.xmType}"
                           required readonly/>
                    <img src="<%=basePath %>images/scMobile/payBudget/addBudget/wupinguanli_img_13.png"/>
                </p>
            </div>
            <input class="searchBtn" type="button" value="行业搜索" onclick="searchBusinessType();"/>
            <div class="clearfix" id="inv">
                <p class="clearfix">
                    ...
                    <input type="text" class="inv_name" id="ttsw_inv_n_name" name="cashierBills.dataDepotName"
                           value="${empty cashierBills.dataDepotName ? "" : cashierBills.dataDepotName}" placeholder="请选择仓库" required readonly/>
                    <input type="text" class="inv_id" id="ttsw_inv_n_id" name="cashierBills.dataDepotID"
                           value="${empty cashierBills.dataDepotID ? "" : cashierBills.dataDepotID}" readonly style="display: none;"/>
                    <img src="<%=basePath %>images/scMobile/payBudget/addBudget/wupinguanli_img_13.png"/>
                </p>
            ...
            <div class="clearfix">
                <p class="clearfix">
                    ...
                    <input type="text" class="xiala-1"
                           value="${empty costBaseBean.staffName ? cashierBills.staffName:costBaseBean.staffName}(${empty costBaseBean.staffCode ? "":costBaseBean.staffCode})"
                           placeholder="" required readonly/>
                    <input type="text" name="cashierBills.staffID" class="ttsw_emp_id"
                           value="${empty costBaseBean.staffFzrId ? cashierBills.staffID:costBaseBean.staffFzrId}"
                           style="display: none;"/>
                    <input type="text" name="cashierBills.staffName" class="ttsw_emp_name"
                           value="${empty costBaseBean.staffName ? cashierBills.staffName:costBaseBean.staffName}"
                           style="display: none;"/>
                    <input type="text" name="cashierBills.staffCode" class="ttsw_emp_code"
                           value="${empty costBaseBean.staffCode ? cashierBills.staffCode:costBaseBean.staffCode}"
                           style="display: none;"/>
<%--                    <img src="<%=basePath %>images/scMobile/payBudget/addBudget/wupinguanli_img_13.png"/>--%>
                </p>
                <p>
                    ...
                    <input type="text" name="" id="ttsw_singleName"
                           value="${empty costBaseBean.singleName ? cashierBills.inputName:costBaseBean.singleName}"
                           placeholder="自动获取" required readonly/>
                </p>
<%--                <p>--%>
<%--                    <label for=""><span></span>位置：</label>--%>
<%--                    <input type="text" name="" id="address" name="cashierBills.address" value="金牛区红星美凯龙"--%>
<%--                           required readonly/>--%>
<%--                    <input type="text" name="" id="coordinate" style="display: none;" name="cashierBills.coordinate"--%>
...
<%--                </p>--%>
            ...
            <div class="clearfix">
                ...
                <p>
                    ...
                    <input type="text" name="cashierBills.ctUserName" value="${cashierBills.ctUserName}" placeholder="" />
                </p>
                <p style="display: none">
                    <label for="">开始日期： </label>
                    <input type="text" name="cashierBills.startTime" value="${cashierBills.startTime}" placeholder=""/>
                </p>
                <p style="display: none">
                    <label for="">结束日期： </label>
                    <input type="text" name="cashierBills.endTime" value="${cashierBills.endTime}" placeholder="" />
                </p>
                <p>
                    ...
                    <c:if test="${!empty cashierBills.status}">
                        ...
                        <c:if test="${cashierBills.status eq '02'}"><input type="text" value="已通过-招标前"
                                                                           readonly/></c:if>
                        <c:if test="${cashierBills.status eq '03'}"><input type="text" value="比价审核中"
                                                                           readonly/></c:if>
                        <c:if test="${cashierBills.status eq '04'}"><input type="text" value="已提交资金申请"
                                                                           readonly/></c:if>
                        <c:if test="${cashierBills.status eq '05'}"><input type="text" value="待会计审核"
                                                                           readonly/></c:if>
                        <c:if test="${cashierBills.status eq '06'}"><input type="text" value="待出纳审核"
                                                                           readonly/></c:if>
                        <c:if test="${cashierBills.status eq '07'}"><input type="text" value="已审核" readonly/></c:if>
                        <c:if test="${cashierBills.status eq '20'}"><input type="text" value="税务单据"
                                                                           readonly/></c:if>
                        <c:if test="${cashierBills.status eq '08'}"><input type="text" value="三审已归档"
                                                                           readonly/></c:if>
                        <c:if test="${cashierBills.status eq '09'}"><input type="text" value="待确认收款"
                                                                           readonly/></c:if>
                        ...
                        <c:if test="${cashierBills.status eq '46'}"><input type="text" value="系统生成"
                                                                           readonly/></c:if>
                        <c:if test="${cashierBills.status eq '11'}"><input type="text" value="驳回待修改"
                                                                           readonly/></c:if>
            ...
            <div class="clearfix">
                <section class="headNav"  id="billsTypeSection">
                    <ul>
                        <li><a href="javascript:nav('采购单')"><img id="srdj"
                                                                      src="<%=basePath%>images/ea/office/contract/selectp/dxseleted.png">采购单</a>
                        </li>
                        <li><a href="javascript:nav('销售单')"><img id="zcfb"
                                                                      src="<%=basePath%>images/ea/office/contract/selectp/dxwselet.png">销售单</a>
                        </li>
                    </ul>
                    <p>
                        <input type="text" id="billsType" name="cashierBills.billsType"
                               value="${empty cashierBills.billsType ? '采购单' : cashierBills.billsType}" /><%--所选项目类型--%>
                    </p>
                </section>
<%--                <c:if test="${menuId eq 'ng' or menuId eq 'cy'}">--%>
...
<%--                </c:if>--%>
            ...
            <c:if test="${!empty cashierBillsExt.reviewerName }">
                <div class="clearfix">
                    <p>
                        <label for="">审核人： </label>
                        <input type="text" class="reviewerName" id="reviewerName" name="cashierBillsExt.reviewerName"
                               value="${empty cashierBillsExt.reviewerName ? '' : cashierBillsExt.reviewerName}" readonly/>
                    </p>
                    <p>
                        <label for="">审核时间： </label>
                        <input type="text" name="cashierBillsExt.reviewTime" value="${cashierBillsExt.reviewTime}" readonly/>
                    </p>
                </div>
            </c:if>
        </section>
        <section class="sec_con2">
                    ...
                    <c:forEach items="${scanningMap}" var="entity" varStatus="v">
                        <li class="clearfix" id="${entity.key}">
                            <aside class="aside_no">
                                <img class="img_no"
                                     src="<%=basePath %>images/scMobile/payBudget/addBudget/wupinguanli_07.png"/>
                                <img class="img_yes"
                                     src="<%=basePath %>images/scMobile/payBudget/addBudget/wupinguanli_10.png"/>
                            </aside>
                            <section class="clearfix">
                                ...
                                <ul>
                                    ...
                                    <img onclick="toRemove(${entity.key});" class="img_del"
                                         src="<%=basePath %>images/scMobile/payBudget/addBudget/yusuandan_01_del.png"/>
                                </ul>
                            </section>
            ...
            <div class="clearfix">
                ...
                <div class="right">
                    <p id="commit" onclick="toUpdate();">提交</p>
                </div>
            </div>
        </section>
    </s:form>
...
<s:form id="f1" action="" namespace="/ea/scBudget" method="POST">
    <input type="hidden" name="costBaseBean.cashierBillsId" value="${cashierBills.cashierBillsID}"/><%--已添加预算单id（修改用）--%>
    <input type="hidden" name="costBaseBean.cashierBillsKey" value="${cashierBills.cashierBillsKey}"/><%--已添加预算单id（修改用）--%>
    ...
    <input type="hidden" id="ttsw_hid_itemName" name="costBaseBean.itemName" value=""/><%--项目名称--%>
    <input type="hidden" id="ttsw_hid_itemType" name="costBaseBean.itemType" value=""/><%--项目分类--%>
    ...
    <input type="hidden" name="costBaseBean.identification" value="update"/><%--跳转页面标识--%>
    <input type="hidden" id="ttsw_hid_delGoods" name="delGoodsBillsIds"
           value="${delGoodsBillsIds}"/><%--修改页面删除已保存的货物id数组--%>
</s:form>
</body>
<%--下拉控制js文件--%>
<script type="text/javascript" src="<%=basePath%>js/ea/projectBudgetBill/costBudgetBillUpdate.js"></script>
<%--调用安卓js文件--%>
<script type="text/javascript" src="<%=basePath %>js/ea/projectBudgetBill/productslaunch.js"></script>
<script>
    ...
    function toBarCodeInfo(goodsBillsID) {
        window.location.href = basePath + "ea/scBudget/ea_toProjectItemUpdate.jspa?keyNum=" + goodsBillsID + "&editFlag=edit&showFlag=${showFlag}&departmentID=${departmentID}&cashierBillsId=${cashierBillsId}&menuId="+menuId+"&billsType="+billsType;
    }

    //根据id删除以保存数据
    function toRemoveGoodsBills(goodsBillsId) {
        ...
        if(id){
            document.location.replace(basePath + "ea/scBudget/ea_removeProjectGoodsBillOfUpdate.jspa?keyNum=" + id+"&editFlag=edit&cashierBillsId="+cashierBillsId+"&menuId="+menuId+"&billsType="+billsType);
        }
