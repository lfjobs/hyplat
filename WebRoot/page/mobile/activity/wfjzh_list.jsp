...
<div class="main_main">
            ...
            <c:forEach items="${pageForm.list}" var="a">
                <tr id="${a[0]}">
                    <td>
						...
						<input type="hidden" name="staffid" id="staffid" value="${a[5]}">
                    </td>
                    <td ><span><%=number%></span></td>
                    <td><span id="s_account">${a[1]}</span></td>

                    <td><span id="author">${a[2]}</span></td>
                    <td><span id="logoNote">${a[3]}</span></td>
                    <td><span>${a[4]=='1'?'个人会员':a[4]=='2'?'企业会员':null}</span></td>
                </tr>
                <%  number++; %>
            </c:forEach>
    ...
    <c:import url="../../ea/page_navigator.jsp">
        <c:param name="actionPath"
                 value="ea/wfjzh/ea_getlist.jspa?mz=${mz}&zh=${zh}&type=${type}&isdx=${isdx}">
        </c:param>
    </c:import>
    <s:token></s:token>
</div>
                    ...
                    case '查询':
                        ...
                        $('#formtest')
                            .attr(
                                "action",
                                basePath
                                + "/ea/wfjzh/ea_getlist.jspa?isdx=${isdx}");
