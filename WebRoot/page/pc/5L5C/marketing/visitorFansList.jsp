...
<head>
    ...
    <script src="${urlPrefix}/js/pc/5L5C/marketing/branch/visitorFans.es.js" type="module"></script>
    <title>标准模板</title>
</head>
...
<article class="layout-flex-column mobile-screen" >
    ...
    <main class="page-visitor-fans layout-flex-content" id="main" >
        <nav class="nav-href">
            <ul class="ul-link">
                <li>批量导入</li>
                <li>群发短信</li>
...
<template id="template">
    <section class="table-box" id="tableBox">
        <table class="table-nowrap table-cool-theme" @mounted="onMounted()">
            <thead>
            <tr>
                <th>序号</th>
                <th v-for="item of tableFields">{{item}}</th>
            </tr>
            </thead>
            <tbody>
                <tr v-for="(item,index) of tableRows" :key="item.id">
                    <td>{{index+1}}</td>
                    <td>{{item.name}}</td>
                    <td>{{item.number}}</td>
                    <td>{{item.contact}}</td>
