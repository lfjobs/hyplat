...
<script type="text/javascript">
	...
	$("#DaoRuFanqd").click(function(){// 选择确定
	  ...
	  $("#table4").find("span").each(function(){
	   $(this).text("");
		$("#clearcom").hide();
	   })
	   $("select#contactConnections").hide();
	   }

	   function fdxddfperson(){
	  $("#table5").find("span").each(function(){
	   $(this).text("");
		$("#clearperson").hide();
	   })
	   $("select#phone").hide();
      ...
      <tr>
        ...
        <table class="table" width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
          	...
          	<input type="text" STYLE="display:none" id="concertcompany"/>
            <td width="50">&nbsp;</td>
            <td width="65">气瓶编号</td>
            <td width="50">类型</td>
          ...
          <tr id="informationline">
            ...
            <td><input type="text" name = "carcymap[0].cylinderNum" class="textType" /><span>${obj[0]}</span><input id="carlinderID" style="display:none" value="${obj[41] }"/> </td>
            <td><select style="width:100%;" id="carlindertype" name = "carcymap[0].cylinderType" class="selecttype">
        <option value="--请选择--">--请选择--</option>
          <option value="G:钢瓶">G:钢瓶</option>
          <option value="C:缠绕瓶">C:缠绕瓶</option>
        </select><span> ${obj[1] }</span></td>
            <td><input type="text" class="textType" name="carcymap[0].designThickness" /><span> ${obj[2] }</span></td>
            <td>
            <select style="width:88px;" id="carlindermodel" name="carcymap[0].cylinderModel" style="display:none">
        </select>
        <span> ${obj[3] }</span>
        </td>
            <td><input type="text" class="textType" name="carcymap[0].manufactureCompany"/><span> ${obj[4] }</span></td>
            <td><input type="text" class="textType" onfocus="date(this)" name="carcymap[0].leavefactorydate"/><span>  ${fn:substring(obj[5], 0, 10)}</span></td>
            <td><input type="text" class="textType" name="carcymap[0].volume"/><span> ${obj[6] }</span></td>
            <td><input type="text" class="textType" name="carcymap[0].weight"/><span> ${obj[7] }</span></td>
            <td><input type="text" class="textType" name="carcymap[0].nominalworkpressure" value="20"/><span> ${obj[8] }</span></td>
            <td class="hydraulicTestPressure"><input type="text" class="textType" name="carcymap[0].hydraulicTestPressure" id="hydraulicTestPressure" readonly="readonly"/><span> ${obj[9] }</span></td>
            <td><input type="text" class="textType" name="carcymap[0].texture" value="30CrMo"/><span> ${obj[10] }</span></td>
            <td><input type="text" class="textType" name="carcymap[0].fiber"/><span> ${obj[11] }</span></td>
            <td><input type="text" class="textType" name="carcymap[0].resin"/><span> ${obj[12] }</span></td>
            <td><input type="text" class="textType" name="carcymap[0].manufacturingcountry" value="中国"/><span> ${obj[13] }</span></td>
            <td class="bor_r"><input type="text" class="textType" name="carcymap[0].madecode"/><span> ${obj[14] }</span></td>
            <td style="display:none"><input type="text" name="carCylinderInformation.status" value="01"/></td>
          </tr>
        </table>
        ...
        <td>
          <div class="tj" style="width:100%; float:left; height:80px;">
              ...
              <input name="" type="button" value="返回" style="margin-left:20px; margin-top:30px;" onclick="window.location.href='<%=basePath%>ea/productregister/ea_getListProductregister.jspa'"/>
          	  <input  class = "jquemaddprint" type="button" value="填写派工单" style="margin-top:30px;" onclick="window.location.href='<%=basePath%>page/ea/main/driving/signup/joborderAdd.jsp'"/>
          </div>
        </td>
      </tr>
     </table>
