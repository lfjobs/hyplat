...
<script type="text/javascript" src="<%=basePath%>js/ea/driving/sheethandle.js"></script>
		...
		<div id="main_main" class="main_main">
			<table class="JQueryflexme">
				<thead>
					<tr>
						...
						<th width="80" align="center">检验编号</th>
						...
						<th width="120" align="center">制造单位</th>
						<th width="100" align="center">出厂年月</th>
						<th width="80" align="center">登记证号</th>
						<th width="80" align="center">重量</th>
						<th width="80" align="center">容积</th>
				...
				<tbody>
					...
					<tr id="${item[11]}">
							...
							<td><span id="testNum">${item[0]}</span></td>
							<td><span id="cylinderNum">${item[1]}</span></td>
							
							<td><span id="staffname">${item[2]}</span></td>
							<td><span id="licensenumber">${item[3]}</span></td>
							<td><span id="cylinderModel">${item[4]}</span>
							</td>
							<td>
								<span id="manufactureCompany">${item[5]}</span>
							</td>
							<td>
								<span id="leavefactorydate">${fn:substring(item[6], 0, 10)}</span>
							</td>
							<td>
								<span id="certificateNumber">${item[7]}</span>
							</td>
							<td>
								<span id="weight">${item[8]}</span>
							</td>
							<td>
								<span id="volm">${item[9]}</span>
							</td>
							<td>
								<span id="cylinderType">${item[10]}</span>
							</td>
						</tr>
				</c:forEach>
				</tbody>
			...
			<c:import url="../../../page_navigator.jsp">
				<c:param name="actionPath"
					value="ea/dispitch/ea_getListorderwork.jspa?pageNumber=${pageNumber}&search=${search}"></c:param>
			</c:import>
		</div>
	</form>
	<!-- 查询窗口 -->
