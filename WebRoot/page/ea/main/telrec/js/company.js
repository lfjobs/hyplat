$(document).ready(function() {
	...
	function action(com, grid) {
		switch (com) {
			...
			case '设置每页显示条数' :
				...
				var url = basePath + "ea/tel/tel_companyRecords.jspa?search="
	...
	$("#btnDeal").click(function() {
		...
		if (dealContent) {
			$f = $('#cstaffForm');
			var url = basePath + "ea/tel/tel_dealTel.jspa?visitRecord.id="
					+ telMessageID + "&pageNumber=" + pNumber;
	...
	$("#searchStaff").click(function() {
		...
		$("#postSearchForm").attr(
				"action",
				basePath + "ea/tel/tel_searchCompany.jspa?pageNumber="
						+ pNumber);
...
function re_load() {
	if (token) {
		document.location.href = basePath
				+ "ea/tel/tel_companyRecords.jspa?pageNumber=" + pNumber
...
function lookMSG(msgID) {
	...
	$("#recordType").val($.trim($temp.find(".recordType").text()));
