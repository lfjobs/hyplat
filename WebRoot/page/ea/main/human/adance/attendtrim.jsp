...
<script type="text/javascript" src="<%=basePath%>js/flexigrid.js"></script>
<script src="<%=basePath%>js/ea/human/adance/attendtrim.js"></script>
...
<body>
	<form name="trimForm" id="trimForm" method="post">
		<s:token></s:token>
		...
		<input type="submit" name="submit" style="display:none" />
			<table class="trimtable">
				<thead>
					<tr>
						...
						<th width="100" align="center">审核</th>
						<th width="80" align="center">员工编号</th>
						<th width="80" align="center">员工姓名</th>
						<th width="120" align="center">所属部门</th>
						<th width="80" align="center">考勤日期</th>
						<th width="80" align="center">修改前项目</th>
						<th width="120" align="center">修改前考勤时间</th>
						<th width="150" align="center">修改后项目</th>
						<th width="150" align="center">修改后考勤时间</th>
						<th width="180" align="center">备注</th>
						<th width="120" align="center">审核人</th>
					</tr>
				</thead>
				<tbody id="tbwid">
					<s:iterator value="pageForm.list">
						<tr id="${attendTrimId}">
							...
							<td align="center">
								<c:if test="${audstate == '00'}">
								<a href="#" onclick="audit('01','${attendTrimId}')"> 通过</a>
								<a href="#" onclick="audit('02','${attendTrimId}')"> 驳回</a>
								</c:if>
								<c:if test="${audstate == '01'}"> 通过</c:if>
								<c:if test="${audstate == '02'}"> 驳回</c:if>
							</td>
							...
							<td><span id="oldStus">${oldStus }</span></td>
							<td><span id="oldtime">${oldtime}</span></td>
							<td><span id="newStus">${newStus }</span></td>
							<td><span id="newtime">${newtime}</span></td>
							<td><span id="remark">${remark }</span></td>
							<td><span id="audname">${audname }</span></td>
			...
			<c:import url="../../../page_navigator.jsp">
				<c:param name="actionPath"
					value="ea/attendtrim/ea_getAttendTrim.jspa?pageNumber=${pageNumber}&search=${search}">
				</c:param>
			</c:import>
		</div>
	</form>
