...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript"
			src="<%=basePath%>js/ea/office_ea/corporationcode/StampLog.js"></script>
		<script type="text/javascript"
			...
			<div id="main_main" class="main_main">
				<table class="registration">
					...
					<tbody id="tbwid">
						<!-- <tr id="sa" style="display: none" class="td_bg01 saveAjax model2">
							...
							<td class="td_bg01">
								<input name="stampDate" id="stampDate" onfocus="date(this);" />
							</td>
							<td class="td_bg01">
								<s:select list="%{#request.orgnizationList}" id="organizationID"
									listKey="organizationID" listValue="organizationName"
									name="corganizationID" theme="simple"></s:select>
							</td>
							<td class="td_bg01">
								<input name="documentsTheme" id="documentsTheme" />
							</td>
							<td class="td_bg01">
								<input name="contractors" id="contractors" />
							</td>
							<td class="td_bg01">
								<input name="stampCopies" id="stampCopies" />
							</td>
							<td class="td_bg01">
						...
						<s:iterator value="pageForm.list">
							<tr class="td_bg01 saveAjax" id="${stampLogID}">
								...
								<td class="td_bg01">
									<span id="docTitle">${docTitle}</span>

								</td>
								<!-- <td class="td_bg01" >							                                        
								<span id="corganizationID" class="datas" style="display:none">${corganizationID}</span>
								<span id="corganizationName" class="datas" >${corganizationName}</span>
								 <s:select cssStyle="display:none" list="%{#request.orgnizationList}" id="organizationID" listKey="organizationID" listValue="organizationName" name="corganizationID" theme="simple" ></s:select>
								
							</td>
							<td class="td_bg01">
								<span id="documentsTheme">${documentsTheme}</span>
								<input class="model1" value="${documentsTheme}" name="documentsTheme" />
							</td>
							<td class="td_bg01">
								<span id="contractors">${contractors}</span>
								<input class="model1" value="${contractors}"
									name="contractors" />
							</td>
							<td class="td_bg01">
								<span id="stampCopies">${stampCopies}</span>
								<input class="model1" value="${stampCopies}"
									name="stampCopies" />
							</td>
						    -->
								<td class="td_bg01">
									...
									<s:else>
										<span id="scanAttachs"><a
											href="javascript:OpenWord2('${scanAttach}', 3,'${ext}','auto')">查看</a>
										</span>
									</s:else>
				...
				<c:import url="../../../page_navigator.jsp">
					<c:param name="actionPath"
						value="ea/stamplog/ea_getListStampLog.jspa?stampLog.enterpriseStampID=${stampLog.enterpriseStampID}&pageNumber=${pageNumber}&search=${search}&gore=e&type=${type}">
					</c:param>
				</c:import>
			</div>
		</form>
			...
			<div class="jqmWindow" style="width: 300px; right: 25%; top: 10%"
				...
				<table id="cataffSearchTable">
					<tr>
						...
						<td>
							<input name="stampLog.gore" type="hidden" value="e" />
