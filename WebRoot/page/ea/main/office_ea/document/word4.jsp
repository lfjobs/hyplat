...
<%
	...
	SOAOfficeX.SOAOfficeCtrl SOACtrl = new SOAOfficeX.SOAOfficeCtrl(
			pageContext);
	SOACtrl.CanCopy = false;//设置页面是否可以复制；
	// 设置SOAOFFICE中间件服务页面
	...
	String formalNum = request.getParameter("formalNum");
	String fileTypedoc = "";
	...
	//设置保存文档的服务器页面
	SOACtrl.SaveDocURL = basePath + "ea/zoffice/ea_saveOffice.jspa?fileId="+fileId;
	SOACtrl.SaveHtmlURL = "SaveHtml.jsp";

	
	SOACtrl.webOpen(basePath + docPath, 2, userName, fileTypedoc);
	// -------- SOAOFFICE 服务器端编程结束 ------------------------ //
%>
...
<HTML>
	<HEAD>
		...
		<script type="text/javascript">
      	...
      	var fileId = '<%=fileId%>';  
      	function checkLeave() { 
      	         var formal = $("#formalnum").text();
      	         if(formal!=undefined&&formal!="无"&&formal!=""){
      	         event.returnValue="如需必要,请将正式编号复制到文档相应位置处"; 
					...
					<TR>
						...
						<TD style="font-size: 10pt" width="100%">
							...
							<font color="#FF0000"><input type="hidden"
									name="fileShowNameField" id="fileShowNameField" value="" /><%=fileShowNameField%></font>&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;<% if(!formalNum.equals("无")){%>正式编号：<font  color="#FF0000"><span id="formalnum"><%=formalNum%></span></font><font color="blue"> 请将正式编号复制到文档相应位置处。<% }%> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color="#FF0000">双击控件标题栏最大化运行</font>
						</TD>
						<TD width="6" bgcolor="#FFFFFF">
							&nbsp;
						</TD>
					...
					<TR vAlign="top">
						...
						<TD style="OVERFLOW-X: hidden" width="144" height="100%">
							<DIV class="mnpMenuTop" id="mnpMenuTop"
								...
								<DIV class="menuItem">
									<A href="javascript:jsInsertSeal();">插入印章</A>
								</DIV><!--
								<DIV class="menuItem">
									<A href="javascript:jsInsertLocalSeal();">插入本地印章</A>
								</DIV>
								-->
								<DIV class="menuItem">
									<A href="javascript:jsInsertSignature();">插入手写签名</A>
								</DIV>
								<DIV class="menuItem">
									<A href="javascript:jsValidateSeal();">验证印章/签名</A>
								</DIV>
								<DIV class=mnpMenuBorder style="WIDTH: 141px"></DIV>
								<%
									if (fileType == "W" || fileType.equals("W")) {
	...
	function SaveAsHtml()
	{
		...
		document.all("SOAOfficeCtrl").EnableFileCommand(3) = false;//保存 为什么不支持excel
		document.all("SOAOfficeCtrl").EnableFileCommand(1) = false;//打开
		document.all("SOAOfficeCtrl").EnableFileCommand(4) = false;//另存
	</SCRIPT>
