...
<html>
	<head>
		...
		<title>已审批公文</title>
		<script
			src="<%=basePath%>js/ea/office_ea/document/documentApproveyes.js"></script>
	<body>
		<!--已审批公文 -->
		...
		<div id="ysp">
					...
					<s:iterator value="pageForm.list">
						<tr class="docs" id="${docid}">
							...
							<td class="td_bg01">
								<c:choose>
									<c:when
										test='${outcome=="to Seal"}'>审批通过</c:when>
									<c:when test='${outcome=="Not Approve"}'>不批准</c:when>
								</c:choose>
							</td>

							<td class="td_bg01">
								<span id="examinetime">${fn:substring(examinetime,0,19)}</span>
							</td>
						</tr>
						<%
							number++;
			...
			<c:import url="../../../page_navigator.jsp">
				<c:param name="actionPath"
					value="ea/documentflow/ea_getExamineDocList.jspa?pageNumber=${pageNumber}&search=${search}&finishType=examineyes">
				</c:param>
			</c:import>
		</div>
