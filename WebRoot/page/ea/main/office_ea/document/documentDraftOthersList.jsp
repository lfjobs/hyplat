...
<html>
	<head>
		...
		<title>收件箱</title>
		<script
			src="<%=basePath%>js/ea/office_ea/document/documentDraftOthersList.js?version=20210220"></script>
		...
		<div id="draft">
			<table class="draft0">
				<thead>
					<tr>
						...
						<th class="thtitle" width="210" align="center">
							<span>文件标题</span>
						</th>
						...
						<tr class="docs" id="${docId}">
							<td class="td_bg01">
								<input type="checkbox" name="checkinput"
									class="JQuerypersonvalue" id="${docId}" value="${docId}" />
								<input type="hidden" value="${receiverID}" id="receiverID">
								<c:choose>
									<c:when test='${status=="R"}'>
										<img src="<%=basePath%>images/complaint/reject.jpg" width="30"
											height="16" />
							...
							<td class="td_bg01">
								<span id="passTime">${fn:substring(passTime,0,19)}</span>
							</td>
						</tr>
						<%
							number++;
			...
			<c:import url="../../../page_navigator.jsp">
				<c:param name="actionPath"
					value="ea/documentinfo/ea_getReceiveBoxList.jspa?pageNumber=${pageNumber}&search=${search}&searchType=${searchType}">
				</c:param>
			</c:import>
		</div>
