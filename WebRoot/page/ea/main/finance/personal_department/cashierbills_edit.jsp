...
<%@page import="hy.ea.bo.Company"%>
	<head>
		...
		<script language="javascript" type="text/javascript"
			src="<%=basePath%>js/ea/finance/cashierbills_edit.js"></script>
		<link rel="stylesheet" type="text/css"
...
$(document).ready(function() {//销售单FORM
	...
	$("#DaoRuFanqd").click(function(){// 选择确定
		...
		$("#chooseContext").attr("value",childopertionID);
		if(childopertionID == ""){
			alert("请选择");
			return;
		}
...
{  
	...
	<body style="overflow-y: auto;">
			...
			<div class="content1" style="width: 100%;">
				...
				<table width="99%" border="0" id="table3" align="center"
					...
					<tr>
						...
						<td>
							<span id="spanBillsType">${cashierBillsVO.billsType}</span><!--  <c:if	test="${cashierBillsVO==null||cashierBillsVO.billsType==''}">${BType}</c:if> -->
						...
						<td width="15%">
							<input type="hidden" id="partnerID" name="cashierBills.staffID"
								readonly="readonly" value="${cashierBillsVO.staffID==null?canStaffId:cashierBillsVO.staffID}" />
							<input type="text" id="partnerName" name="partnerName" class="put3" 
								readonly="readonly"
								value="<c:if test="${cashierBillsVO.staffname!=null}">${canStaffName}---${canStaffCode}</c:if>"
								size="15" />
							<a href="#"
								onclick="importGY('table3','partnerID','partnerName','childPartnerName','ea/cosincumbent/ea_getStaffForCashier.jspa')">选择</a>
							<input type="hidden" id="staffName" name="cashierBills.staffName" value="${cashierBills.staffName==null?canStaffName:cashierBills.staffName}"/>
							<input type="hidden" id="staffCode" name="cashierBills.staffCode" value="${cashierBillsVO.recordcode==null?canStaffCode:cashierBillsVO.recordcode}"/>
						</td>
					...
					<tr>
						...
						<td align="right">
							部门：
							<input type="hidden" id="departmentName" name="cashierBills.departmentName"/>
						</td>
							...
							<div id="Layer1"
								...
								<table align="center" cellpadding="0" cellspacing="0"
									...
									<s:iterator value="pageForm.list">
										<tr class="xggoods" id="${goodsBillsID}">
											...
											<td align="center" bgcolor="#FFFFFF">
												...
												<input name="quantity" class="jisuan isNaN put3 model1"
									...
									<tr id="kelong" style="display: none;">
										...
										<td height="30" align="center" bgcolor="#FFFFFF"
											><span class="bitian">*</span>
											<input name="startDate" onfocus="daytime(this)" class="input put3"
				...
				<table width="99%" height="30" border="0" align="center"
					...
					<tr>
						<td bgcolor="#FFFFFF" colspan="3">
							<input type="hidden" class="ACT_btn" name="button4" value="导入数据" />
							...
							<input type="button" class="ACT_btn" id="newG" name="button7"
								value="选择设备" />
							<input type="button" class="ACT_btn" id="chooseRecruiter" name="button7"
								value="选择招生员" />
							<input type="button" class="ACT_btn" name="button4" value="选择产品"
								id="chanpin" />
						</td>
					</tr>
				...
				<table width="99%" border="0" align="center" id="recruiterTable"
					cellpadding="0" cellspacing="0" style="background: #FFFFFF;"
					class="table">
					<tr>
						<td width="10%" height="25" align="right">
							<span class="STYLE1">分校\报名点：</span>
						</td>
						<td width="25%">
							<select class="model1" id="assignsID"  style="display: none;"></select>	
							<span id="assignsName" class="qk">${cashierBills.assignsName}</span>
							<input type="hidden" id="assignsID"
								name="cashierBills.assignsID"
								value="${cashierBills.assignsID}" />
							<input type="hidden" id="assignsName"
								name="cashierBills.assignsName"
								value="${cashierBills.assignsName}" />
						</td>
						<td width="7%" align="right">
							<span class="STYLE1">报名时间：</span>
						</td>
						<td width="10%">
							<input type="text" id="signUpDate" class="model1"  style="display: inline;"
								name="cashierBills.signUpDate"
								value="${fn:substring(cashierBills.signUpDate,0,11)}"  onfocus="date()" size="14"/>
						</td>
						<td width="7%" align="right">
							<span class="STYLE1">招生员：</span>
						</td>
						<td width="10%">
							<span id="referrerName" class="qk">${cashierBills.referrerName}</span>
							<input type="hidden" id="referrerID"
								name="cashierBills.referrerID"
								value="${cashierBills.referrerID}" />
							<input type="hidden" id="referrerName"
								name="cashierBills.referrerName"
								value="${cashierBills.referrerName}" />
						</td>
						<td width="10%" align="right">
							<span class="STYLE1">招生员联系方式：</span>
						</td>
						<td width="15%">
							<span id="referrerPhone" class="qk">${cashierBills.referrerPhone}</span>
							<input id="referrerPhone" type="hidden"
								name="cashierBills.referrerPhone"
								value="${cashierBills.referrerPhone}" />
						</td>
					</tr>
					<tr>
						<td height="30" align="right">
							<span class="STYLE1" >招生员身份证号：</span>
						</td>
						<td>
							<span id="referrerIdentityCard" class="qk">${cashierBills.referrerIdentityCard}</span>
							<input id="referrerIdentityCard" type="hidden"
								name="cashierBills.referrerIdentityCard"
								value="${cashierBills.referrerIdentityCard}" />
						</td>
						<td align="right">
							<span class="STYLE1">邮箱：</span>
						</td>
						<td>
							<span id="referrerEmail" class="qk">${cashierBills.referrerEmail}</span>
							<input id="referrerEmail" type="hidden"
								name="cashierBills.referrerEmail"
								value="${cashierBills.referrerEmail}" />
						</td>
						<td align="right">
							<span class="STYLE1">地址：</span>
						</td>
						<td colspan="3">
						<input id="referrerAddress" type="hidden"
								name="cashierBills.referrerAddress"
								value="${cashierBills.referrerAddress}" />
							<span id="referrerAddress" class="qk">${cashierBills.referrerAddress}</span>
						</td>
					</tr>
				</table>	
					...
					<tr>
						<td height="30" align="center">
							...
							<input type="button" class="btn001 JQueryprint" name="button"
								value="打印预览" />
							<input type="button" class="btn001 JQueryunitret" name="button" id="chongzhi1"
								value="重置往来单位" />
							<input type="button" class="btn001 JQuerypersonret" name="button"  id="chongzhi2"
								value="重置往来个人" />
							<input type="button" style="display: none;"
								class="btn001 JQuerySubmitgd" name="button3" value="提交审核"/>
							<input type="button" style="display: none;"
								class="btn001 JQuerySubmit" name="button4" value="草稿保存" />
							<input type='text' id='chooseContext' style="display:none;"/>
							<input type="button" style="display: none;"
				...
				<div class="content1" style="width: 100%; height: 400px;">
					...
					<table width="99%" height="33" id="searchproduct" border="0"
						...
						<tr>
							...
							<td width="60">
								<input name="parameter" class="input" id="parameter3" size="15"
 	...
 	<form name="SubjectsForm" id="SubjectsForm" method="post"
					...
					<table width="99%" height="33" id="searchgood" border="0"
						...
						<tr>
							...
							<td height="33">
								...
								<input type="button" class="btn02" id="selectGood1"
									name="button5" value="确定" />
		...
		<div id="bankJqmStafff" class="jqmWindow"
			style="width: 95%; height: 320px; absolute; display: none; left: 2.5%; top: 1%; background: #eff; overflow-x: hidden; overflow-y: auto;">
			<iframe name="daoRuStafff" id="daoRuStafff" width="100%" height="270px"
				frameborder="0" style="overflow-x: hidden; overflow-y: auto;"></iframe>
			<div align="center">
				<input type="button" class="input-button" id="DaoRuFanqdStafff"
					value=" 确定 " style="cursor: hand; border: 0;" />
				<input type="button" class="input-button" id="DaoRuFanStafff" value=" 关闭 "
