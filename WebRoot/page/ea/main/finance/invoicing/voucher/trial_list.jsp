...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#but").click(function(){
			if($("#edate").val()==""||$("#sdate").val()==""||$("#sdate").val()<$("#edate").val()){
				alert("起止日期必须输入,且终止日期必须大于或等于起始日期");
				return;
			}
			var b=true;
			$.ajax({
				url:basePath+"ea/voucher/sajax_ea_notNullData.jspa",
			...
			if(b){
				$("#form").attr("action",basePath+"ea/voucher/ea_trialBalance.jspa").submit();
			}			
		})	
	})
</script>
  ...
  <body style="background-color: #DAE7F6">
    ...
    <form action="" method="post" id="form" name="form" target="_blank">
    	<center><font size="6">试算表</font></center>
    	<table cellspacing="20px">
    		<tr>
    			<td>报表类型：</td>
    			<td><input type="radio" name="check" value="总额式" style="width: 40px" checked="checked">总额式</td>
    			<td><input type="radio" name="check" value="余额式" style="width: 40px">余额式</td>
    		</tr>
    		<tr>
    		...
    		<tr>
    			<td>起止科目：</td>
    			<td><input type="text" name="ssub" id="ssub">&nbsp;-</td>
    			<td><input type="text" name="esub" id="esub"></td>
    		</tr>
