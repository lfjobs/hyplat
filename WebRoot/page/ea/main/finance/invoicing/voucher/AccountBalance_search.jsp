...
$(function(){
 $("#tosearch").click(function(){
						...
						//var s2=new Date().toLocaleDateString().substring(0,4)+new Date().toLocaleDateString().substring(5,6);
                        $(".put3").trigger("blur");
	                    if ($("#SearchForm .error").length) {
	                        alert("请填完所有必填项");
	                        return;
	                    }
	                  //  if(parseInt(s1)>parseInt(s2) || parseInt(s1)==parseInt(s2)){
						//	alert("请输入本月前日期！");
						//	return;
						//}
                    window.open(basePath + "ea/csbjects/ea_toStatements.jspa?sdate="+$("#sdate").val());
                });
});
