	  ...
	  $(function(){ 
            $(".xsxyCloneClass").live('click',function(){
            	...
            	if(pageCount!=''&&pageNum<=parseInt($.trim(pageCount))){
					...
					if($.trim(one)=='one'){
						 url =basePath
								+ "ea/warehousing/ea_getInventoryPoolList.jspa?search="+search+"&pageForm.pageNumber="+pageNum
										+"&pageNumber="+pageNumber+"&print=print";
					}
					if($.trim(one)=='one0'){
						 url=basePath+ "ea/warehousing/ea_getInventoryDetailList.jspa?search="+search+"&pageForm.pageNumber="+pageNum 
						 +"&pageNumber="+pageNumber+"&print=print";
					}
