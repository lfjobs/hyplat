	  ...
	  $(function(){ 
            $(".xsxyCloneClass").live('click',function(){
            	...
            	if(pageCount!=''&&pageNum<=parseInt($.trim(pageCount))){
					...
					if($.trim(one)=='one'){
						 url =basePath
								+ "ea/purchase1/ea_getinspectList.jspa?search="+search+"&pageForm.pageNumber="+pageNum
										+"&pageNumber="+pageNumber+"&print=print";
					}
					if($.trim(one)=='one0'){
						 url =basePath
								+ "ea/storage/ea_getChooseWarehousingList.jspa?search="+search+"&pageForm.pageNumber="+pageNum
										+"&pageNumber="+pageNumber+"&print=print";
					}
