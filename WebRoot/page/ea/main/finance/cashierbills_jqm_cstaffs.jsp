...
<script type="text/javascript" src="<%=basePath%>js/photoup/ImagePreviewd.js"></script>
		...
		var notoken = 0;
		var photosizes = 0;
		var titles = '<%=request.getAttribute("title1") %>';
		var checkOrgID = '${checkOrgID}';
		var title1='${title1}'
...
//		.jqDrag('.drag');// 添加拖拽的selector
	...
	function action(com, grid){
		switch (com) {
			case '查询':
				...
				break;
			case '设置每页显示条数':
			var url=basePath + "ea/remind/ea_getStaffForCashier.jspa?search="+psearch+"&parameter=${parameter}&oid=${oid}&title1="+title1+"&checkOrgID="+checkOrgID;
				numback(url);
				break; 
		}
	}
	$(".JQueryflexme tr[id]").toggle(function(){
     ...
     $("#searchStaff").click(function(){
         $("#cstaffSearchForm").attr("action", basePath + "ea/remind/ea_toSearchStaffForCashier.jspa?pageNumber="+ppageNumber+"&oid=${oid}&title1="+title1+"&checkOrgID="+checkOrgID);
		...
		<div class="main_main">
			...
			<c:import url="../../page_navigator.jsp">
				<c:param name="actionPath"
					value="ea/remind/ea_getStaffForCashier.jspa?search=${search}&pageNumber=${pageNumber}&parameter=${parameter }&oid=${oid }&title1=${title1}&checkOrgID=${checkOrgID}">
				</c:param>
			</c:import>
		</div>
		<!--搜索窗口 -->
