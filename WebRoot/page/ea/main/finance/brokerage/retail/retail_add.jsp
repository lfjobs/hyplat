                        ...
                        <table width="1700">
                            ...
                            <tr>
                                <td class="td-0" style="display: none">
                                    <input type="text" id="ppid" name="proSetup.ppid">
                                </td>
                                <input type="hidden" id="brokerages" name="proSetup.proxyprice"
                                       class="calculate brokerages">
                                <td class="td-1"></td>
                                <td class="td-2"><input type="text" class="ppname" name="proSetup.ppname"/></td>
                                <td class="td-4"><input type="number" value="0" id="showRetail"
                                                        class="calculate showRetail"></td>
                                <td class="td-3"><input type="number" value="0" id="rePrice" class="calculate rePrice"
                                                        name="proSetup.rePrice">
                                </td>
                                <td class="td-5"><input type="number" placeholder="请输入" id="cost"
                                                        name="proSetup.efPrice" class="calculate cost"></td>
                                <td class="td-6">
                                ...
                                <td class="td-13"><input type="number" value="0" id="ywyj" name="proSetup.brokerage"
...
<script type="application/javascript">
    $(function () {
        ...
        $("#sure").click(function () {
            ...
            var facto = $("#rePrice").val() - $("#sbtz").val() - $("#tp").val() - $("#sbaz").val() - $("#sjdl").val() - $("#xjdl").val() - $("#cjdl").val() - $("#khjf").val() - $("#ywyj").val();
            var sj = $("#cost").val();
            if(parseFloat(sj)==0){
                alert("成本价不能为0！");
                return;
            ...
            if (Number(sj).toFixed(2) == Number(facto).toFixed(2)) {
                ...
                $.ajax({
                    type: "POST",
                    url: basePath + "ea/retail/sajax_ea_addRetailBrokerage.jspa",
                    data: $("#addRetail").serialize(),
        ...
        $("input[type='number']").not(".cost").not(".ywyj").on("keyup keydown focus blur", function (event) {
            var parentNode = $(this).parent().parent();
            var ywyj = parentNode.find(".retail").val() - parentNode.find(".cost").val() - parentNode.find(".sbtz").val() - parentNode.find(".tp").val()
    ...
    function productList() {
        $.ajax({
            type: "POST",
            url: basePath + "ea/retail/sajax_ea_getProductByStatus.jspa",
            data: {
