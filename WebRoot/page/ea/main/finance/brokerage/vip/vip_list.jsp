...
<div id="main_main" class="main_main">
    <table class="address">
        ...
        <c:forEach var='arr' items="${pageForm.list}">
            <tr id="${arr[0]}">
                ...
                <td>
                    <p id="price">${arr[7]}</p><%--系统VIP价--%>
                </td>
                <td>
                    <span class="showPrice">${arr[8]}</span><%--展示VIp价--%>
                </td>
                <td>
                    <span id="showweixin">${arr[9]=="01"?"已发布":"未发布"}</span>
                </td>
                <td>
                    <span id="vipstatus">${arr[10]=="01"?"已设计佣金":"未设计佣金"}</span>
                </td>
            </tr>
            <% number++; %>
        </c:forEach>
    ...
    <c:import url="../../../../page_navigator.jsp">
        <c:param name="actionPath"
                 value="ea/vip/ea_selectVipList.jspa?pageNumber=${pageNumber}&search=${search}"></c:param>
    </c:import>
</div>

<iframe name="hidden" frameborder="0" noresize="noresize" border="0" framespacing="0" height="0"></iframe>

<script type="text/javascript" src="<%=basePath%>js/ea/finance/brokerage/vip/vip.js"></script>
</body>
</html>
