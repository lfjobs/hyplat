...
<section class="commission des">
    ...
    <article>
        ...
        <figure>
                ...
                <table width="1700" class="tit">
                    ...
                    <tr>
                        ...
                        <c:forEach items="${agencyTypeList}" var="alist">
                            ...
                            <th style="width: 128px;">设备投资佣金</th>
                            <th style="width: 95px;">贴牌佣金</th>
                            <th style="width: 126px;">设备安装佣金</th>
                            <th style="width: 125px;">省级代理佣金</th>
                        ...
                        <table width="1700">
                            ...
                            <tr>
                                <td class="td-0" style="display: none">
                                    <input type="text" id="ppid" name="vip.ppid">
                                </td>
                                <input type="hidden" id="brokerages" name="vip.brokerages" class="calculate brokerages">
                                <td class="td-1"></td>
                                <td class="td-2"></td>
                                <td class="td-4"><input type="number" value="0" id="showVip" class="calculate showVip">
                                </td>
                                <td class="td-3"><input type="number" value="0" id="vip" class="calculate vip"
                                                        name="vip.vip">
                                </td>
                                <td class="td-5"><input type="number" placeholder="请输入" id="cost"
                                                        name="vip.factory" class="calculate cost"></td>
                                <td class="td-6">
                                    <input type="number" value="0" id="sbtz" name="mapPro['sbtz']"
                                           class="calculate sbtz">
                                ...
                                <td class="td-8">
                                    <input type="number" value="0" id="sbaz" name="mapPro['sbaz']"
                                           class="calculate sbaz">
                                    <input type="hidden" value="${agencyTypeList.get(2).ppID}" name="mapId['sbazId']">
                                </td>
                                <td class="td-9">
                                    <input type="number" value="0" id="sjdl" name="mapPro['sjdl']"
                                           class="calculate sjdl">
                                    <input type="hidden" value="${agencyTypeList.get(3).ppID}" name="mapId['sjdlId']">
                                </td>
                                <td class="td-10">
                                    <input type="number" value="0" id="xjdl" name="mapPro['xjdl']"
                                           class="calculate xjdl">
                                    <input type="hidden" value="${agencyTypeList.get(4).ppID}" name="mapId['xjdlId']">
                                </td>
                                <td class="td-11">
                                    <input type="number" value="0" id="cjdl" name="mapPro['cjdl']"
                                           class="calculate cjdl">
                                    <input type="hidden" value="${agencyTypeList.get(5).ppID}" name="mapId['cjdlId']">
                                </td>
                                <td class="td-12">
                                    <input type="number" value="0" id="khjf" name="mapPro['khjf']"
                                           class="calculate khjf">
                                    <input type="hidden" value="${agencyTypeList.get(6).ppID}" name="mapId['khjfId']">
                                </td>
                                <td class="td-13"><input type="number" value="0" id="ywyj" name="vip.brokerage"
                                                         class="calculate ywyj">
                                </td>
                                <td class="td-14">
                                    <select name="vip.investType" class="selector">
            ...
            <ul class="footer">
                <li>
                    设计VIP价责任人<input type="text" readonly="readonly" value="${cac.staffName}">
                </li>
                <li>
                    设计VIP价日期<input type="text" readonly="readonly" value="${myDate}">
                </li>
            </ul>
        </figure>
        <!--内容 end-->
    ...
    <div class="alert-com">
        ...
        <div class="alert_com">
            ...
            <div class="tab">
                <p>*注:该页面只展示已设置零售价佣金并且未设置vip价佣金的产品</p><br>
...
<script type="application/javascript">
    $(function () {
        ...
        $("#sure").click(function () {
            ...
            var facto = $("#vip").val() - $("#sbtz").val() - $("#tp").val() - $("#sbaz").val() - $("#sjdl").val() - $("#xjdl").val() - $("#cjdl").val() - $("#khjf").val() - $("#ywyj").val();
            var sj = $("#cost").val();
            if(parseFloat(sj)==0){
                alert("成本价不能为0！");
                return;
            ...
            if (Number(sj).toFixed(2) == Number(facto).toFixed(2)) {
                ...
                $.ajax({
                    type: "POST",
                    url: basePath + "ea/vip/sajax_ea_addVipBrokerage.jspa",
                    data: $("#addVip").serialize(),
                ...
                success: function (data) {
                    ...
                    if (member.code == '200') {
                        ...
                        if (member.pPercentage == null) {
                            ...
                            //console.log(member.pPercentage);
                            var vip = member.pPercentage.vip;
                            vip = val * (vip / 100 + 1);
                            //vip = vip.toFixed(2);
                            parentNode.find(".vip").valvip.toFixed(4))//系统vip价
                            //展示VIP价判断获取
                            if (member.beanList == null || member.beanList.length == 0) {
                                parentNode.find(".showVip").val(vip.toFixed(2))//展示VIP价[系统VIP价]
                            } else {
                                var showVip = vip * (member.beanList[0][1] / 100 + 1);
                                parentNode.find(".showVip").val(showVip.toFixed(2))//展示VIP价
                            }
                            //佣金判断获取
                            if (member.bPercentage == null) {
                                ...
                                var commission = vip - val;//佣金（VIP价-成本价）
                                //计算设备投资佣金
        ...
        $("input[type='number']").not(".cost").not(".ywyj").on("keyup keydown focus blur", function (event) {
            var parentNode = $(this).parent().parent();
            var ywyj = parentNode.find(".vip").val() - parentNode.find(".cost").val() - parentNode.find(".sbtz").val() - parentNode.find(".tp").val()
