    ...
    <article>
        ...
        <figure>
            <div class="head">
                ...
                <input type="hidden" value="${product.ppID}" id="ppid"/>
            </div>
            <div class="grid-1">
                        ...
                        <table width="1700">
                            ...
                            <tr>
                                <td class="td-0" style="display: none">
                                    <input type="text" name="vip.vipId" value="${pVips[0]}">
                                </td>
                                <input type="hidden" id="brokerages" name="vip.brokerages" class="calculate brokerages"
                                       value="${pVips[4]}"><%--代理佣金和--%>
                                <td class="td-1"><input type="text" value="${product.barCode}" readonly="readonly"></td>
                                <td class="td-2"><input type="text" value="${product.goodsName}" readonly="readonly">
                                </td>
                                <td class="td-4"><input type="number" value="${product.price}" id="showVip"
                                                        class="calculate showVip">
                                <td class="td-3"><input type="number" value="${pVips[1]}"
                                                        data-value="${pVips[1]}" id="vip" name="vip.vip"
                                                        class="calculate vip">
                                </td>
                                </td>
                                <td class="td-5"><input type="number" value="${pVips[2]}" placeholder="请输入"
                                                        id="cost" class="calculate cost"
                                                        name="vip.factory" data-value="${pVips[2]}"></td>
                                <td class="td-6">
                                    <!--[if supportFields]>
                                ...
                                <td class="td-13"><input type="number" value="${pVips[3]}" id="ywyj"
                                                         class="calculate ywyj"
                                                         name="vip.brokerage" data-value="${pVips[3]}">
                                </td>
                                <td class="td-14">
                                    <select name="vip.investType" class="selector">
                                        <option value="00" <c:if test="${pVips[5]=='00'}">selected</c:if>>选择投资类别
                                        </option>
                                        <option value="01" <c:if test="${pVips[5]=='01'}">selected</c:if>>教练车
                                        </option>
                                        <option value="02" <c:if test="${pVips[5]=='02'}">selected</c:if>>创客单车
                                        </option>
                                        <option value="03" <c:if test="${pVips[5]=='03'}">selected</c:if>>超市
                                        </option>
                                    </select>
                                </td>
                            </tr>
...
<script type="application/javascript">
    $(function () {
                ...
                success: function (data) {
                    ...
                    if (member.code == '200') {
                        ...
                        if (member.pPercentage == null) {
                            ...
                            vip = val * (vip / 100 + 1);
                            //vip = vip.toFixed(2);
                            parentNode.find(".vip").val(vip.toFixed(4))//系统vip价
                            //展示VIP价判断获取
        ...
        $("#sure").click(function () {
            var cost_flag = false;
            var num = 1;
            $("form input[type='number']").not(".showVip").each(function () {
            ...
            if (Number(sj).toFixed(2) == Number(facto).toFixed(2)) {
                ...
                $.ajax({
                    type: "POST",
                    url: basePath + "ea/vip/sajax_ea_updateVipBrokerage.jspa",
                    data: $("#updateVip").serialize(),
                    success: function (data) {
                        var member = eval("(" + data + ")");
