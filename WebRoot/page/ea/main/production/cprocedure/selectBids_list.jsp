...
<script type="text/javascript" src="<%=basePath%>js/ea/production/cprocedure/selectBids_list.js"></script>
	...
	<div style="margin-top:10px;margin-left:10px;display:none;"
		...
		<form id="SearchForm" name="SearchForm" method="post">
			...
			<table>
				...
				<!-- 	<td>&nbsp;有效期：</td>
					<td><input type="text" style="width:140px;" onfocus="daytime(this)" name="inviteBids.startDate"
								readonly  />
							   &nbsp;至&nbsp;&nbsp;<input type="text" style="width:140px;" onfocus="daytime(this)" name="inviteBids.endDate"
								readonly />
					</td> -->
					<td>&nbsp;选标状态：</td>
					<td><select name="inviteBids.status">
					            <option value='' selected="selected">全部</option>
					            <option value='01' >未选标</option>
	...
	<div class="main_main">
			...
			<tbody>
				...
				<s:iterator value="pageForm.list">
					<tr id="${ibId}">
						...
						<td>
							<span id="startDate">${fn:substring(startDate,0,10)}-${fn:substring(endDate,0,10)}</span>
						</td>
						<td>
							<span id="selectbidDate">${fn:substring(selectbidDate,0,19)}</span>
						</td>
						<td>
							<span id="selectName">${selectName}</span>
						</td>
						
						<td>
						   <span id="status" style="display:none;">${status}</span>

						   <s:if test="status=='01'">未选标</s:if>
		...
		<c:import url="../../../page_navigator.jsp">
			<c:param name="actionPath"
				value="ea/purchasebids/ea_findSelectBidsList.jspa?pageNumber=${pageNumber}&search=${search}&fiveClear=${fiveClear}">
			</c:param>
		</c:import>
	</div>
