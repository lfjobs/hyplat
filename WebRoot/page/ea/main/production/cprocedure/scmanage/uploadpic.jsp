...
<body>
    ...
    <div class="wrap_page">
        <form name="formsave" id="formsave"  action="<%=basePath%>ea/scm/ea_saveContent.jspa" method="post">
            <textarea class="photo_des" placeholder="添加图片描述…"  name="materialContent.describe"></textarea>
            <div class="up_group_wrap clearfix">
               <a href="<%=basePath%>/ea/scm/ea_selectGroupList.jspa?type=${type}" style="color:#333333;">
                ...
                <input type="hidden" value="${materialGroup.groupname}" name="materialGroup.groupname"/>
                <input type="hidden" value="00" name="materialContent.fileType" />
                </a>
            </div>
            <ul class="photos_wrap clearfix">
            <c:forEach items="${list}" var="item">
               <li style="background-image: url(<%=basePath%>/${item.filepath})">
                 <i class="up_dele"></i>
                 <div class="progress">
    ...
    <script src="<%=basePath%>js/ea/production/cprocedure/scmanage/image-upload.js"></script>
