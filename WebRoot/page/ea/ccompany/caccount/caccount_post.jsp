...
<html xmlns="http://www.w3.org/1999/xhtml">
			...
			<table height="100" border="0" align="center"
				...
				<tr>
					...
					<td width="10" align="center">
						<div class="right_dan" id="rightdan"></div>
						<div class="left_dan" id="leftdan"></div>
					</td>
	...
	<script type="text/javascript">
        ...
        tree1.setOnDblClickHandler(function(){
           ...
           //账号所属的部门列表
            var searchurl=  basePath +"ea/ccaccount/sajax_ea_getOrganizationForAccount.jspa?accountID="+encodeURI(accountID)+"&compid="+compid+"&date="+new Date().toLocaleString();
            $.ajax({
                        url: searchurl,
                        type: "get",
                        async: true,
           ...
           error: function cbf(data){
             ...
             //单位下所有的部门列表
           var url = basePath +"ea/ccaccount/sajax_ea_getOrganizationList.jspa?compid="+compid+"&date="+new Date().toLocaleString();
           $.ajax({
                        url: url,
                        type: "get",
                        async: true,
                 ...
                 var organizationList = member.organizationList;
                 var companylist = member.companylist;
                alert(companylist[0].companyName);
                if (null != companylist) {
                    for (var i = 0; i < companylist.length; i++) {
                      tree.insertNewChild('0', companylist[i].companyID, companylist[i].companyName, 0, 0, 0, 0);
                    }
                }
                if (null != organizationList) {
                      for (var i = 0; i < organizationList.length; i++) {
                      tree.insertNewChild('0', organizationList[i].organizationID, organizationList[i].organizationName, 0, 0, 0, 0);
                      }
                }
            },  error: function cbf(data){
                    alert("数据获取失败！");
        ...
        $(".jquerysub").click(function(){
          ...
          document.location.href= basePath+"ea/ccaccount/ea_accountRedeployOrganization.jspa?pageNumber=<%=request.getParameter("pageNumber")%>&organizations="+organizations+"&accountID="+accountID+"&roleIDX="+roleIDX+"&methodX="+methodX+"&search=${param.search}&compid="+compid+"&title=${param.title}";
        });
        $("#cosed").click(function(){
          document.location.href= basePath+"/ea/ccaccount/ea_getListCAccount.jspa?pageNumber=<%=request.getParameter("pageNumber")%>&roleIDX="+roleIDX+"&methodX="+methodX+"&search=${param.search}&title=${param.title}&compid="+compid;
        });        
