...
<div class="content">
    <s:form id="f2" action="ea_upCostSheet.jspa" namespace="/ea/scBudget" method="POST">
        <input type="hidden" name="cashierBills.cashierBillsID" value="${cashierBillsId}"/><%--已添加预算单id（修改用）--%>
        <input type="hidden" id="ttsw_hid_delGoods2" name="delGoodsBillsIds" value="${delGoodsBillsIds}"/><%--修改页面删除已保存的货物id数组--%>
        <section class="sec_con1">
            <p>
                <label for=""><span></span>单据凭证号：</label>
                <input type="text" name="cashierBills.journalNum" id="ttsw_billID" value="${cashierBills.journalNum}" placeholder="自动获取" required readonly/>
            </p>
            <div class="clearfix">
                <p>
                    <label for=""><span></span>公司：</label>
                    <input type="text" id="ttsw_companyNmae" name="cashierBills.companyName"  class="gs_name" value="${cashierBills.companyName}" placeholder="" required readonly/>
                </p>
                <%--所选分部门则显示不可修改--%>
                <c:if test="${showFlag eq true}">
                    <p>
                        <label for=""><span></span>创收部门：</label>
                        <input type="text" name="cashierBills.departmentName" id="ttsw_dep_y_name" value="${cashierBills.departmentName}" placeholder="请选择部门" required readonly />
                        <input type="text" name="cashierBills.departmentID" id="ttsw_dep_y_id" value="${cashierBills.departmentID}" readonly style="display: none;"/><%--所选部门id--%>
                    </p>
                </c:if>
                <%--所选为总列表，则显示下来自己选择--%>
                <c:if test="${showFlag eq false}">
                    <p class="clearfix">
                        <label for=""><span>*</span>创收部门： </label>
                        <input type="text" class="csbm_xiala-1" id="ttsw_dep_n_name" name="cashierBills.departmentName" value="${empty addBean.departmentName ? cashierBills.departmentName:addBean.departmentName}" placeholder="请选择部门" required readonly/>
                        <input type="text" class="csbm_id_xiala-1" id="ttsw_dep_n_id" name="cashierBills.departmentID" value="${empty addBean.departmentID ? cashierBills.departmentID:addBean.departmentID}" readonly style="display: none;"/><%--所选部门id--%>
            ...
            <div class="clearfix">
                <p class="clearfix">
                    ...
                    <input type="text" name="cashierBills.projectName" id="project-name" value="${empty addBean.itemName ? cashierBills.projectName:addBean.itemName}" placeholder="请选择项目名称"required readonly/>
                    <img src="<%=basePath %>images/scMobile/payBudget/addBudget/wupinguanli_img_13.png"/>
                </p>
                <p class="clearfix">
                    <label for=""><span>*</span>项目分类： </label>
                    <input type="text" name="cashierBills.xmtypename" id="project-fl" value="${empty addBean.itemType ? cashierBills.xmtypename:addBean.itemType}" placeholder="请选择项目分类" required readonly/>
                    <img src="<%=basePath %>images/scMobile/payBudget/addBudget/wupinguanli_img_13.png"/>
                </p>
                <input type="text" id="ttsw_item_check_id" name="cashierBills.proID" value="${empty addBean.itemId ? cashierBills.proID:addBean.itemId}" style="display: none;"/><%--所选项目ppid--%>
                <input type="text" id="ttsw_item_check_type" name="cashierBills.xmtype" value="${empty addBean.xmType ? cashierBills.xmtype:addBean.xmType }" style="display: none;"/><%--所选项目类型--%>
                <input type="text" id="ttsw_item_check_code" name="cashierBills.projectCode" value="${empty addBean.itemCode ? cashierBills.projectCode:addBean.itemCode }" style="display: none;"/><%--所选项目编号--%>
            </div>
            <div class="clearfix">
                <p class="clearfix">
                    <label for=""><span>*</span>责任人： </label>
                    <input type="text" class="xiala-1"  value="${empty addBean.staffName ? cashierBills.staffName:addBean.staffName}(${empty addBean.staffCode ? "":addBean.staffCode})" placeholder="" required readonly/>
                    <input type="text" name="cashierBills.staffID" class="ttsw_emp_id" value="${empty addBean.staffFzrId ? cashierBills.staffID:addBean.staffFzrId}" style="display: none;"/>
                    <input type="text" name="cashierBills.staffName" class="ttsw_emp_name" value="${empty addBean.staffName ? cashierBills.staffName:addBean.staffName}" style="display: none;"/>
                    <input type="text" name="cashierBills.staffCode" class="ttsw_emp_code" value="${empty addBean.staffCode ? cashierBills.staffCode:addBean.staffCode}" style="display: none;"/>
                    <img src="<%=basePath %>images/scMobile/payBudget/addBudget/wupinguanli_img_13.png"/>
                </p>
            ...
            <div class="clearfix">
                <p>
                    ...
                    <input type="text" name="" id="ttsw_singleName" value="${empty addBean.singleName ? cashierBills.inputName:addBean.singleName}" placeholder="自动获取" required  readonly/>
                </p>
                <p>
        ...
        <ul class="ul_con">
            <c:if test="${!empty goodBeanslist}">
                <c:forEach items="${goodBeanslist}" var="entity" varStatus="v">
                    <li class="clearfix" id="${entity[0].goodsBillsID}">
                        ...
                        <section class="clearfix">
                            ...
                            <ul>
                                ...
                                <img onclick="toRemoveGoodsBills('${entity[0].goodsBillsID}');" class="img_del" src="<%=basePath %>images/scMobile/payBudget/addBudget/yusuandan_01_del.png"/>
                            </ul>
                        </section>
                        <input type="hidden" class="ttsw_jj" value="${entity[0].price}"/><%--价格--%>
                    </li>
                </c:forEach>
            </c:if>
            <%--选择具体项目后显示项目详细信息--%>
...
<s:form id="f1" action="" namespace="/ea/scBudget" method="POST">
    <input type="hidden" name="addBean.cashierBillsId" value="${cashierBillsId}"/><%--已添加预算单id（修改用）--%>
