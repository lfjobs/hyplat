...
(function(a) {
	a.addFlex = function(n, r) {
		...
		x = {
			...
			rePosDrag : function() {
				...
				a("thead tr:first th:visible", this.hDiv).each(function() {
							var s, p;
							s = a("thead tr:first th:visible", x.hDiv)
									.index(this);
							if (i == 0) {
								i -= Math.floor(r.cgwidth / 2)
							}
			...
			dragMove : function(t) {
				...
				if (this.colresize) {
					...
					if (this.vresize) {
						...
						E = I.h + H;
						if ((E > r.minheight || r.height < r.minheight)
								&& !I.hgo) {
						...
						if (this.colCopy) {
							a(this.dcol).addClass("thMove")
									.removeClass("thOver");
							if (t.pageX > this.hset.right
									|| t.pageX < this.hset.left
									|| t.pageY > this.hset.bottom
									|| t.pageY < this.hset.top) {
			...
			dragEnd : function() {
				...
				if (this.colresize) {
					i = this.colresize.n;
					g = this.colresize.nw;
					a("th:visible div:eq(" + i + ")", this.hDiv)
							.css("width", g);
					a("tr", this.bDiv).each(function() {
								a("td:visible div:eq(" + i + ")", this).css(
										"width", g)
							});
			...
			switchCol : function(i, g) {
				a("tbody tr", n).each(function() {
					if (i > g) {
						a("td:eq(" + g + ")", this).before(a(
								"td:eq(" + i + ")", this))
					} else {
						a("td:eq(" + g + ")", this).after(a("td:eq(" + i + ")",
								this))
					}
				});
				if (i > g) {
					a("tr:eq(" + g + ")", this.nDiv).before(a("tr:eq(" + i
									+ ")", this.nDiv))
				} else {
					a("tr:eq(" + g + ")", this.nDiv).after(a(
							"tr:eq(" + i + ")", this.nDiv))
				}
		...
		a("thead tr:first th", x.hDiv).each(function() {
			...
			a(this).empty().append(q).removeAttr("width").mousedown(
					function(g) {
				...
				if (a(this).attr("abbr") != r.sortname && !x.colCopy
						&& !x.colresize && a(this).attr("abbr")) {
		...
		if (f != null) {
			...
			x.cdpad = 0;
			x.cdpad += (isNaN(parseInt(a("div", f).css("borderLeftWidth")))
					? 0
					: parseInt(a("div", f).css("borderLeftWidth")));
			x.cdpad += (isNaN(parseInt(a("div", f).css("borderRightWidth")))
					? 0
					: parseInt(a("div", f).css("borderRightWidth")));
			x.cdpad += (isNaN(parseInt(a("div", f).css("paddingLeft")))
					? 0
					: parseInt(a("div", f).css("paddingLeft")));
			x.cdpad += (isNaN(parseInt(a("div", f).css("paddingRight")))
					? 0
					: parseInt(a("div", f).css("paddingRight")));
			x.cdpad += (isNaN(parseInt(a(f).css("borderLeftWidth")))
					? 0
					: parseInt(a(f).css("borderLeftWidth")));
			x.cdpad += (isNaN(parseInt(a(f).css("borderRightWidth")))
					? 0
					: parseInt(a(f).css("borderRightWidth")));
			x.cdpad += (isNaN(parseInt(a(f).css("paddingLeft")))
					? 0
					: parseInt(a(f).css("paddingLeft")));
			x.cdpad += (isNaN(parseInt(a(f).css("paddingRight")))
					? 0
		...
		if (r.usepager) {
			...
			a("div", x.pDiv)
					.html(' <div class="pGroup"> <div class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">Page <input type="text" size="4" value="1" /> of <span> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>');
			...
			if (r.useRp) {
				...
				for (B = 0; B < r.rpOptions.length; B++) {
					...
					d += "<option value='" + r.rpOptions[B] + "' " + u + " >"
							+ r.rpOptions[B] + "&nbsp;&nbsp;</option>"
				}
				a(".pDiv2", x.pDiv)
			...
			if (r.searchitems) {
				...
				a(".pSearch", x.pDiv).click(function() {
					a(x.sDiv).slideToggle("fast", function() {
								a(".sDiv:visible input:first", x.gDiv)
										.trigger("focus")
							})
				});
				...
				for (o = 0; o < sitems.length; o++) {
					...
					e += "<option value='" + sitems[o].name + "' " + u + " >"
							+ sitems[o].display + "&nbsp;&nbsp;</option>"
				}
				if (r.qtype == "") {
					r.qtype = sitems[0].name
				}
				a(x.sDiv)
						.append("<div class='sDiv2'>Quick Search <input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>"
								+ e
								+ "</select> <input type='button' value='Clear' /></div>");
				a("input[name=q],select[name=qtype]", x.sDiv).keydown(
						function(g) {
							if (g.keyCode == 13) {
								x.doSearch()
							}
		...
		if (a("th", x.hDiv).length) {
			...
			a("td.ndcol2", x.nDiv).click(function() {
				if (a("input:checked", x.nDiv).length <= r.minColToggle
			...
			a(x.nBtn).addClass("nBtn").html("<div></div>").attr("title",
