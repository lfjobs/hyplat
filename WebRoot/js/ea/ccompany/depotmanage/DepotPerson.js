$(document).ready(function() {
	...
	function action(com, grid) {
		switch (com) {
			...
			case '全部保存' :
				...
				$('#staffappraisalForm')
						...
						.attr(
								"action",
								basePath
										+ "ea/cdepotperson/ea_saveDepotPerson.jspa?pageNumber="
										+ pNumber);
				...
				if (confirm("是否删除？")) {
					$f
							...
							.attr(
									"action",
									basePath
											+ "ea/cdepotperson/ea_delDepotPerson.jspa?pageNumber="
											+ pNumber
			...
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/cdepotperson/ea_getListDepotPerson.jspa?depotPerson.depotID="
						+ depotID + "&search=" + search;
				numback(url);
				break;
			case '导出' :
				var url = basePath
						+ "ea/cdepotperson/ea_showExcel.jspa?search=" + search
						+ "&depotPerson.depotID=" + depotID;
				open(url);
				break;
			case '查询' :
	...
	$("#tosearch").click(function() {
		...
		$("#appraisalForm").attr(
				"action",
				basePath + "ea/cdepotperson/ea_toSearch.jspa?pageNumber="
						+ pNumber);
 ...
 * for(var m=0;m<this._item.length;m++) { this._item[m].options.add(new
 * Option("" + this._data[i].text, this._data[i].id)); } for (var j = 0; j <
 * len; j++) { this.createSubOption(len, this._data[i], this._data[j]); } } } }
 * 
 ...
 * a++ ) blank += " "; blank += "├"; for(var m=0;m<this._item.length;m++) {
 * if(this._item[m].options.value!=next.id) { this._item[m].options.add(new
 * Option(blank + next.text, next.id)); } }
 * 
 * for (var j = 0; j < len; j++) { this.createSubOption(len, next,
 * this._data[j]); } } } TreeSelector.prototype.getLevel = function(datasources,
 * topId, currentitem){ var pid = currentitem.pid; if (pid != topId) { for (var
 * i = 0; i < datasources.length; i++) { if (datasources[i].id == pid) {
 * intLevel++; this.getLevel(datasources, topId, datasources[i]); } } } return
 * intLevel; }
 */
