...
//     var { lastPageNumber, pageSize, videoList } = JSON.parse(localStorage.getItem("MY_LOCAL_VIDEO_PAGE"));
//     this.pageNumber = lastPageNumber
//     this.pageSize = pageSize
//     appendVideoSlides(videoList);
// }
...
function loadData() {
    ...
    var url = basePath + (viewType === "1" ? "/ea/dsp/sajax_ea_myVideos.jspa" : "/ea/dsp/sajax_ea_myLikedVideos.jspa")
    $.ajax({
        url: encodeURI(url),
        type: "GET",
        async: true,
        ...
        success: function (data) {
            ...
            if (videoIndex !== -1) {
                mySwiper.slideTo(videoIndex, 0, false);
                videoIndex = -1;
            }
                ...
                <div class="video w-full h-full">
                    ...
                    <div class="absolute right-4 bottom-32 space-y-4">
                        ...
                        <div class="flex flex-col">
                            <img width="24" height="24" style="margin: auto" id="like-${video.videoId}" onclick="like('${video.videoId}')" class="${video.isLiked === '1' ? 'active' : ''}" src="${basePath + (video[10] === '1' ? 'images/ea/collage/dsp/img_19.png' : 'images/ea/collage/dsp/img_21.png')}"/>
                            <div id="like-text-${video.videoId}" class="text-center">
                                ${video.likedCount}
                            </div>
                        </div>
                        
                        <div class="" onclick="down()">
                            <span class="plcountv" style="display: none;">${video.commentCount}</span>
                            <img width="24" height="24" style="margin: auto" src="${basePath}images/ea/collage/dsp/img_26.png"/>
                            <div class="text-center">
                               ${video.commentCount}
                            </div>
                        </div>
                        
                        <!--<div class="" onclick="shareVideo('${video.videoId}')">
                            <span class="sharev" style="display: none;">${video.sharedCount}</span>
                            <img width="24" height="24" style="margin: auto" src="${basePath}images/ea/collage/dsp/img_30.png"/>
                        ...
                        <div>
                            <img onclick="$('#side-menu').toggleClass('hidden')" width="24" height="24" style="margin: auto" id="center" src="${basePath}images/ea/collage/dsp/center.png"/>
                        </div>
                    </div>
                </div>
            </section>
...
function authorHome(videoStaffId) {
    ...
    if (staffId !== '' && staffId === videoStaffId) {
        document.location.href = `${basePath}ea/dsp/ea_myHomepage.jspa`;
    } else {
        document.location.href = `${basePath}ea/dsp/ea_authorHomepage.jspa?videoStaffId=${videoStaffId}`;
    }
}
