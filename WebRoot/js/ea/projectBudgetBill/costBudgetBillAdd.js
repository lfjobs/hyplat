...
function toAddItem(){
    ...
    var url = "ea_toAddProjectItem.jspa?billsType=" + billsType;
...
function toAddPlanItem(){
    ...
    //treeid = "company20180510CQZCDKTT690000006064";//测试用
    $("#ttsw_hid_companyId").val(treeid);
    var url = "ea_toAddProjectItem.jspa";
    $("#f1").attr('action',url);    //通过jquery为action属性赋值
    $("#f1").submit();    //提交ID为myform的表单
}
...
$(document).ready(function () {
    if(billsType == "初始项目单"){
        $("#billsTypeSection").html("<ul>" +
            ...
            "                       </ul>" +
            "                       <p>" +
            "                           <input type='text' id='billsType' name='cashierBills.billsType' value='收入'/>" +
            "                       </p>");
    }
    // if(billsType != "初始项目单"){
...
function searchBusinessType(){
    window.location.href = basePath + "ea/scBudget/ea_toBusinessTypeSearch.jspa?editFlag=add&billsType="+billsType;
}
