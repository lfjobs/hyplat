...
$(function () {
    ...
    $(document).on("click", ".wldw1,.wldw2", function () {
        ...
        if (accountFlag=="company") {
            // getAttri2($("#goodsID").val());
            $(".div-account-company").show();
            //getRecentInfoCompany();
        } else if(accountFlag=="personal"){
            // getAttri1($("#goodsID").val());
    ...
    $(document).on("click", ".wldw3,.wldw4", function () {
        ...
        if (accountFlagFrom=="company") {
            // getAttri2($("#goodsID").val());
            $(".div-account-companyFrom").show();
            //getRecentInfoCompany();
        } else if(accountFlagFrom=="personal"){
            // getAttri1($("#goodsID").val());
...
function toLaunchOrcang(f) {
    ...
    if ($("#ppname").val() == "") {
        ...
        isSubmit = true;
        var url = basePath + "ea/scBudget/ea_toAddProjectBill.jspa";
...
function computeAmount(){
    ...
    if (guigeType=="长度") {
        guigeValue = $(guigeArr[0]+" .ul-con .clearfix").find(".attri1").val();
        amount = amount*parseFloat(guigeValue);
    } else if(guigeType=="面积"){
        // guigeValue = $(guigeArr[1]+" .ul-con .clearfix").find(".attri1").val();
        $(guigeArr[1]+" .ul-con .clearfix").each(function () {
            // console.log($(this).find(".attri1 ").val());
            amount = amount*parseFloat($(this).find(".attri1 ").val());
        });
    } else if(guigeType=="立方"){
        $(guigeArr[2]+" .ul-con .clearfix").each(function () {
            amount = amount*parseFloat($(this).find(".attri1 ").val());
        });
    } else if(guigeType=="重量"){
        $(guigeArr[3]+" .ul-con .clearfix").each(function () {
            amount = amount*parseFloat($(this).find(".attri1 ").val());
        });
    } else if(guigeType=="时间"){
        $(guigeArr[4]+" .ul-con .clearfix").each(function () {
            amount = amount*parseFloat($(this).find(".attri1 ").val());
        });
    } else if(guigeType=="个数"){
        $(guigeArr[5]+" .ul-con .clearfix").each(function () {
            amount = amount*parseFloat($(this).find(".attri1 ").val());
        });
    } else if(guigeType=="比例"){
        $(guigeArr[6]+" .ul-con .clearfix").each(function () {
            amount = amount*parseFloat($(this).find(".attri1 ").val());
        });
    }
    $("#amount").val(amount);
...
function getSpecsInfo(code,parentId) {
