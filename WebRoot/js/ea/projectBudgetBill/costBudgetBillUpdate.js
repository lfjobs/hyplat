...
function toAddItem(){
    ...
    var billsTypePage = $("#billsType").val();
    if((billsTypePage == "入库单" || billsTypePage == "出库单" || billsTypePage == "盘库单" || billsTypePage == "报损单") && (hid_dataDepotName == null || hid_dataDepotName == "")){
        alert("请选择仓库");
        return false;
    }
    ...
    $("#ttsw_hid_productType").val(hid_xmTypeName);
    ...
    $("#ttsw_hid_billsType").val(billsTypePage);
    ...
    var url = "ea_toAddProjectItem.jspa?editFlag=edit&cashierBillsId="+cashierBillsId+"&billsType="+billsType;
...
$(document).ready(function () {
    if(billsType == "初始项目单"){
        $("#billsTypeSection").html("<ul>" +
            "                        <li><a href=\"javascript:nav('收入')\">" +
            "                               <img id='srdj' src='"+basePath+"images/ea/office/contract/selectp/dxseleted.png'>收入</a>" +
            "                        </li>"+
            "                        <li><a href=\"javascript:nav('支出')\">" +
            "                               <img id='zcfb' src='"+basePath+"images/ea/office/contract/selectp/dxwselet.png'>支出</a>" +
            "                        </li>" +
            "                       </ul>" +
            "                       <p>" +
            "                           <input type='text' id='billsType' name='cashierBills.billsType' value='"+cashierBillsBillsType+"'/>" +
            "                       </p>");
    }else if(billsType != "" && billsType != "采购单" && billsType != "销售单"){
...
$(document).on("click", ".div-hangye .div-right .ul-04 li", function () {
    ...
    $(".hangye2").show();
    $(".hyfl").empty();
    addBusinessTypeRecent($("#hyflId").val(), $("#hyfl").val());
});
...
$(document).on("click", ".div-xiangmu .div-right .ul-04 li", function () {
    ...
    $(".xiangmu2").show();
    $(".xmfl").empty();
    addBusinessTypeRecent($("#xiangmuId").val(), $("#xiangmu").val());
});

function searchBusinessType(){
    window.location.href = basePath + "ea/scBudget/ea_toBusinessTypeSearch.jspa?editFlag=edit&billsType="+billsType+"&cashierBillsId="+cashierBillsId;
}

function setBusinessType(){
    if(null != typeId && "" != typeId){
        $("#hyflId").val(typeId);
        $("#xiangmuId").val(typeId);
    }
    if(null != typeName && "" != typeName) {
        $("#hyfl").val(typeName);
        $("#xiangmu").val(typeName);
    }
}
...
function addBusinessTypeRecent(typeId,typeName){
    ...
    $.ajax({
        ...
        dataType: "json",
        data: {
            "businessTypeRecent.typeId": typeId,
            "businessTypeRecent.typeName": typeName,
