$(function(){
	...
	$(".but").click(function(){
		if($(this).val()=="提交"){
			...
			$.ajax({
				url:basePath+"ea/resultInv/sajax_ea_ResultInvSingleAudit.jspa?deptpost="+str+"&cashierBillsID="+$("#cashierBillsID").val()+"&audit="+$("#textarea").val()+"&status="+$('input[name="radio"]:checked').val(),
				type:"post",
				async : false,
	...
	$(".radioButton").each(function(){
		var $this=$(this),ppID=$(this).find(".ppID").attr("ppid");
		$.ajax({
			...
			success:function(data){
				...
				for (var i=0;i<list.length;i++){
					...
					var tt="<input radio='fieldConStors[6].content' type='radio'  name='fieldConStors[6].content'   class='radioClassContent' value='"+list[i].goodsName+"'  "+str+" />";
	...
	$(".startAndEndTime").each(function(){
		var strs=$(this).find(".dateContent").val().split(" - ");
		$(this).find(".start").val(strs[0]);
		$(this).find(".end").val(strs[1]);
		
		var date1 = new Date(strs[0].replace(/-/g, "/"));
		var date2 = new Date(strs[1].replace(/-/g, "/"));
	...
	$(".radioAdmin").find(".radioClassContent").click(function(){
...
$(".overTimeStartDate").blur(function() { // 起时间失去焦点事件
	...
	$(".JQuerySave").click(function(){
		$("#form").attr("target", "hidden").attr("action",basePath+"ea/resultInv/ea_editResultsData.jspa?fiveClear="+fiveClear+"&category="+category+"&fiveClearName="+fiveClearName);
