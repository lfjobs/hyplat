$(function(){
	//判断状态
	if(type=="add"){
		$("#preservation").removeClass("grey").attr("disabled","");
		$("#submitAudit").removeClass("grey").attr("disabled","");
		$("#close").removeClass("grey").attr("disabled","");
	}
	...
	$(".goodsName").click(function(){
		...
		if($("#choosedepotName").val()=="金币仓库"){
			
		}
		$.ajax({
			url:basePath+"ea/finished/sajax_ea_ajaxGetGoodsInformation.jspa?category="+category+"&fiveClear="+fiveClear,
			type:"post",
			async : false,
	...
	$(".butt").click(function(){
		if($(this).val()=="保存"){
			...
			if(ir>0)
				return;
			$("#form").attr("target", "hidden").attr("action",basePath+"ea/finished/ea_addOutboundOrder.jspa");
	...
	$(".goodsButton").click(function(){
		if($(this).val()=="查询"){
			...
			$(this).parent().parent().addClass("goods");
			$.ajax({
				url:basePath+"ea/finished/sajax_ea_ajaxGetGoodsInformation.jspa?likeGoodsName="+$("#likeGoodsName").val()+"&likeCategory="+$("#likeCategory").val()+"&category="+category+"&fiveClear="+fiveClear,
				type:"post",
				async : false,
				success:function(data){
					var member = eval("(" + data + ")");
			...
			var lex=true,profitMargin="",profit="",pretium="";
			if(status==1){
				$.ajax({
			...
			if(status==1){
				$(".goods").find("input").eq(9).val(parseInt(pretium)-parseInt($("#"+goodsId).find("td").eq(5).text()));
				$(".goods").find("input").eq(10).val(pretium);
				$(".goods").find("input").eq(11).val("");
			}else{
				$(".goods").find("input").eq(9).val("");
			}
	...
	$("#ckok").click(function() {
		...
		$(".hidetr").attr("style","display: none;");
			if(depotName=="销售库"){
				...
				$("#tr").find("td").eq(7).find("input").addClass("profit").attr("name","profitAmount").attr("readonly","");
				...
				$("#sale").find("th").attr("width","12.15%");
				for(var i=0;i<2;i++){
					$("#sale").find("th").eq(7).remove();		
					$("#tr").find("td").eq(7).remove();
				}
				$("#tr").find("td").eq(7).find("input").attr("readonly","");
				$("#sale").attr("style","");
			}
...
$(function(){
	...
	$(".Warehouse").click(function(){
		if("wu"==$(this).attr("name"))
			return;
	    var srcUrl=basePath +"ea/cdepotmanage1/ea_getListDepotManageTreenew.jspa?pageNumber="+pageNumber+"&depotID="+ $(this).attr("id") + "&usetype=ck&deType=screen&fiveClear="+fiveClear+"&fiveClearName="+fiveClearName;
	    $("#mainframe").attr(
		"src",srcUrl);
		$(window).resize();
	});
...
function queryWarehouse(direction,obj){
    var	getListCCodeurl=basePath+"ea/cdepotmanage/sajax_ea_getListDepotmanage.jspa?depotID="+obj+"&date="+new Date().toLocaleString();
    $.ajax({
