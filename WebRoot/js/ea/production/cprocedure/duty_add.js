$(document).ready(function() {
	...
	if(dutyid!=null&&dutyid!=""){
		...
		var w=$("#b").clone();
		w.find("table").attr("id","clone0");
		w.find("input").each(function(){
			$(this).attr("id","clo"+$(this).attr("id"));
		});
		$("#adda").html("<br/>"+w.html()+"<br/>");
		$("#adda").find("input:eq(0)").val("00:00:00");
		$("#adda").find("input:eq(1)").val("23:59:59");
	}
	...
	$("#selecttype").change(function(){
		...
		var d="<br/>";
		for(var i=0;i<b;i++){
			$c.find("table").attr("id","clone"+i);
			$c.find("input").each(function(index){
				if(index==0||index==1){
					$(this).attr("class","timeclone");
				}
				$(this).attr("id","clo"+i+"-"+index);
				var f="dutyb["+i+"]."+$(this).attr("title");
				$(this).attr("name",f);
			});
			d+=$c.html()+"<br/>";
		}
		$("#adda").append(d);
		$("#adda").find("input.timeclone").each(function(index){
			var id=$(this).attr("id");
			var top=id.substring(3,4);
			var bom=id.substring(5,6);
			if(index==0){
				$(this).val("00:00:00");
			}else if(index==(parseInt(b)*2-1)){
				$(this).val("23:59:59");
			}else if(bom==0){
				$(this).val(parseInt(g)*(parseInt(top))+":00:00");
			}else{
				$(this).val(parseInt(g)*(parseInt(top)+1)-1+":59:59");
			}
		});
	});
	...
	$(".save").click(function() {
		...
		if(dutyid==null||dutyid==""){
			var str=$("#selecttype option:selected").val();
			$("#dutyType").val(str);
		}
		...
		$("form :input:.positiveNumZ").trigger("blur");
          ...
          { 
			...
			$("#mainForm").attr("target", "hidden").attr("action",
					basePath + "/ea/duty/ea_savePuty.jspa");
	...
	$(function() {
		...
		$("#qduser").click(function() {
			if (cuID != "") {
				...
				var bznum=$("#bznum").val();
			     $("#"+bznum).find(".staffid").val(contactUserID);
			     $("#"+bznum).find(".staffname").val(name);
			     $("#members").hide();
...
function ss($but){
	var id=$but.id;
	$("#bznum").val($("#"+id).parent().parent().parent().parent().attr("id"));
}
// 查询项目产品
function pcx(typeCN) {
	if (notoken) {
