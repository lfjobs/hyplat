...
function ajax() {
	var url = basePath + "ea/companyforum/sajax_ea_myFans.jspa?";
	$.ajax({
		url : encodeURI(url),
		type : "post",
		async : false,
			...
			if (pageForm != null && pageForm.list != null
				...
				$(pageForm.list).each(function(i, dom) {
					if ($(pageForm.list).length - 1 == i) {
						forum.push("<li class='last' onclick='fans(this)'>");
					} else {
						forum.push("<li onclick='fans(this)'>");
					}
