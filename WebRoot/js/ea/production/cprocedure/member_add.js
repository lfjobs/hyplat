          ...
          { 
            return;
			 ...
			 $("#mainForm").attr("target", "hidden").attr("action",
					basePath + "ea/member/ea_savePlan.jspa?category="+category+"&type="+type);
	...
	$(function() {
		...
		$("#qduser").click(function() {
			...
			if (cuID != "") {
				...
				$obj.parent().find(".ids").val(contactUserID);
				$obj.parent().find(".names").val(name);
...
function pcx(typeCN) {
	...
	var searchurl = basePath
			+ "ea/assembly/sajax_ea_ajaxGetProductionList.jspa?category="+category+"&fiveClear="+fiveClear;

	$.ajax({
		url : encodeURI(searchurl + typeCN + "&date="
				+ new Date().toLocaleString()+"&type=01"),
		type : "get",
		async : true,
		dataType : "json",
		data : {
		...
		success : function cbf(data) {
			...
			for (var i = 0; i < pageForm.list.length; i++) {
				tabletr += "<tr style='cursor: hand;' id = "
						+ (pageForm.list[i][0]==null?"":pageForm.list[i][0] )+ ">";
				tabletr += "<td height='27' id='check' align='center'>"
						+ "<input type ='radio' class='raporducts' value="
						+ (pageForm.list[i][0]==null?"":pageForm.list[i][0]) + " name='check'/></td>";
				...
				tabletr += "<td id='ppID' align='center' style='display:none;'>"
						+ (pageForm.list[i][0]==null?"":pageForm.list[i][0] )+"</td>";
				tabletr += " </tr>";
			}
			tabletr += " </table>";
