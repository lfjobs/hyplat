...
function toAddItemBack(){
    var url = "ea_scanningPlanCostBudgetInfo.jspa";
    ...
    for (var i = 0; i < oList.length; i++) {
        ...
        if (obj.barCode != null && obj.barCode != "") {
            ...
            innerHtml += '<td colspan="2" id="' + obj.barCode + '" cusXmtype="' + obj.xmtype + '" cusXmcode="' + obj.goodsNum + '">'
                ...
                + '</td>'
                + '<td>' + obj.barCode + '</td>'
                + '<td>' + (obj.goodsName != null ? obj.goodsName : "") + '</td>'
                + '</tr>';
        }
    }
    return innerHtml;
...
$(document).on("click",".ul-list2 li",function(){
    ...
    if($(this).is(".active")){
        ...
        window.location.href = basePath + "ea/scBudget/ea_getPlanCostBudgetItem.jspa?keyNum=" + goodsBillsId +"&editFlag="+editFlag+"&cashierBillsId="+cashierBillsId;//&showFlag=${showFlag}&departmentID=${departmentID}&cashierBillsId=${cashierBillsId}";//&menuId="+menuId;
        $(this).removeClass("active");
    }
})