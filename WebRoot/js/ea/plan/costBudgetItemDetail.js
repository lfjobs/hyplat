...
$(function () {
    ...
    var rewardData = ["升级","奖金"];
    // crateListPop("launchForm",rewardData,"teamRewardPop","团队奖励","teamReward","tdjl1","tdjl2");
    // crateListPop("launchForm",rewardData,"personalRewardPop","个人奖励","personalReward","grjl1","grjl2");

    var punishmentData = ["降级","扣款"];
    // crateListPop("launchForm",punishmentData,"teamPunishmentPop","团队奖励","teamPunishment","tdzk1","tdzk2");
    // crateListPop("launchForm",punishmentData,"personalPunishmentPop","个人奖励","personalPunishment","grzk1","grzk2");
    ...
    if (true) {
        ...
        if ($("#variableID").val() != null && $("#variableID").val() != "") {
            $(".danwei1").hide();
            $(".danwei2").show();
            if($("#variableID").val()=="时间"){
                $("#budgetNumber").attr("readonly", true);
            }else{
                $("#budgetNumber").removeAttr("readonly");
            }
        }
        ...
        if ($("#teamPunishment").val() != null && $("#teamPunishment").val() != "") {
            $(".tdzk1").hide();
            $(".tdzk2").show();
        }
        if ($("#personalPunishment").val() != null && $("#personalPunishment").val() != "") {
            $(".grzk1").hide();
            $(".grzk2").show();
        }
        if ($("#teamReward").val() != null && $("#teamReward").val() != "") {
            $(".tdjl1").hide();
            $(".tdjl2").show();
        }
        if ($("#personalReward").val() != null && $("#personalReward").val() != "") {
            $(".grjl1").hide();
            $(".grjl2").show();
        }
    ...
    $(document).on("click", ".div-shipin2 .ul-top li", function () {
        $(this).addClass("active");
        $(this).siblings().removeClass("active");
        $(".div-shipin2 .div-tab .div-01").hide();
        $(".div-shipin2 .div-tab .div-01").eq($(this).index()).show();
    });
    // 	传入选项 更改显示模块
    ...
    $(document).on("click", ".wldw1,.wldw2", function () {
        ...
        if (accountFlag=="companyPlan") {
            // getAttri2($("#goodsID").val());
            $(".div-account-company").show();
            //getRecentInfoCompany();
        } else if(accountFlag=="personalPlan"){
            // getAttri1($("#goodsID").val());
            $(".div-account-personal").show();
            //getRecentInfoPersonal();
        }
    ...
    $(document).on("click", ".wldw3,.wldw4", function () {
        var accountFlagFrom = $("#accountFlagFrom").val();
        if (accountFlagFrom=="companyPlan") {
            // getAttri2($("#goodsID").val());
            $(".div-account-companyFrom").show();
            //getRecentInfoCompany();
        } else if(accountFlagFrom=="personalPlan"){
            // getAttri1($("#goodsID").val());
    ...
    $(document).on("click", ".div-accountType ul li", function () {
        ...
        $("#accountFlag").val($(this).attr("atype"));
    ...
    function getRecentInfoCompany(comClass){
        $.ajax({
            ...
            aysnc:false,
            data:{
                accountFlag:"companyPlan"//初始项目——往来单位最近联系人
            },
            success:function(data){
                if(data!=null) {
    ...
    function getRecentInfoPersonal(perClass){
        $.ajax({
            ...
            aysnc:false,
            data:{
                accountFlag:"personalPlan"//初始项目——往来个人最近联系人
            },
            success:function(data){
                if(data!=null) {
                    var htmlstr = new Array();
    ...
    $.ajax({
        ...
        success: function cbf(data) {
            ...
            if(unit == "时间" ){
                if(oList[0].specs=="起止时间" || oList[0].specs=="截止时间"){
                    ...
                    if(oList[0].specs!="小时"){
                        ...
                        for (var i = 0; i < arrList.length; i++) {
                            ...
                            // str.push("<input type='text' info='"+oList[i].specs+"' class='attri1'>");
                            if(i==0){
                                str.push("<input type='text' info='"+arrList[i].specs+"' id="+i+" class='attri1' readonly autocomplete='off' onfocus=\"WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss', maxDate:'#F{$dp.$D(\\'"+(i+1)+"\\')}'})\" >");
                            } else {
                                str.push("<input type='text' info='"+arrList[i].specs+"' id="+i+" class='attri1' readonly autocomplete='off' onfocus=\"WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss', minDate:'#F{$dp.$D(\\'"+(i-1)+"\\')}'})\" >");
                            }
                ...
                // str.push("<li class='clearfix'>");
                // str.push("<p>数量</p>");
                // str.push("<input id='budgetNumber2' type='text' readonly>");
                // str.push("</li>");
                // str.push("<li class='clearfix'>");
                // str.push("<p>单价</p>");
                // str.push("<input id='unitPrice2' type='text'>");
                // str.push("</li>");
            }else{
                for (var i = 0; i < oList.length; i++) {
                    str.push("<li class='clearfix'>");
...
// 团队奖励
// 	显示
$(document).on("click", ".tdjl1,.tdjl2", function () {
    $("#teamRewardPop").show();
});
$(document).on("click", ".grjl1,.grjl2", function () {
    $("#personalRewardPop").show();
});
$(document).on("click", ".tdzk1,.tdzk2", function () {
    $("#teamPunishmentPop").show();
});
$(document).on("click", ".grzk1,.grzk2", function () {
    $("#personalPunishmentPop").show();
});
