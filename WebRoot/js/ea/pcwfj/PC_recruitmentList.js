		...
		success:function(data){
			...
			if (pageForm != null && pageForm.list != null
				...
				$(pageForm.list).each(function(i) {
                    ...
                    array.push("<input class='tpID' type='hidden' value='"+this[7]+"'/>");
					array.push("<input class='riID' type='hidden' value='"+this[0]+"'/>");
					array.push("<li class='job_title'>"+(this[1]==null||this[1]=='null'?'':this[1])+"</li>");
					array.push("<li class='job_category'>"+(this[2]==null||this[2]=='null'?'':this[2])+"</li>");
					// array.push("<li class='number'>"+(this[3]==null||this[3]=='null'?'':this[3])+"</li>");
					array.push("<li class='site'>"+(this[4]==null||this[4]=='null'?'':this[4])+"</li>");
					array.push("<li class='pubdate'>"+(this[5]==null||this[5]=='null'?'':this[5])+"</li>");
					array.push("</ul>");
				});
			}
			$("#position").empty();
		...
		success:function(data){
			...
			if (pageForm != null && pageForm.list != null
				...
				$(pageForm.list).each(function() {
					array.push("<li><label onclick='hotDetails(this)'>");
					array.push("<input class='riID' type='hidden' value='"+this[0]+"'/>");
...
function selPosition(){
	...
	$.ajax({
		...
		data:{
			"pageForm.pageNumber":pageNumber,
			"pageForm.pageSize":10,
			"recruitInfo.jobTitle":positionName,
		        ...
		        for ( var i = 1; i <= pageCount; i++) {
		        	if(pageNumber==i){
		        		...
		        		if(i==1||i==pageCount||(i>=pageNumber-1&&i<=pageNumber+1)){
		        			...
		        			paging.push("<li style='border:none;background-color:white;' class='poin·t'><span>...</span></li>");
		        		}
		        	}
				}
		        paging.push("</ul><button style='float: right;' onclick='nextStep();'><img src='"+basePath+"page/newMyapp/images/newPCHomepage/page-right.png'></button>");
...
function details(obj){
	...
	var url=basePath+"ea/newpcend/ea_pcRecruitmentDetails.jspa?recruitInfo.riId="+riID+"&tpID="+tpID;
	window.location.href=url;
}
//热门职位详情
function hotDetails(obj) {
    var riID=$(obj).find(".riID").val();
    var url=basePath+"ea/newpcend/ea_pcHotRecruitmentDetails.jspa?recruitInfo.riId="+riID;
    window.location.href=url;
}
