    ...
    $.ajax({
        ...
        success: function cbf(data){
            ...
            if(member){
                ...
                var oList = [{"menuId":"ng","text":"拟稿("+member.ng+")"},{"menuId":"cy","text":"传阅("+member.ng+")"},{"menuId":"receive","text":"收件("+member.receive+")"},{"menuId":"sent","text":"已发送("+member.sent+")"}];
...
function toMenuClick(id,depName){
    ...
    if(id == "ng" || id == "cy"){
        ...
        url = "ea/scBudget/ea_toCostBudgetBillList.jspa?tenantFlag="+tenantFlag+"&cashierBillsId="+cashierBillsId+"&menuId="+id+"&billsType="+billsType+"&head="+head;
    }else if(id == "receive"){
        url = "ea/scBudget/ea_toCostBudgetBillReceiveList.jspa?menuId="+id+"&billsType="+billsType+"&head="+head;
    } else if(id == "sent"){
        url = "ea/scBudget/ea_toCostBudgetBillSentList.jspa?menuId="+id+"&billsType="+billsType+"&head="+head;
    } else if(id == "approval"){
        url = "ea/scBudget/ea_toCostBudgetBillApprovalList.jspa?menuId="+id+"&billsType="+billsType+"&head="+head;
    }
    // console.log("路径："+url);
    if(url){
