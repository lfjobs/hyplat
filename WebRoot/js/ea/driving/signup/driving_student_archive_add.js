		...
		if (pabillID == "") {
			$("#PArchiveForm")
					...
					.attr(
							"action",
							basePath
									+ "/ea/clinch/ea_saveStudentArchive.jspa?pageNumber="
									+ pNumber);
...
function re_load() {
	if (token)
		document.location.href = basePath
				+ "ea/clinch/ea_getListStudentArchive.jspa?pageNumber="
				+ pNumber+ "&search=" + search+"&module_Identifier=customer_Deal&view_Identifier=educational_train&pageForm.pageNumber="
...
function getArchive(typeCN){
	...
	notoken = 1;
	var searchurl = basePath
			+ "ea/clinch/sajax_ea_getStudentArchive.jspa?";
	$.ajax({
		url : encodeURI(searchurl + typeCN + "&date="
				+ new Date().toLocaleString()),
		type : "get",
