$(document).ready(function() {
	...
	parent.document.getElementById("mainframe6").style.height = 80 + len * 27 + "px";
	$('.contact').flexigrid({
				height : 'auto',
				width : 'auto',
				minwidth : 30,
	...
	function action(com, grid) {
		switch (com) {
			...
			case '全部保存' :
				...
				notoken = 1;
				var re = 0;
				$("#applyDate", $(".check")).each(function(i, tmp) {
					if (this.value == "") {
						alert("请输入日期");
...
function re_load() {
	if (token)
		document.location.href = basePath
				+ "ea/enroll/ea_getStudentPapersList.jspa?dtDrivingAllInformation.dataTitle=06&dtDrivingAllInformation.staffID="+ staffID+"&dtDrivingAllInformation.relationID="+relationID;
}