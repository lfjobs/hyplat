$(document).ready(function() {
	...
	$('.address').flexigrid({
				...
				width : 'auto',
				minwidth : 30,
				title : '测试合格率分析 > ' +(docstatus=='01'?'科一':docstatus=='02'?'科二':docstatus=='03'?'科三':docstatus=='04'?'科四':'汇总')+searchFormHtml,
				minheight : 50,
				buttons : [  {
					name : '设置每页显示条数',
					bclass : 'mysearch',
	...
	function action(com, grid) {
		switch (com) {
			...
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/driving/ea_getAnalysisListOfTest.jspa?docstatus="+docstatus+"&sdate="+sdate+"&edate="+edate+"&search="+search+"&other="+other;
				numback(url);
				break;
			case '导出' :
				var url = basePath
						+ "ea/driving/ea_getAnalysisListOfTest.jspa?docstatus="+docstatus+"&sdate="+sdate+"&edate="+edate+"&search="+search+"&excel=excel";
				open(url);
				break;
			case '(个人)教练业绩明细表打印' :
	 ...
	 $("#tosearch").click(function(){
            ...
            $("#SearchForm").attr("action", basePath+"ea/driving/ea_toSearchAnalysisOfTest.jspa?other="+other);
