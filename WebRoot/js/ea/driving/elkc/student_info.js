$(function() {
	...
	var query = "<form name='cstaffSearchForm' id='cstaffSearchForm' method='post'>" +
			"<span style=\"margin-left:20px;\">学员姓名：</span>" +
			"<input type='text' style=\"width: 90px\" id='name' name='staffName'/>" +
			"<span style=\"margin-left:10px;\">身份证：</span>" +
		    "<input type='text' style=\"width: 130px\" id='card' name='cardnum'/>" +
	...
	function action(com, grid) {
		switch (com) {
			case '修改' :
				...
				break;
			case '导出' :
				url = pbasePath + "ea/coachreserv/ea_excl.jspa?search="+psearch+"&showType="+showType+"&parameter="+parameter;
				open(url);
				break;
			case '导入' :
			...
			case '设置每页显示条数' :
				var url = pbasePath
						+ "/ea/coachreserv/ea_registrationTrainee.jspa?search="
            ...
            $.ajax({
                url:pbasePath+"/ea/coachreserv/sajax_ea_SyncStudent.jspa?",
                type : "post",
                async : true,
                data : {
                   "staffId":personvalue
	...
	$(".JQueryflexme tr[id]").click(function() {
         ...
         staffCode = $(this).find("span#staffCode").text();
       staffName = $(this).find("span#staffName").text();
       sex = $(this).find("span#sex").text();
         cardnum = $(this).find("span#cardnum").text();
         phone = $(this).find("span#phone").text();
        brith = $(this).find("span#brith").text();
         trainTypes = $(this).find("span#trainType").text();
    ...
    $("#searchStaff").click(function() {
        ...
        $("#cstaffSearchForm").attr(
            "action",
            pbasePath + "/ea/coachreserv/ea_registrationTrainee.jspa?pageNumber="
            + ppageNumber + "&aa=" + aa +"&flexbutton="+flexbutton+"&staffname="+name+"&staffCode="+card);
