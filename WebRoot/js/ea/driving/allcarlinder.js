$(document).ready(function() {
	...
	function action(com, grid) {
		switch (com) {
			case '检测' :
				if(jQuery.trim(cylinderType) == ""){
					alert("请选择一个气瓶！");
					return;
					
				}
					if(cylinderType == "G:钢瓶"){
						window.location.href = basePath + "ea/productregister/ea_detectionInformation.jspa?carlinderID="+ carCylinderId+"&bottletype=cylinders";
						//window.location.href = basePath + "page/ea/main/driving/cylinderdetection.jsp";
					}else{
						window.location.href = basePath + "ea/productregister/ea_detectionInformation.jspa?carlinderID="+ carCylinderId+"&bottletype=Winding ";
					}
				break;
			case '查看' :
				if(carCylinderId == ""){
