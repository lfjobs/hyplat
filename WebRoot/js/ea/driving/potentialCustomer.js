$(document).ready(function() {
    ...
    function action(com, grid){
        switch (com) {
			case '查询' :
		     ...
		     case '设置每页显示条数':
			   	var url = basePath
						+ "/ea/customermanage/ea_getPotentialList.jspa?frameid="+frameid+"&goodsid="+goodsid+"&status1="+status1+"&search="+search;
				numback(url);
				break; 
        }
    }
	...
	$("#tosearch").click(function(){
		$("#SearchForm").attr("action",
				basePath + "/ea/customermanage/ea_toSearchPotential.jspa?frameid="+frameid+"&goodsid="+goodsid+"&status1="+status1+"&pageNumber=" + pNumber);
