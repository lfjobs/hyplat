$(document).ready(function() {
	...
	$('.JQueryflexme').flexigrid({
				...
				width : 'auto',
				minwidth : 30,
				title : (docstatus=='01'?'科一':docstatus=='02'?'科二':docstatus=='03'?'科三':'科四')+"预约考试"+searchFormHtml,
				minheight : 80,
		...
		switch (com) {
			...
			case '打印考试记录登记表' :
				...
				window.open(basePath
						+ "/ea/driving/ea_toPrintOfDrivingExam.jspa?cstaff.staffID="
	...
	$("#tosearch").click(function() { // 查询
				$("#SearchForm").attr(
						...
						basePath
								...
								+ pNumber+"&docstatus="+docstatus+"&studentstatus="+studentstatus+"&title="+title+"&other="+other+"&extensionStaffCoach="+extensionStaffCoach+"&dtDrivingPrincipal.studentid="+studentID );
...
function re_load() {
	if (token)
		document.location.href= basePath
				...
				+ pNumber+"&docstatus="+docstatus+"&studentstatus="+studentstatus+"&title="+title+"&search="+search+"&conditions="+conditions+"&other="+other+"&extensionStaffCoach="+extensionStaffCoach+"&dtDrivingPrincipal.studentid="+studentID ;
}