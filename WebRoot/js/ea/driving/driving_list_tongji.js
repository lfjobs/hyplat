$(document).ready(function() {
	...
	function action(com, grid) {
		switch (com) {
			...
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/driving/ea_getStatisticsList.jspa?docstatus="+docstatus+"&sdate="+sdate+"&edate="+edate+"&search="+search+"&other="+other
						+"&extensionStaffCoach="+extensionStaffCoach+"&dtDrivingPrincipal.studentid="+studentID;
				numback(url);
				break;
			case '导出':
				var url = basePath
						+ "ea/driving/ea_getStatisticsList.jspa?docstatus="+docstatus+"&sdate="+sdate+"&edate="+edate+"&search="+search+"&excel=excel"
						+"&extensionStaffCoach="+extensionStaffCoach+"&dtDrivingPrincipal.studentid="+studentID;
				open(url);
				break;	
			case com:
				...
				$("input[type='checkbox']",$("table.JQueryflexme")).each(function(i) {
							if ($(this).is(':checked')) {
								...
								$(this).parents("tr").find(":input.data").each(function(){
									$(this).attr("name","dtDrivingTestMap["+i+"]."+$(this).attr("name"));
								});
								drivingtestid = $(this).val();
								var examresult1=$("tr[id=" + drivingtestid+"]").find("span#examresult").text();
								var examresult2=$.trim($("tr[id=" + drivingtestid+"]").find("input#examScore").val());
	...
	$("#tosearch").click(function(){
                $(".put3").trigger("blur");
                   if ($("#SearchForm .error").length) {
                    ...
                    $("#SearchForm").attr("action", basePath+"ea/driving/ea_toSearchStatistics.jspa?other="+other
                    +"&extensionStaffCoach="+extensionStaffCoach+"&dtDrivingPrincipal.studentid="+studentID);
	...
	/*$("tr[id]").dblclick(function(){
		...
		$(this).find(":input[id]").each(function(){
			$(this).val($t.find("span[id="+$(this).attr("id")+"]").text()).show();;
			$t.find("span[id="+$(this).attr("id")+"]").hide();
		});
		$t.find("a[id][class]").each(function(){
			$(this).show();
		});
	});*/
});
function re_load() {
	if (token)
		document.location.href = basePath
				+ "ea/driving/ea_getStatisticsList.jspa?pageNumber=" + pNumber + "&pageForm.pageNumber="
