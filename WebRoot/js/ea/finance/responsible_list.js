$(function() {
	...
	var query ="<form method='post' name='SearchForm' id='SearchForm'><input type='submit' style='display:none;' name='submit' /><input type='hidden' name='search' value='search'/>待主管审核列表"	
		+ "<table id='SearchTable' a><tr>"
		...
		switch (com) {
			...
			case '查看历史单据' :
				...
				document.location.href = basePath
						+ "ea/responsible/ea_toSearch.jspa?search=search&cashierBillsVO.status=hostStatus&pageNumber"
						+ pNumber;
				break;
			case '转会计审核' :
				if (hostStatus == "hostStatus") {
			...
			case '审核记录' :
				...
				+ "ea/responsible/sajax_ea_AjxaGetCheckbill.jspa?cashierBillsID="+cashierBillsID+"&date="
				...
				break;
			case '导出' :
				url = basePath + "ea/responsible/ea_showExcel.jspa?search="
						+ search + "&sdate=" + sdate + "&edate=" + edate;
				open(url);
				break;
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/responsible/ea_getResponsibleList.jspa?search="
						+ search + "&sdate=" + sdate + "&edate=" + edate;
				numback(url);
				break;
			case '查询' :
	...
	var opt=$("#btype").find("option:selected").text();
		...
		$("#SearchForm").attr(
				"action",
				basePath + "ea/responsible/ea_toSearch.jspa?pageNumber="
						+ pNumber);
		...
		if (confirm("确定执行该操作？")) {
			$form
					...
					.attr(
							"action",
							basePath
									+ "ea/responsible/ea_updateResponsible.jspa?search="
...
function re_load() {
	document.location.href = basePath
			+ "ea/responsible/ea_getResponsibleList.jspa?pageNumber=" + pNumber
