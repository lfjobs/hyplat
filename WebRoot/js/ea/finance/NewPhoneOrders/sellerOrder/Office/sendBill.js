...
function loaded(obj, pl) {
    //clearTimeout(t);
    pagenumber++;
    url = "ea/seller/sajax_ea_getSendAjax.jspa?";
    $.ajax({
        url: url,
        type: "post",
        async: false,
        ...
        success: function (data) {
            ...
            if (pageForm != null && pageForm.recordCount > 0) {
                for (var i = 0; i < list.length; i++) {
                    ...
                    htl.push("<ul class='ul-con'  onClick='gatSend(&quot;" + pp[0] + "&quot;)'>");
                    htl.push('<li>发货单号：' + pp[1] + "</li>");
                    htl.push('<li>订单号：' + pp[2] + "</li>");
                    htl.push('<li>下单时间：' + pp[3] + "</li>");
                    htl.push('<li>发货时间：' + pp[4] + "</li>");
                    ...
                    if (pp[7] == '01') {
                        ...
                        htl.push("<p onClick='gatSend(&quot;" + pp[0] + "&quot;)'>提交送货</p>");
                        htl.push("</section>");
                    }
                    htl.push('</div>');
                }
...
function gatSend(sendid) {
    var url = "ea/seller/ea_getSendGoods.jspa?sendid=" + sendid+"&staffid=" + staffid+"&sort="+sort;
