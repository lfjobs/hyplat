...
function loaded(obj, pl) {
    //clearTimeout(t);
    pagenumber++;
    url = "ea/seller/sajax_ea_getDeliveryAjax.jspa?";
    $.ajax({
        url: url,
        type: "post",
        async: false,
        ...
        success: function (data) {
            ...
            if (pageForm != null && pageForm.recordCount > 0) {
                for (var i = 0; i < list.length; i++) {
                    ...
                    htl.push("<ul class='ul-con'  onClick='gatDetails(&quot;" + pp[0] + "&quot;)'>");
                    htl.push('<li>拣货出库号：' + pp[1] + "</li>");
                    htl.push('<li>订单号：' + pp[2] + "</li>");
                    htl.push('<li>下单时间：' + pp[3] + "</li>");
                    htl.push('<li>拣货时间：' + pp[4] + "</li>");
                    htl.push('<li>采购商：' + pp[5] + "</li>");
                    htl.push('<li>拣货人：' + pp[6] + "</li>");
                    ...
                    if (pp[7] == '01') {
                        ...
                        htl.push("<p onClick='gatDetails(&quot;" + pp[0] + "&quot;)'>提交发货</p>");
                        htl.push("</section>");
                    }
                    htl.push('</div>');
                }
...
function gatDetails(ordid) {
    var url = "ea/seller/ea_getDeliveryDetails.jspa?ordid=" + ordid+"&staffid=" + staffid+"&sort="+sort;
