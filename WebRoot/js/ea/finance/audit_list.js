$(function() {
		...
		switch (com) {
				...
				if (confirm("确定驳回？")) {
					$form
							...
							.attr(
									"action",
									basePath
											+ "/ea/audit/ea_updateResponsible.jspa?search="
					...
					$form
							...
							.attr(
									...
									basePath
											...
											+ search + "&pageNumber=" + pNumber);
					$form.find("input#cashierBillsID").val(cashierBillsID);
					$form.find("input#taxstatus").val("04");
					document.Auditform.submit.click();
					$("tr#" + cashierBillsID).remove();
					cashierBillsID = "";
					token = 12;
			...
			case '查看' :
				...
				document.location.href = basePath
						+ "/ea/audit/ea_toCashier.jspa?pageNumber=" + pNumber
				...
				break;
			case '导出' :
				url = basePath + "/ea/audit/ea_showExcel.jspa?search=" + search
						+ "&sdate=" + sdate + "&edate=" + edate + "&tsdate="
						+ tsdate + "&tedate=" + tedate + "&taxstatusDu="
						+ taxstatusDu;
				open(url);
				break;
			case '设置每页显示条数' :
				var url = basePath
						+ "/ea/audit/ea_getDutiableList.jspa?search=" + search
						+ "&sdate=" + sdate + "&edate=" + edate + "&tsdate="
						+ tsdate + "&tedate=" + tedate;
				numback(url);
				break;
			case '查询' :
	...
	var opt=$("#btype").find("option:selected").text();
		...
		$("#SearchForm").attr("action",
				basePath + "/ea/audit/ea_toSearch.jspa?pageNumber=" + pNumber);
...
function re_load() {
	document.location.href = basePath
			+ "/ea/audit/ea_getDutiableList.jspa?pageNumber=" + pNumber
