       ...
       $(".jqmreturn").click(function(){
                ...
                function action(com, grid){
                    switch (com) {
                    	case '驳回':
							...
							if (confirm("确定驳回？")){
	                            $form.attr("target","hidden").attr("action", basePath+"/ea/manager/ea_updateResponsible.jspa?search="+search+"&pageNumber="+pNumber);
	                            $form.find("input#cashierBillsID").val(cashierBillsID);
	                            $form.find("input#taxstatus").val("10");
	                            document.Managerform.submit.click();
                        ...
                        case '转财务审核':
							...
							if (confirm("确定转财务审核？")){
	                            $form.attr("target","hidden").attr("action", basePath+"/ea/manager/ea_updateResponsible.jspa?search="+search+"&pageNumber="+pNumber);
	                            $form.find("input#cashierBillsID").val(cashierBillsID);
	                            $form.find("input#taxstatus").val("03");
	                            document.Managerform.submit.click();
	                            $("tr#"+cashierBillsID).remove();
	                            cashierBillsID = "";
	                            token = 12;
                        ...
                        case '查看':
                            ...
                            document.location.href = basePath+"/ea/manager/ea_toCashier.jspa?pageNumber="+pNumber+"&cashierBills.cashierBillsID="+cashierBillsID+"&sdate="+sdate+"&edate="+edate+"&search="+search+"&tsdate="+tsdate+"&tedate="+tedate+"&taxstatusDu="+taxstatusDu;
                            break;
                        case '导出':
							url = basePath+"/ea/manager/ea_showExcel.jspa?search="+search+"&sdate="+sdate+"&edate="+edate+"&tsdate="+tsdate+"&tedate="+tedate+"&taxstatusDu="+taxstatusDu;
							open(url);
                            break;
                        case '设置每页显示条数':
						    var url=basePath+"/ea/manager/ea_getDutiableList.jspa?search="+search+"&sdate="+sdate+"&edate="+edate+"&tsdate="+tsdate+"&tedate="+tedate;
							numback(url);
							break;  
					    case '查询':
		...
		if(opt=="请选择"){
			...
			$("#btype").find("option:selected").attr("value",$("#btype").find("option:selected").text().substr(num+1,leng));
                    ...
                    $("#SearchForm").attr("action", basePath+"/ea/manager/ea_toSearch.jspa?pageNumber="+pNumber);
...
function re_load(){
	document.location.href=basePath+"/ea/manager/ea_getDutiableList.jspa?pageNumber="+pNumber+"&pageForm.pageNumber="+$("#pageNumber").attr("value")+"&search="+search+"&sdate="+sdate+"&edate="+edate+"&tsdate="+tsdate+"&tedate="+tedate+"&taxstatusDu="+taxstatusDu;
}
function fj(cID){
