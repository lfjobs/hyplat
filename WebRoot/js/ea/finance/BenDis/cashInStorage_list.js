...
var query = "<form id='form' name='form' method='post'>现金入库单&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" +
...
function action(com, grid) {
	switch (com) {
		...
		case '导出' :
			...
			window.location.href=basePath+"ea/cashStock/ea_CashExportExcelTable.jspa?type=00&title=现金入库单&str="+str;
			break;
		case '查看预览' :
			if(id==""){
				alert("请选择！");
			...
			open(basePath+"ea/cashStock/ea_getGoldPreviewPage.jspa?type=00&utboundOrderVo.cashierbillsid="+id);
			break;
		case '设置每页显示条数':
			var pr=prompt("请输入0~20的正整数");
			if(pr>0&&pr<=20){
				window.location.href=basePath+"ea/cashStock/ea_getCashPreviewPage.jspa?type=00&pageNumber="+pr;
			}else{
				alert("您输入的有误，请重新输入");
			}
			break;
...
$(function(){
	...
	$("#tosearch").click(function(){
		...
		window.location.href=basePath+"ea/cashStock/ea_getCashOfTheHomePage.jspa?type=00" +
