$(function() {
			...
			function action(com, grid) {
				switch (com) {
					...
					case '导出' :
						url = basePath
								+ "ea/goodscashier/ea_showExcel.jspa?search="
								+ search + "&sdate=" + sdate + "&edate="
								+ edate;
						open(url);
						break;
					case '查询历史数据' :
						document.location.href = basePath+ "/ea/goodscashier/ea_getCashierList.jspa?cc=jt";
						break;
					case '设置每页显示条数' :
						var url = basePath
								+ "ea/goodscashier/ea_getCashierList.jspa?search="
								+ search + "&sdate=" + sdate + "&edate="
								+ edate;
						numback(url);
						break;
			...
			$("#btype").find("option:selected").attr("value",$("#btype").find("option:selected").text().substr(num+1,leng));
				...
				$("#SearchForm")
						.attr(
								"action",
								basePath
										+ "ea/goodscashier/ea_toSearch.jspa?pageNumber="
										+ pNumber);
