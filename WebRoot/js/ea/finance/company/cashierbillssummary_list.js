$(function() {
		...
		switch (com) {
			case '查看' :
				...
				document.location.href = basePath
						+ "ea/cashiersummary/ea_toCashier.jspa?pageNumber="
						+ pNumber + "&cashierBillsVO.cashierBillsID="
						+ cashierBillsID;
				break;
			case '导出' :
				url = basePath + "ea/cashiersummary/ea_showExcel.jspa?search="
						+ search + "&sdate=" + sdate + "&edate=" + edate;
				open(url);
				break;
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/cashiersummary/ea_getCashierList.jspa?search="
						+ search + "&sdate=" + sdate + "&edate=" + edate;
				numback(url);
				break;
			case '查询历史数据' :
	...
	var opt=$("#btype").find("option:selected").text();
		...
		$("#SearchForm").attr(
				"action",
				basePath + "ea/cashiersummary/ea_toSearch.jspa?pageNumber="
						+ pNumber);
...
function re_load() {
	document.location.href = basePath
			+ "ea/cashiersummary/ea_getCashierList.jspa?pageNumber=" + pNumber
