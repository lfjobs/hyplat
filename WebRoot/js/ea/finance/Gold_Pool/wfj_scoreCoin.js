...
//	 });
    ...
    $(".exchange").click(function(){
        ...
        //生成订单号
        $.ajax({
            url:basePath+"/ea/bonuspoints/sajax_getJum.jspa?d="+new Date(),
            type:"get",
            data:+object.join("")+"&user="+user+"&khd="+khd+"&flag="+flag,
            async: false,
...
function zf(){
    ...
    if(zffs==null){
        ...
        if(zffs=='1'){
            ...
            par.push("&WIDsubject=(北京天太世统科技有限公司)微分金积分");
            ...
            var goodsname = "(北京天太世统科技有限公司)微分金积分";
            ...
            if (!isWeixin) {
                ...
                //app微信支付
                $.ajax({
                    url:basePath+"ea/wfjshop/sajax_ea_appWechatPay.jspa?d="+new Date(),
                    type:"get",
                    async : false,
                    data:{
                        "payDto.orderId":jum,
                    ...
                    success:function suc(data){
                        ...
                        if(isAndroid==true){
                            ...
                            Android.callAndroidappChat(appid,partnerid,prepayid,packages,noncestr,timestamp,sign,jum,err_code,goodsname,'wfjjf');
                        }else if(isiOS==true){

                            var url= "func=" + 'ioscallappChat';
                            params={'appid':appid,'partnerid':partnerid,'prepayid':prepayid,'noncestr':noncestr,'timestamp':timestamp,'sign':sign,'journalNum':jum,'errcode':err_code,'goodsname':goodsname,'attach':'wfjjf'};
                            for(var i in params){
                                url = url + "&" + i + "=" + params[i];
                            }
