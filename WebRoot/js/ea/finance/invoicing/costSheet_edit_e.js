$(function() {
	...
	if(zctype=="t"){
	$(".tdiv").find(":input").attr("disabled",true);
	$(".jisuant").attr("disabled",false);
	
	$(".addline").attr("disabled",true).addClass("grey");
	$(".deleteline").attr("disabled",true).addClass("grey");
	$(".btncon").attr("disabled",true).addClass("grey");
	$(".remark").attr("disabled",true).addClass("grey");
	$(".klsc").parent().text("无");
	}else{
		
		$(".tdiv").find(":input").attr("disabled",false);
...
//	 }
	 ...
	 if($(".updatesheet").attr("disabled")==true){
		 $(".print").attr("disabled",true).addClass("grey");
		 $(".audit").attr("disabled",true).addClass("grey");
		 $(".deletesheet").attr("disabled",true).addClass("grey");
				...
				if (this.value != "") {
					if (isNaN(this.value)) {
						...
						if (!isNaN(dj) && !isNaN(price)) {
							...
							jine = Math.round(jine * 100) / 100;
							$(this).parent().parent().parent().find(
									":input#tiaoMoney").attr("value", jine);
						}
					}
					...
					function() {
                        ...
                        $("#CostSheetForm")
								.attr(
										"action",
										basePath
												+ "/ea/costsheet/ea_saveCostSheet.jspa?pageNumber="
												+ pNumber);
				...
				function() {
					...
					.click(
							function() {
								window
										.open(basePath
												+ "ea/productdesign/ea_getListProductdesign.jspa?ghua=ghua");
							});
				...
				function() {
					...
					$("input#searchdept").click(
							function() {
								...
								getStaffMember("parameter="+parameter+"&selecDept="+selectDept);
							});
							...
							function() {
								...
								if (sy != 0) {
									...
									var parameter = $("input#parameterrm",
											$("table#searchdept")).val();
									var selectDept = $(":input#selectdept").val();
									getStaffMember("parameter="+parameter+"&selecDept="+selectDept+"&pageForm.pageNumber=" + sy);
								} else {
									alert("已是首页！");
								}
							});
							...
							function() {
								...
								if (xy != 0) {
									...
									var parameter = $("input#parameterrm",
											$("table#searchdept")).val();
									var selectDept = $(":input#selectdept").val();
									getStaffMember("parameter="+parameter+"&selecDept="+selectDept+ "&pageForm.pageNumber=" + xy);
								} else {
									alert("已是尾页！");
								}
							});
...
function getProjectByParent(ppID,parentName){
	...
	.ajax({
		...
		success : function (data) {
			...
			$("#xmul").find("span").each(function(){
						...
						select++;
							if ($(this).find("#ppID").val() == proid) {
								...
								tbody += "<tr id='kelong"+select+"' class='checkgoods cs'>" + $(this).html() + "</tr>";
								$(this).remove();
		    	...
		    	tbody+="<tr id='kelong1'>"+$("#kelong").clone().html()+"</tr><tr style='display:none;'><td id='line'></td></tr>";
			...
			$("tr.checkgoods").each(function() {
			   var selectkelong = $(this).attr("id").substring(6);
			   
			   
			   $(this).find(':input').each(function(){
				   $(this).attr("name","logbookmap["+ selectkelong+ "]."+ this.name);
