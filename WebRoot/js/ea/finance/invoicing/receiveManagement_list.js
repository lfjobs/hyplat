	$(function() {
       ...
       var query = "<form name='SearchForm' id='SearchForm' method='post'>"
		...
		+ "领用出库<span style=\"margin-left:10px;\">单据编号：</span><input type='text' style=\"width: 100px\" name='cashierBills.journalNum'/>&nbsp;&nbsp;"
		+ "责任人：<input name=\"cashierBills.staffName\" style=\"width: 90px\" />&nbsp;&nbsp;"
		...
		+ "单据状态：<select id=\"taxstatus\" style=\"width:65px\"  name=\"cashierBills.status\">"
		...
		function action(com, grid) {
			switch (com) {
				case '添加' :
				    var url=basePath
							+ "/ea/receive/ea_addWareManagement.jspa?pageNumber="
							+ pNumber + "&sdate=" + sdate
			   ...
			   case '修改' :
					...
					var status="";
					var URL = basePath
					+ "ea/receive/sajax_ea_getcashierBillstatus.jspa?cashierBillsVO.cashierBillsID="
					...
					if (status != "22") {
						alert("只有“草稿”状态单据才能修改！");
						document.location.href =basePath
							+ "/ea/receive/ea_getWareManagementList.jspa?billStatus=07";
						return;
					}
					var url1=basePath
							+ "/ea/receive/ea_addWareManagement.jspa?pageNumber="
				...
				case '查看审核' :
					...
					var url= "/ea/receive/ea_toWareManagement.jspa?pageNumber="+ pNumber 
				...
				case '设置每页显示条数' :
					var url = basePath
							+ "/ea/receive/ea_getWareManagementList.jspa?search="
							+ search + "&sdate=" + sdate
				...
				case '出库确认':
					...
					$.ajax({
						type:"post",
						url :"ea/receive/sajax_ea_AuditAcquisition.jspa",
						...
						success:function(data){
							...
							if(list.length!=0){
								if(confirm("确定出库？")){
								 	 document.location.href=basePath+"ea/receive/ea_updatback.jspa?" 
								 	 +"cashierBills.cashierBillsID="+cashierbillsID+"&billStatus="+billStatus; 
								} 
							}else{
								alert("该物品未审核");
							}
		...
		$("#tosearch").click(function() {
			$("#SearchForm")
					.attr(
							"action",
							basePath
									+ "/ea/receive/ea_toSearchWare.jspa?pageNumber="
...
function re_load() {
document.location.href = basePath
		+ "/ea/receive/ea_getWareManagementList.jspa?pageNumber="
