$(function() {
			...
			function action(com, grid) {
				switch (com) {
					case '添加' :
						document.location.href = basePath+ "/ea/stock/ea_addShift.jspa?";
						break;
					case '查看' :
						if (financialbillID == "") {
							alert("请选择！");
						...
						document.location.href = basePath
								+ "/ea/stock/ea_toShift.jspa?pageNumber="
					...
					case '设置每页显示条数' :
						var url = basePath
								+ "/ea/stock/ea_getShiftList.jspa?type=06&search="
			...
			$("#tosearch").click(function() {
				$("#SearchForm").attr(
						"action",
						basePath + "/ea/stock/ea_toShiftSearch.jspa?pageNumber="
								+ pNumber+"&type=06");
...
function re_load() {
	alert(search);
	document.location.href = basePath
			+ "/ea/stock/ea_getShiftList.jspa?pageNumber=" + pNumber
			+ "&pageForm.pageNumber=" + $("#pageNumber").attr("value")+"&type=06"
