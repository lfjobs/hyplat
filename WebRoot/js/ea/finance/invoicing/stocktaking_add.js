$(function() {
	...
	$("input#stocktakingQuantity").change(function(){
			if ($(this).val()!= "") {
				//盘库数量
				var sq=$(this).val();
				//库存数量
				...
				if(parseFloat(sq)<0){
					alert("盘库数量不能为负数");
					$(this).attr("value","");
					$(this).parent().parent().find(":input#stocktakingresults").attr("value","");
					$(this).focus();
					return;
				}
				if (!isNaN(sq) && !isNaN(iq)) {
					...
					if(parseFloat(sq)>parseFloat(iq)){
						$(this).parent().parent().find(":input#stocktakingresults").attr(
								"value", "盈");
					}else if(parseFloat(sq)<parseFloat(iq)){
						$(this).parent().parent().find(":input#stocktakingresults").attr(
								"value", "亏");
					}
				}
		...
		if (financialbillID == "") {
			$("#PurchaseForm")
					...
					.attr(
							"action",
							basePath
									+ "/ea/stock/ea_saveStocktaking.jspa?pageNumber="
									+ pNumber);
...
function re_load() {
var url = basePath+"/ea/stock/ea_getStocktakingList.jspa?pageNumber="+pNumber+"&pageForm.pageNumber="+$("#pageNumber").attr("value")+"&type=05&search="+search+"&sdate="+sdate+"&edate="+edate;
		...
		if ($("[name='check']").is(':checked')) {
			$(":radio[name='check']").each(function() {
				if ($(":radio[name='check']").is(':checked')) {
					...
					$("input#stocktakingQuantity",$("#"+trID)).addClass("isNaN put3");
					$("input#stocktakingQuantity",$("#"+trID)).show();
					$("input#stocktakingresults",$("#"+trID)).show();
					$("span.xx",$("#"+trID)).show();
					//修改当前行的所有input的name
