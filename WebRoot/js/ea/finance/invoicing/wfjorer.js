$(function() {
			...
			$('.flexme11').flexigrid({
						...
						width : 'auto',
						minwidth : 30,
						title : "微分金订单管理<br/><form action='ea/costsheet/ea_getlisorer.jspa' method='post'>凭证号：<input type='text' name='ppID'/>发货时间：<input type='text' name='year'  onfocus='date(this);'/><input type='submit'/></form>",
						minheight : 80,
						buttons : [
						           {
							name : '添加',
			...
			function action(com, grid) {
				switch (com) {
					...
					case '删除' :
						...
						var url = basePath
						+ "/ea/costsheet/ea_datedd.jspa?ppID="+financialbillID;
						if (confirm("确定删除？")) {
							$("#cstaffForm")
									.attr("target", "hidden")
									.attr(
											"action",
											url);
							document.getElementById("cstaffForm").submit();
							financialbillID = "";
							token = 11;
						}
						break;
...
//						break;
					case '设置每页显示条数' :
						var url = basePath
								+ "/ea/costsheet/ea_getlisorer.jspa?search="
