$(function(){
	$(".cherid").each(function(){
		$input=$(this);
		$.ajax({
			url:basePath+"ea/voucher/sajax_ea_examineVoucherToPrint.jspa",
			data:{"voucherId":$(this).val()},
			type:"post",
			async: false,
			success:function(data){
				var member = eval("("+data+")");
				var list=member.baseList;
				var debit=0; 
				var credit=0; 
				var ir=0;
				for(var i=0;i<list.length;i++){
					if(i>4){
						if(i%5==0){
							...
							div.find("table").find("tr").find("td.u1").text((ir+2)+"/"+(list.length%5!=0?list.length/5+1:list.length/5));
							if(ir!=0){
