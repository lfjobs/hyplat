$(function() {
           ...
           var query = "<form name='SearchForm' id='SearchForm' method='post'>"
			+ "调拨出库&nbsp;<input type='submit' style='display:none;' name='submit'/>" 
			+ "<input type='hidden' name='search' value='search'/>" 
			+ "<span style=\"margin-left:10px;\">单据编号：</span><input type='text' style=\"width: 100px\" name='cashierBills.journalNum'/>&nbsp;&nbsp;"
			...
			+ "<option value=\"16\">已出库</option></select>"
			...
			function action(com, grid) {
				switch (com) {
					case '添加' :
					    var url=basePath
								+ "/ea/allocation/ea_addWareManagement.jspa?pageNumber="
								+ pNumber + "&sdate=" + sdate
				   ...
				   case '修改' :
						...
						if (status != "22") {
							alert("只有“草稿”状态单据才能修改！");
							document.location.href =basePath
								+ "/ea/allocation/ea_getWareManagementList.jspa?billStatus=07";
							return;
						}
					...
					case '查看审核' :
						...
						var url= "/ea/allocation/ea_toWareManagement.jspa?pageNumber="+ pNumber 
					...
					case '设置每页显示条数' :
						var url = basePath
								+ "/ea/allocation/ea_getWareManagementList.jspa?search="
								+ search + "&sdate=" + sdate
					...
					case '出库确认':
						...
						//判断出库的物品是不是报损了。
						if(confirm("确定出库？")){
						 	 document.location.href=basePath+"ea/allocation/ea_updatback.jspa?" 
			...
			$("#tosearch").click(function() {
				$("#SearchForm")
						.attr(
								"action",
								basePath
										+ "/ea/allocation/ea_toSearchWare.jspa?pageNumber="
...
function re_load() {
	document.location.href = basePath
			+ "/ea/allocation/ea_getWareManagementList.jspa?pageNumber="
