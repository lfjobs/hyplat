$(function() {
	...
	// 计算误差金额(调查金额-单价=误差金额)
	$(".surveyAmount").live("keyup", function(event) {
		if (this.value != "") {
			if (isNaN(this.value)) {
				return;
			} else {
				var price = $(this).parent().parent().find(":input#price").val();
				var amount = $(this).parent().parent().find(":input#surveyAmount").val();
				if (!isNaN(amount) && !isNaN(price)) {
					var jine = amount - price;
					jine = Math.round(jine * 100) / 100;
					$(this).parent().parent().find(":input#errorAmount").attr(
							"value", jine);
				}
			}
		}
	...
	$(".surveyAmount").live("keydown", function(event) {
		if (this.value != "") {
			if (isNaN(this.value)) {
				return;
			} else {
		...
		if (financialbillID == "") {
			$("#PurchaseForm")
					...
					.attr(
							"action",
							basePath
									+ "/ea/purchase/ea_saveMarketResearch.jspa?pageNumber="
									+ pNumber);
...
function re_load() {
var url = basePath+"/ea/purchase/ea_getMarketResearchList.jspa?pageNumber="+pNumber+"&pageForm.pageNumber="+$("#pageNumber").attr("value")+"&type=04"+"&search="+search+"&sdate="+sdate+"&edate="+edate;
	...
	function cx(typeCN) {
		...
		$.ajax({
			...
			success : function cbf(data) {
				...
				tabletr += "<th align='center' bgcolor='#E4F1FA'>类型</th><th align='center' bgcolor='#E4F1FA'>品牌</th>";
				tabletr += "<th align='center' bgcolor='#E4F1FA'>库存量</th><th align='center' bgcolor='#E4F1FA'>单位</th>";
				tabletr += "<th align='center' bgcolor='#E4F1FA'>默认规格</th><th align='center' bgcolor='#E4F1FA'>型号</th></tr>";
