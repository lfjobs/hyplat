...
$(function() {
   ...
   //--------初始化计算合计--------
	...
	// 选择往来个人、往来单位
	$(".wlgr").click(function() {
		$(".wlgrToos").removeClass("wlgrToos");
		$(this).addClass("wlgrToos");
		bunid=$(this).attr("id");
	...
	//往来单位行单击事件
	$(".trrs").live("click",function(){
		cuID=$(this).attr("id");
		$("#"+cuID).find("td").eq(0).find("input").attr("checked","checked");
	});
	$(".rauser").live("click",function(){
		cuID=$(this).attr("id");
		$("#"+cuID).find("td").eq(0).find("input").attr("checked","checked");
	});
	...
	$("#qduser").click(function() {
		if (cuID != "") {
			$(".wlgrToos").val($("#"+cuID).find("td").eq(1).text());
			$(".wlgrToos").parent().find("input").eq(0).val(cuID);
    ...
    $("input.JQuerySubmitgd").click(function() {
        ...
        $("#InventoryForm").attr("target", "hidden").attr(
            "action",
            basePath + "/ea/newstorage/ea_saveWareManagement.jspa?type="+subtype);
        ...
        $('#icon').parent().css("padding-left", $(document).width() / 2.5);
        $('#icon').parent().css("position","absolute");
        $("#icon").show();
...
function cxwlgr(typeCN,cutype) {
	...
	$.ajax({
		...
		success : function cbf(data) {
			...
			for ( var i = 0; i < pageForm.list.length; i++) {
				tabletr += "<tr style='cursor: hand;' class='trrs' id = "
...
$.ajax({
	...
	success : function cbf(data) {
		...
		for(var i=0;i<StaffList.length;i++){
			var op="<option value='"+StaffList[i].codeID+"'>"+StaffList[i].codeValue+"</option>";
			$("#seleToo").append(op);
		}
...
$("#searchuu").click(function(){
	var typeName = $("input#comUserID",$("table#searchuser")).val();
	var relation=$("#seleToo option:selected").text();
	if(relation=="请选择")
		relation="";
