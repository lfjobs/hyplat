$(document).ready(function () {
    ...
    $('.address').flexigrid({
        ...
        title: "佣金设计<form name='yjForm' id='yjForm' method='post'>" +
        ...
        switch (com) {
            ...
            case '修改佣金':
                ...
                if (typeof(ppid) == "undefined" || ppid == "") {
                    ...
                    url = basePath + "ea/retail/ea_toRetailUpdate.jspa?" +
            ...
            case '删除':
                var ppid = $('input[name="checkedId"]:checked').val();
                var suid = $('input[name="checkedId"]:checked').siblings(".suid").val();
                if (typeof(suid) == "undefined" || suid == "") {
                    alert('请选择！');
                    return
                } else {
                    if (confirm("确定删除吗?")) {
                        $.ajax({
                            type: "POST",
                            url: basePath + "ea/retail/sajax_ea_retailDeleteById.jspa",
                            data: {
                                "proSetup.suid": suid,
                                "proSetup.ppid": ppid
                            },
                ...
                break;
            case 'excel导出':
                window.open(basePath + "ea/retail/ea_RetailShowExcel.jspa?search="+search+"&showweixin="+showweixin);
                break;
        }
        ;
    }
    ...
    $("#search").click(function () {
        ...
        $("#yjForm").attr(
            "action",
            basePath + "ea/retail/ea_selectRetailList.jspa?pageForm.pageNumber="
            + pNumber + "");
