$(function() {
	/*************************招生员信息操作部分开始*****************************/
	//----------------选择在职人员操作------------//
	$("#chooseRecruiter").live("click",function(){
		$("#daoRuStafff").attr("src",basePath+'/ea/academicadmin/ea_getCompanyListEmployeeReferral.jspa');
		$("div#bankJqmStafff").jqmShow();
	});
	$("#DaoRuFanStafff").click(function(){// 返回
       $("#bankJqmStafff").jqmHide();
	}); 
	$("#DaoRuFanqdStafff").click(function(){// 选择确定
		var childopertionID = window.frames["daoRuStafff"].opertionID;
		if(childopertionID == ""){
			alert("请选择");
			return;
		}
		var staffName = $(window.frames["daoRuStafff"].$('tr#'+childopertionID)[0]).find("span#staffName").text();
		var staffIdentityCard =$(window.frames["daoRuStafff"].$('tr#'+childopertionID)[0]).find("span#staffIdentityCard").text();
		var reference =$(window.frames["daoRuStafff"].$('tr#'+childopertionID)[0]).find("span#reference").text();//mainframe4
		var staffAddress =$(window.frames["daoRuStafff"].$('tr#'+childopertionID)[0]).find("span#staffAddress").text();
		var referenceOrganization =$(window.frames["daoRuStafff"].$('tr#'+childopertionID)[0]).find("span#referenceOrganization").text();
		
		$t=$("table#recruiterTable");
		$t.find("input#referrerName").attr("value",staffName);
		$t.find("span#referrerName").text(staffName);
		$t.find("input#referrerID").attr("value",childopertionID);
		
		$t.find("input#referrerPhone").attr("value",reference);
		$t.find("span#referrerPhone").text(reference);
		
		$t.find("input#referrerIdentityCard").attr("value",staffIdentityCard);
		$t.find("span#referrerIdentityCard").text(staffIdentityCard);
		
		$t.find("input#referrerAddress").attr("value",staffAddress);
		$t.find("span#referrerAddress").text(staffAddress);
		
		$t.find("input#referrerEmail").attr("value",referenceOrganization);
		$t.find("span#referrerEmail").text(referenceOrganization);
		
		$("#daoRuStafff").attr("src","");
        $("#bankJqmStafff").jqmHide();
        bmDept(companyID,gongsiname);
        $t.find("span#assignsName").hide();
   });
	...
	if (cashierBillsID == "") {
		...
		if ($("#cashierstatus").val() != ''
				&& $("#cashierstatus").val() == '00') { // 当单据状态不为空或是草稿状态
			$("input.JQuerySubmit").show(); // 添加页面草稿保存显示
		}
		if ($("#cashierstatus").val() == '04'
				|| $("#cashierstatus").val() == '00') { // 当单据状态为草稿或是待主管审核
	...
	$("input.JQueryInvalid").click(function() {
		if (confirm("是否继续操作？")) {
			var jurl = basePath
					+ "ea/cashierbills/sajax_ea_updateCashierBillsInvalid.jspa?date="
	...
	$(".jisuan").live("keydown", function(event) {
		if (this.value != "") {
			if (isNaN(this.value)) {
				...
				var dj = $(this).parent().parent().find("span:#quantity")
						.html();
				var price = $(this).parent().parent().find(":input#price")
						.val();
				if (!isNaN(dj) && !isNaN(price)) {
		...
		if (cashierBillsID == "") {
			$("#cashierTallyForm")
					...
					.attr(
							"action",
							basePath
									+ "ea/cashierbills/ea_saveCashierBills.jspa?pageNumber="
									+ pNumber);
			...
			$("table#recruiterTable").find("select#assignsID").css("display","none");/*****隐藏招生点下拉选项*******/
			token = 1;
			return;
		}
		$("#cashierTallyForm")
	...
	$("input.JQuerySubmitgd").click(function() {
		...
		$("input#companyTel").attr("value",$("span#companyTel").text());
		$("input#cresponsible").attr("value",$("span#cresponsible").text());
		$("input#responsibleTel").attr("value",$("span#responsibleTel").text());
		$("input#companyAddr").attr("value",$("span#companyAddr").text());
		$("input#industryType").attr("value",$("span#industryType").text());
		$(".put3", $("table#table3")).trigger("blur");
		$(".put3", $("tr.checkgoods")).trigger("blur");
		$(".notnull", $("tr.checkgoods")).trigger("blur");
		$(".isNaN", $("tr.checkgoods")).trigger("blur");
		...
		notoken = 1;
		//$("input#staffName").attr("value",$("#staffName").text());
		$("#cashierstatus").attr("value", "04");
		if(Showtype == "carbill"){
			
			var childopertionID =$("#chooseContext").val();
			$("#cashierTallyForm")
				.attr(
						"action",
						basePath
								+ "ea/cashierbills/ea_toSaveCarCashierBills.jspa?pageNumber="
								+ pNumber+"&staffIDvalues=" + staffIDvalues+"&staffZerenren=" + childopertionID);
		document.cashierTallyForm.submit.click();
		token = 1;
		return;
		}
...
$(document).ready(function() {
	...
	$("#chanpin").click(function() {
				$("input#parameter3", $("table#searchproduct")).val("");
	...
	function cp(typeCN,search) {
		...
		$.ajax({
			...
			success : function cbf(data) {
				...
				var tabletr = "<table width='98%' height='26' align='center' cellspacing='0' cellpadding='1' style='font-size:12px;' class='bannb_01'><tr><td height='24' align='left' valign='top' class='txt01' >&nbsp;点击选择产品</td></tr></table>";
				...
				tabletr += " <th align='center' bgcolor='#E4F1FA'>价格类型</th>";
				tabletr += " <th align='center' bgcolor='#E4F1FA'>产品价格</th>";
				...
				for (var i = 0; i < pageForm.list.length; i++) {
					tabletr += "<tr style='cursor: hand;' title="
							...
							+ pageForm.list[i][0] + ">";
					tabletr += "<td id='checkboxp' align='center'>" +
					...
					tabletr += "<td id='category' align='center'>"
					...
					tabletr += "<td id='mnemonicCode' align='center' style='display:none'>"
						+ (pageForm.list[i][18]==null?"":pageForm.list[i][18]) + "</td>";
					tabletr += "<td id='typeID' align='center' style='display:none'>"
						+ (pageForm.list[i][19]==null?"":pageForm.list[i][19]) + "</td>";
					tabletr += "<td id='model' align='center' style='display:none'>"
						+ (pageForm.list[i][20]==null?"":pageForm.list[i][20]) + "</td>";
					tabletr += "<td id='standard' align='center' style='display:none'>"
						+ pageForm.list[i][21] + "</td>";
					tabletr += "<td id='variableID' title='ava' align='center' style='display:none'>"
						+ (pageForm.list[i][22]==null?"":pageForm.list[i][22]) + "</td>";
					tabletr += "<td id='variable1ID' title='ava' align='center' style='display:none'>"
						+ (pageForm.list[i][23]==null?"":pageForm.list[i][23]) + "</td>";
					tabletr += "<td id='variable2ID' title='ava' align='center' style='display:none'>"
						+ (pageForm.list[i][24]==null?"":pageForm.list[i][24]) + "</td>";
					tabletr += "<td id='variable3ID' title='ava' align='center' style='display:none'>"
						+ (pageForm.list[i][25]==null?"":pageForm.list[i][25]) + "</td>";
					tabletr += "<td id='variable4ID' title='ava' align='center' style='display:none'>"
						+ (pageForm.list[i][26]==null?"":pageForm.list[i][26]) + "</td>";
...
$(document).ready(function() {
	...
	$(".xzgr").click(function() {
				window.open(basePath
						+ "ea/cstaff/ea_getListCStaffByCompanyID.jspa?cstaff.status=99");
			});
		...
		if (cuID != "") {
			...
			$.ajax({
				...
				success : function cbf(data) {
					...
					$se0.attr("name", "cashierBills.staffAddress");
					$se0.show();
				},
				error : function cbf(data) {
					notoken = 0;
