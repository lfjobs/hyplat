$(function() {
	...
	function action(com, grid) {
		...
		case '删除':
			...
			if (confirm("确认要删除吗")) {
				...
				$("#postSearchForm").attr("target", "hidden").attr("action",
						basePath + "ea/travelapply/ea_deleteByTravel.jspa?");
		...
		case '查看':
			...
			url = basePath
					+ "ea/travelapply/ea_getDetailsByTravel.jspa?dtMytravel.id="
					+ id;
			open(url);
			break;
		case '查询':
			...
			break;
		case '导出':
			url = basePath + "ea/travelapply/ea_exportByTravel.jspa?search="
					+ search+"&type="+type;
			open(url);
			break;
		case '打印预览':
			...
			url = basePath
					+ "ea/travelapply/ea_toPrintPreviewByTravel.jspa?dtMytravel.id="
					+ id;
			open(url);
			break;
		case '设置每页显示条数':
			var url = basePath + "ea/travelapply/ea_getListByTravel.jspa?search="
					+ search+"&type="+type;
			numback(url);
			break;
		}
	 ...
	 * 保存草稿
	 */
	$("input.JQuerySave", $("form#travelForm")).click(
			function() {// 提交审核或保存草稿
				...
				$("form#travelForm").attr("target", "hidden").attr("action",
						basePath + "ea/travelapply/ea_saveOrEditByTravel.jspa?");
				document.travelForm.submit.click();
				document.travelForm.reset();
				token = 2;
			});
	/**
	 * 提交审核
	 */
	$("input.JQuerySubmit", $("form#travelForm")).click(function() {// 提交审核或保存草稿
		$(".put3").trigger("blur");
		if ($("form .error").length) {
			return;
		}
	...
	$("#tosearch").click(
			function() {
				$("#postSearchForm").attr("action",
						basePath + "ea/travelapply/ea_toSearchByTravel.jspa?");
				document.postSearchForm.submit.click();
			});
			...
			.click(
					function() {
						...
						if (confirm("确认要发送至领导审批？")) {
							...
							$("input#buttonType", $("form#travelForm")).attr(
									"value", "1");
							...
							$("form#travelForm")
									...
									.attr(
											"action",
											basePath
													+ "ea/travelapply/ea_saveOrEditByTravel.jspa?");
							document.travelForm.submit.click();
							document.travelForm.reset();
							token = 2;
						}
	...
	$("#startDate", $("#travelForm")).blur(
			function() { // 起时间失去焦点事件
				$("td.errortime").find(".corect").remove();
				var start = $("#startDate", $("#travelForm")).val()
						.replace(/-/g, '');
				var end = $("#endDate", $("#travelForm")).val().replace(
						/-/g, '');
				if (end != '' && start > end) {
					alert('起时间必须小于止时间！');
					$(".errortime").find(".input").val("");
					$("#startDate", $("#travelForm")).focus();
				}
			});

	$("#endDate", $("#travelForm"))
			.focus(function() { // 止时间获取焦点事件
				if ($("#startDate", $("#travelForm")).val() == '') {
					alert("请先填写起时间！");
					$("#startDate", $("#travelForm")).focus();
				}
			})
			.blur(
					function() { // 止时间失去焦点事件
						...
						var start = $("#startDate", $("#travelForm")).val();
						var end = $("#endDate", $("#travelForm")).val();
						if (end != '') {
							if (start.replace(/-/g, '') > end.replace(/-/g, '')) {
								if (times == '0') {
									alert('起时间必须小于止时间！');
									$(".errortime").find(".input").val("");
									$("#startDate", $("#travelForm"))
											.focus();
									times++;
								}
								if ($("#startDate", $("#travelForm"))
										.focus()) {
									times = '0';
								}
							} else {
								...
								$("#travelDays", $("#travelForm")).val("");
								$("#travelHours", $("#travelForm")).val("");
...
function re_load() {
	...
	if (token)
		document.location.href = basePath
				+ "ea/travelapply/ea_getListByTravel.jspa?pageNumber="
				+ pageNumber + "&pageForm.pageNumber="
				+ $("#pageNumber").attr("value")+"&type="+type;
}

function getSerialNumber() {
	var url = basePath + "ea/travelapply/sajax_ea_getSerialNumber.jspa?date="
	...
	$.ajax({
		...
		success : function cbf(data) {
			...
			vouch = member.BillID;
			$("input#serialnumber", $("form#travelForm")).val(vouch);
		},
