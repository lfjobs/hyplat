$(function() {
	...
	function action(com, grid) {
		...
		case '修改':
			...
			$p.find("span[id]").each(
					function() {
						...
						if (this.id == "leaveType") {
							$(":radio[value=" + $(this).text() + "]").attr(
									"checked", true);
						}
					});
		...
		case '删除':
			...
			if (confirm("确认要删除吗")) {
				...
				$("#postSearchForm").attr("target", "hidden").attr("action",
						basePath + "ea/leaveapply/ea_deleteByLeave.jspa?");
		...
		case '查看':
			...
			url = basePath
					+ "ea/leaveapply/ea_getDetailsByLeave.jspa?dtMyleave.id="
					+ id;
			open(url);
			break;
		case '查询':
			...
			break;
		case '导出':
			url = basePath + "ea/leaveapply/ea_exportByLeave.jspa?search="
					+ search+"&type="+type;
			open(url);
			break;
		case '打印预览':
			...
			url = basePath
					+ "ea/leaveapply/ea_toPrintPreviewByLeave.jspa?dtMyleave.id="
					+ id;
			open(url);
			break;
		case '设置每页显示条数':
			var url = basePath + "ea/leaveapply/ea_getListByLeave.jspa?search="
					+ search+"&type="+type;
			numback(url);
			break;
		}
	 ...
	 * 保存草稿
	 */
	$("input.JQuerySave", $("form#leaveForm")).click(
			function() {// 提交审核或保存草稿
				...
				$("form#leaveForm").attr("target", "hidden").attr("action",
						basePath + "ea/leaveapply/ea_saveOrEditByLeave.jspa?");
	 ...
	 * 提交审核
	 */
	$("input.JQuerySubmit", $("form#leaveForm")).click(function() {// 提交审核或保存草稿
		$(".put3").trigger("blur");
		if ($("form .error").length) {
			return;
		}
	...
	$("#tosearch").click(
			function() {
				$("#postSearchForm").attr("action",
						basePath + "ea/leaveapply/ea_toSearchByLeave.jspa?");
				document.postSearchForm.submit.click();
			});
			...
			.click(
					function() {
						...
						if (confirm("确认要发送至领导审批？")) {
							...
							$("input#buttonType", $("form#leaveForm")).attr(
									"value", "1");
							...
							$("form#leaveForm")
									...
									.attr(
											"action",
											basePath
													+ "ea/leaveapply/ea_saveOrEditByLeave.jspa?");
							document.leaveForm.submit.click();
							document.leaveForm.reset();
							token = 2;
						}
						...
						if (end != '') {
							if (start.replace(/-/g, '') > end.replace(/-/g, '')) {
								if (times == '0') {
									alert('起时间必须小于止时间！');
									$(".errortime").find(".input").val("");
									$(".leaveStartDate", $("#leaveForm"))
											.focus();
									times++;
								}
								if ($(".leaveStartDate", $("#leaveForm"))
										.focus()) {
									times = '0';
								}
							} else {
...
function re_load() {
	...
	if (token)
		document.location.href = basePath
				+ "ea/leaveapply/ea_getListByLeave.jspa?pageNumber="
				+ pageNumber + "&pageForm.pageNumber="
				+ $("#pageNumber").attr("value")+"&type="+type;
}

function getSerialNumber() {
	var url = basePath + "ea/leaveapply/sajax_ea_getSerialNumber.jspa?date="
	...
	$.ajax({
		...
		success : function cbf(data) {
			...
			vouch = member.BillID;
			$("input#serialnumber", $("form#leaveForm")).val(vouch);
		},
...
function importGY() { // 打开页面
	var url = "ea/cosincumbent/ea_getStaffForCashier.jspa?checkOrgID=" + treeID;
	$("#ifr").attr("src", basePath + url + "&date=" + new Date());
	$("#jqmWindow2").jqmShow();
}

function DaoruConfirm() {// 选择确定
	var childopertionID = window.frames["ifr"].opertionID;
	if (childopertionID == "") {
		alert("请选择");
		return;
	}
	var staffName = window.frames["ifr"].$('tr#' + childopertionID).find(
			"span#staffName").text();

	$(".leaveReceiverid").val(childopertionID);
	$(".leaveReceiver").val(staffName);
	$("#ifr").attr("src", "");
	$("#jqmWindow2").jqmHide();
}
function cancelJqm2() {
