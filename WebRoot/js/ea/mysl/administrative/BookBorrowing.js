$(function() {
		...
		switch (com) {
			...
			case '删除' :
				...
				if(confirm("确认要删除吗")){
					...
					$("<input>",{type:"hidden",value:value,name:"dtMybookborrowing.key"}).appendTo($("form#postSearchForm"));
					$("#postSearchForm").attr("target", "hidden").attr(
							"action",
							basePath + "ea/bookborrowing/ea_deleteByBookBorrowing.jspa?");
			...
			case '查看' :
				...
				url = basePath + "ea/bookborrowing/ea_getDetailsByBookBorrowing.jspa?dtMybookborrowing.id="
				+ id;
				open(url);
				break;	
			case '查询' :
			...
			case '导出' :
				url = basePath
						+ "ea/bookborrowing/ea_exportByBookBorrowing.jspa?search="
						+ search;
				open(url);
				break;
			case '打印预览' :
				...
				url = basePath + "ea/bookborrowing/ea_toPrintPreviewByBookBorrowing.jspa?dtMybookborrowing.id="
						+ id;
				open(url);
				break;
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/bookborrowing/ea_getListByBookBorrowing.jspa?search="
						+ search;
				numback(url);
				break;
		}
	 ...
	 * 保存草稿
	 */
	$("input.JQuerySave",$("form#BookBorrowing")).click(function() {// 提交审核或保存草稿
				$(".put3").trigger("blur");
				...
				$("form#BookBorrowing")
						...
						.attr(
								"action",
								basePath
										+ "ea/bookborrowing/ea_saveOrEditByBookBorrowing.jspa?");
				document.BookBorrowing.submit.click();
				document.BookBorrowing.reset();
				token = 2;
			});
	/**
	 * 提交审核
	 */
	$("input.JQuerySubmit",$("form#BookBorrowing")).click(function() {// 提交审核或保存草稿
		$(".put3").trigger("blur");
		if ($("form .error").length) {
			return;
		}
	...
	$("#tosearch").click(function() {
		$("#postSearchForm").attr(
				"action",
				basePath + "ea/bookborrowing/ea_toSearchByBookBorrowing.jspa?");
		document.postSearchForm.submit.click();
	});
	...
	$("#submitResult").click(function() {
			...
			if (confirm("确认要发送至领导审批？")) {
				...
				$("<input>",{type:"hidden",value:auditorcompanyid,name:"dtMycheck.auditorcompanyid"}).appendTo($("form#BookBorrowing"));
				$("<input>",{type:"hidden",value:auditorcompanyname,name:"dtMycheck.auditorcompanyname"}).appendTo($("form#BookBorrowing"));	
				$("<input>",{type:"hidden",value:auditororgID,name:"dtMycheck.auditororgid"}).appendTo($("form#BookBorrowing"));
				$("<input>",{type:"hidden",value:auditororgName,name:"dtMycheck.auditororgname"}).appendTo($("form#BookBorrowing"));
				$("<input>",{type:"hidden",value:auditorid,name:"dtMycheck.auditorid"}).appendTo($("form#BookBorrowing"));
				$("<input>",{type:"hidden",value:auditorname,name:"dtMycheck.auditorname"}).appendTo($("form#BookBorrowing"));
				
	
				$("input#buttonType",$("form#BookBorrowing")).attr("value","1");
...
function re_load() {
	...
	if (token)
		document.location.href = basePath
				+ "ea/bookborrowing/ea_getListByBookBorrowing.jspa?pageNumber="
...
function getSerialNumber(){  
					...
					$.ajax({
							...
							vouch = member.BillID;
							$("input#serialnumber",$("form#BookBorrowing")).val(vouch);
						},
