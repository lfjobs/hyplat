$(function() {
		...
		switch (com) {
			...
			case '删除' :
				...
				if(confirm("确认要删除吗")){
					...
					$("<input>",{type:"hidden",value:value,name:"dtMyconsumerecipt.key"}).appendTo($("form#postSearchForm"));
					$("#postSearchForm").attr("target", "hidden").attr(
							"action",
							basePath + "ea/consumrecip/ea_deleteByConsumerecipt.jspa?");
			...
			case '查看' :
				...
				url = basePath + "ea/consumrecip/ea_getDetailsByConsumerecipt.jspa?dtMyconsumerecipt.id="
				+ id;
				open(url);
				break;	
			case '查询' :
			...
			case '导出' :
				url = basePath
						+ "ea/consumrecip/ea_exportByConsumerecipt.jspa?search="
						+ search+"&type="+type;
				open(url);
				break;
			case '打印预览' :
				...
				url = basePath + "ea/consumrecip/ea_toPrintPreviewByConsumerecipt.jspa?dtMyconsumerecipt.id="
						+ id;
				open(url);
				break;
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/consumrecip/ea_getListByConsumerecipt.jspa?search="
						+ search+"&type="+type;
				numback(url);
				break;
		}
	 ...
	 * 保存草稿
	 */
	$("input.JQuerySave",$("form#consumerForm")).click(function() {// 提交审核或保存草稿
				$(".put3").trigger("blur");
				...
				$("form#consumerForm")
						...
						.attr(
								"action",
								basePath
										+ "ea/consumrecip/ea_saveOrEditByConsumerecipt.jspa?");
				document.consumerForm.submit.click();
				document.consumerForm.reset();
				token = 2;
			});
	/**
	 * 提交审核
	 */
	$("input.JQuerySubmit",$("form#consumerForm")).click(function() {// 提交审核或保存草稿
		$(".put3").trigger("blur");
		if ($("form .error").length) {
			return;
		}
	...
	$("#tosearch").click(function() {
		$("#postSearchForm").attr(
				"action",
				basePath + "ea/consumrecip/ea_toSearchByConsumerecipt.jspa?");
		document.postSearchForm.submit.click();
	});
	...
	$("#submitResult").click(function() {
			...
			if (confirm("确认要发送至领导审批？")) {
				...
				$("<input>",{type:"hidden",value:auditorcompanyid,name:"dtMycheck.auditorcompanyid"}).appendTo($("form#consumerForm"));
				$("<input>",{type:"hidden",value:auditorcompanyname,name:"dtMycheck.auditorcompanyname"}).appendTo($("form#consumerForm"));	
				$("<input>",{type:"hidden",value:auditororgID,name:"dtMycheck.auditororgid"}).appendTo($("form#consumerForm"));
				$("<input>",{type:"hidden",value:auditororgName,name:"dtMycheck.auditororgname"}).appendTo($("form#consumerForm"));
				$("<input>",{type:"hidden",value:auditorid,name:"dtMycheck.auditorid"}).appendTo($("form#consumerForm"));
				$("<input>",{type:"hidden",value:auditorname,name:"dtMycheck.auditorname"}).appendTo($("form#consumerForm"));
				
	
				$("input#buttonType",$("form#consumerForm")).attr("value","1");
...
function re_load() {
	...
	if (token)
		document.location.href = basePath
				+ "ea/consumrecip/ea_getListByConsumerecipt.jspa?pageNumber="
				+ pageNumber + "&pageForm.pageNumber="
				+ $("#pageNumber").attr("value")+"&type="+type;
}

function getSerialNumber(){  
					var url=basePath+"ea/consumrecip/sajax_ea_getSerialNumber.jspa?date="+new Date().toLocaleString();
					$.ajax({
				            url: url,
				            type: "get",
				            async: true,
				            dataType: "json",
				            data:{"serialnumber":"001"},
							...
							vouch = member.BillID;
							$("input#serialnumber",$("form#consumerForm")).val(vouch);
						},
