$(function() {
	...
	function action(com, grid) {
		switch (com) {
			case '添加' :
			    ...
			    document.deviceborrowForm.reset();
			    getSerialNumber();
				$("#jqModel").jqmShow();
				break;
			case '提交审核' :
				...
				var statu = $("tr#"+debid).find("span#status").text();
				if (statu != "草稿") {
					alert('草稿状态单据才能提交审核!');
					return;
				}
			...
			case '删除' :
				...
				$f = $('#deviceborrowForm');
				$f.find(':input#id').val(debid);
				if (confirm("确定继续？")) {
					$f.attr("target", "hidden")
							.attr(
									"action",
									basePath
											+ "ea/deborrow/ea_deldeborrowApply.jspa?pageNumber="
											+ pageNumber + "&search=" + search);
					document.deviceborrowForm.submit.click();
                    token = 2;
					$("tr#" + debid).remove();
					debid = "";
				}
			...
			case '修改' :
				...
				var dname = $("tr#"+debid).find("span#dvusername").text();
				var serialNumber = $("tr#"+debid).find("span#serialNumber").text();
				$t.find("span#dvusername").text(dname);
		   ...
		   case '查看' :
			    ...
			    window.open(basePath + "ea/deborrow/ea_toDetailDeviceBorrow.jspa?deborrow.id="+debid);
				break;
			case '查询' :
				...
				break;
			case '导出' :
			    var url = basePath+ "ea/deborrow/ea_showExcel.jspa?type="+type+"&search="+search;
			    open(url);
				break;
			case '打印预览' :
			    if (debid == "") {
			    ...
			    window.open(basePath + "ea/deborrow/ea_toPrintDeviceBorrow.jspa?deborrow.id="+debid);
				break;
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/deborrow/ea_getDtMydeviceborrowList.jspa?type="+type+"&1=1";
				numback(url);
				break;
		}
	}
	$(".close").click(function() {// 取消查询搜索
		        ...
		        $("table#cataffSearchTable").find("input#dvusername").val("");
	...
	$("#tosearch").click(function() {
		$("#postSearchForm").attr(
				"action",
				basePath + "ea/deborrow/ea_toSearch.jspa?type="+type+"&1=1");
    ...
    $("#submitResult").click(function() {
			...
			if (confirm("确认要提交审核？")) {
                ...
                if(submittype=="savecheck"){
                  $("<input>",{type:"hidden",value:auditorcompanyid,name:"dtMycheck.auditorcompanyid"}).appendTo($("form#deviceborrowForm"));
				  $("<input>",{type:"hidden",value:auditorcompanyname,name:"dtMycheck.auditorcompanyname"}).appendTo($("form#deviceborrowForm"));	
				  $("<input>",{type:"hidden",value:auditororgid,name:"dtMycheck.auditororgid"}).appendTo($("form#deviceborrowForm"));
				  $("<input>",{type:"hidden",value:auditororgname,name:"dtMycheck.auditororgname"}).appendTo($("form#deviceborrowForm"));
				  $("<input>",{type:"hidden",value:auditorid,name:"dtMycheck.auditorid"}).appendTo($("form#deviceborrowForm"));
				  $("<input>",{type:"hidden",value:auditorname,name:"dtMycheck.auditorname"}).appendTo($("form#deviceborrowForm"));
				   $("#deviceborrowForm")
					.attr("target", "hidden")
					.attr(
							"action",
							basePath
									+ "ea/deborrow/ea_saveDeviceBorrow.jspa?submittype="+submittype+"&1=1"
									);
			       document.deviceborrowForm.submit.click();
			       document.deviceborrowForm.reset();
			       token = 2;
				  ...
				  $f.find(':input#id').val(debid);
				  var url = basePath
						+ "ea/deborrow/ea_tocheck.jspa?pageNumber="
						+ pageNumber;
...
function getSerialNumber(){
					...
					$.ajax({
							...
							vouch = member.BillID;
							$("input#serialNumber",$("form#deviceborrowForm")).val(vouch);
							$("span#serialNumber",$("form#deviceborrowForm")).text(vouch);
						},
