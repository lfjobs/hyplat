$(function() {
	...
	function action(com, grid) {
		switch (com) {
			case '添加' :
				...
				document.SanitaryInspection.reset();
				getSerialNumber();
				$("#jqModel").jqmShow();
				break;
			case '修改' :
			...
			case '删除' :
				...
				if(confirm("确认要删除吗")){
					...
					$("<input>",{type:"hidden",value:value,name:"dtMysanitaryinspection.key"}).appendTo($("form#postSearchForm"));
					$("#postSearchForm").attr("target", "hidden").attr(
							"action",
							basePath + "ea/sanitaryinspection/ea_deleteBySanitaryInspection.jspa?");
			...
			case '查看' :
				...
				url = basePath + "ea/sanitaryinspection/ea_getDetailsBySanitaryInspection.jspa?dtMysanitaryinspection.id="
				+ id;
				open(url);
				break;	
			case '查询' :
			...
			case '导出' :
				url = basePath
						+ "ea/sanitaryinspection/ea_exportBySanitaryInspection.jspa?search="
						+ search;
				open(url);
				break;
			case '打印预览' :
				...
				url = basePath + "ea/sanitaryinspection/ea_toPrintPreviewBySanitaryInspection.jspa?dtMysanitaryinspection.id="
						+ id;
				open(url);
				break;
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/sanitaryinspection/ea_getListBySanitaryInspection.jspa?search="
						+ search;
				numback(url);
				break;
		}
	 ...
	 * 保存草稿
	 */
	$("input.JQuerySave",$("form#SanitaryInspection")).click(function() {// 提交审核或保存草稿
				$(".put3").trigger("blur");
				...
				notoken=1;
				$("form#SanitaryInspection")
						.attr("target", "hidden")
						.attr(
								"action",
								basePath
										+ "ea/sanitaryinspection/ea_saveOrEditBySanitaryInspection.jspa?");
				document.SanitaryInspection.submit.click();
				document.SanitaryInspection.reset();
				token = 2;
			});
	/**
	 * 提交审核
	 */
	$("input.JQuerySubmit",$("form#SanitaryInspection")).click(function() {// 提交审核或保存草稿
		$(".put3").trigger("blur");
		if ($("form .error").length) {
			return;
		}
	...
	$("#tosearch").click(function() {
		$("#postSearchForm").attr(
				"action",
				basePath + "ea/sanitaryinspection/ea_toSearchBySanitaryInspection.jspa?");
		document.postSearchForm.submit.click();
	});
	...
	$("#submitResult").click(function() {
			...
			if (confirm("确认要发送至领导审批？")) {
				...
				$("<input>",{type:"hidden",value:auditorcompanyid,name:"dtMycheck.auditorcompanyid"}).appendTo($("form#SanitaryInspection"));
				$("<input>",{type:"hidden",value:auditorcompanyname,name:"dtMycheck.auditorcompanyname"}).appendTo($("form#SanitaryInspection"));	
				$("<input>",{type:"hidden",value:auditororgID,name:"dtMycheck.auditororgid"}).appendTo($("form#SanitaryInspection"));
				$("<input>",{type:"hidden",value:auditororgName,name:"dtMycheck.auditororgname"}).appendTo($("form#SanitaryInspection"));
				$("<input>",{type:"hidden",value:auditorid,name:"dtMycheck.auditorid"}).appendTo($("form#SanitaryInspection"));
				$("<input>",{type:"hidden",value:auditorname,name:"dtMycheck.auditorname"}).appendTo($("form#SanitaryInspection"));
				
	
				$("input#buttonType",$("form#SanitaryInspection")).attr("value","1");
...
function re_load() {
	...
	if (token)
		document.location.href = basePath
				+ "ea/sanitaryinspection/ea_getListBySanitaryInspection.jspa?pageNumber="
...
function getSerialNumber(){  
					...
					$.ajax({
							...
							vouch = member.BillID;
							$("input#serialnumber",$("form#SanitaryInspection")).val(vouch);
						},
