$(function() {
	...
	function action(com, grid) {
		...
		case '查询' :
			...
			break;
		case '导出' :
			var url = basePath + "ea/taskmanage/ea_toExportExcelByTask.jspa?search=" + search+"&myproject.proid="+proid+"&mytask.phasestatus=02";
			open(url);
			break;	
		case '设置每页显示条数' :
			var url = basePath
					+ "ea/taskmanage/ea_getListByTaskManageFinished.jspa?search=" + search+"&myproject.proid="+proid;
			numback(url);
			break;
	}
}
...
$("#search").click(function() {
	$("#searchForm").attr("action",
			basePath + "ea/taskmanage/ea_toSearchByTaskManageFinished.jspa");
	document.searchForm.submit.click();
...
$("#submitResult").click(function() {
	...
	if (confirm("确认要发送至领导审批？")) {
		...
		$("<input>",{type:"hidden",value:"01",name:"dtMyauditMap[" + index + "].taskstatus"}).appendTo($("form#SendForm"));
		$("#SendForm")
					.attr("target", "hidden")
					.attr(
							"action",
...
function re_load() {
	...
	if (token)
	document.location.href = basePath
			+ "ea/taskmanage/ea_getListByTaskManageFinished.jspa?pageNumber="
