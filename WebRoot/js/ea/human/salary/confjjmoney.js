$(document).ready(function() {
	...
	var query = "<form name='cofjjMoneyForm' id='cofjjMoneyForm' method='post'>" +
		...
		"<input class=\"input-button\"type='button' style=\"margin:0px;margin-left:15px;margin-top:11px;\" value='查询' id='searchCofjjM'/>" +
...
function action(com, grid) {
	switch (com) {
		case '设置每页显示条数' :
			var url = basePath
					+ "ea/cofjjm/ea_findItem.jspa?1=1";
			numback(url);
			break;
	}
}
	...
	$("#searchCofjjM").click(function(){
		$('#cofjjMoneyForm').attr("action",basePath + "ea/cofjjm/ea_toSearch.jspa");
		document.cofjjMoneyForm.submit.click();
	});
});
