$(document).ready(function() {
	...
	window.parent.document.getElementById("mainframe3").style.height = 256 + "px";
		...
		switch (com) {
			case '添加' :
				...
				$("#sa" + select).find(':input:gt(0)').each(function() {
					$(this).attr(
							"name",
							"institutionsRegistrationmap[" + select + "]."
									+ this.name);
				});
			...
			case '修改' :
				...
				$p.find('select').each(function() {
					$(this).find("option[text="+$(this).parent().find("span#"+this.id).text()+"]").attr("selected",true); 
					$(this).attr(
							"name",
							"institutionsRegistrationmap[" + select + "]."
									+ this.id);
			...
			case '全部保存' :
				...
				$('#staffappraisalForm')
						...
						.attr(
								"action",
								basePath
										+ "ea/institutionsregistration/ea_addInstitutionsRegistration.jspa?ogName="+ogName+"&pageNumber="
										+ ppageNumber);
				...
				if (confirm("是否删除？")) {
					$f
							...
							.attr(
									"action",
									basePath
											+ "ea/institutionsregistration/ea_delInstitutionsRegistration.jspa?ogName="+ogName+"&pageNumber="
											+ ppageNumber
											+ "&institutionsRegistration.registrationID="
											+ registrationID);
					document.staffappraisalForm.submit.click();
			...
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/institutionsregistration/ea_getListInstitutionsRegistration.jspa?ogName="+ogName+"&institutionsRegistration.organizationPID="
						+ ogID + "&search=" + search;
				numback(url);
				break;
			case '导出' :
				...
				var url = basePath
						+ "ea/institutionsregistration/ea_showExcel.jspa?search="
						+ search + "&institutionsRegistration.organizationPID="
						+ organizationPID;
				open(url);
				break;
			case '查询' :
	...
	//验证事件
	$(".nummin").blur(function() {
		if ($(this).val().length > 50) {
			alert("您输入的字数超出范围 ");
		  $(this).val("");
		}
	});
	$(".nummax").blur(function() {
	...
	$("#tosearch").click(function() {
		...
		$("#appraisalForm")
				.attr(
						"action",
						basePath
								+ "ea/institutionsregistration/ea_toSearch.jspa?ogName="+ogName+"&pageNumber="
								+ ppageNumber);
	...
	$('td.JQueryaddresssearch select[number]').change(function() {
		...
		var url = basePath
				...
				+ searchdistrictPID + "&date2=" + new Date();
		$.ajax({
					url : encodeURI(url),
					type : "get",
					async : true,
...
function aonclick(){
	var heis = parent.document.getElementById("mainframe3").offsetHeight + 200 + "px";
	parent.document.getElementById("mainframe3").style.height = heis;
	accift('02');
}
function paret(e){
	var arr = e.split(",");
	$("tr#"+registrationID).find("#conPerson").attr("value",arr[1]);
	$("tr#"+registrationID).find("#papersType").attr("value","身份证");
	$("tr#"+registrationID).find("#papersNumber").attr("value",arr[4]);
	$("tr#"+registrationID).find("#birthday").attr("value",arr[6]);
  	//隐藏弹出层
  	$("#accift").jqmHide();
  	var heis = parent.document.getElementById("mainframe3").offsetHeight - 200 + "px";
	parent.document.getElementById("mainframe3").style.height = heis;
...
function re_load() {
	if (token)
		document.location.href = basePath
				+ "ea/institutionsregistration/ea_getListInstitutionsRegistration.jspa?institutionsRegistration.organizationPID="
				+ organizationPID + "&pageNumber=" + ppageNumber
