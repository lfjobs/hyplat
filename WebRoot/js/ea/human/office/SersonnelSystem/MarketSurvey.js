$(document).ready(function() {
	...
	function action(com, grid) {
		switch (com) {
			...
			case '删除' :
				...
				$f = $('#cstaffForm');
				$f.find(':input#marketSurveyID').val(marketSurveyID);
				if (confirm("确定继续？")) {
					$f
							.attr("target", "hidden")
							.attr(
									"action",
									basePath
											+ "ea/marketsurvey/ea_delmarketSurvey.jspa?pageNumber="
											+ pNumber + "&search=" + search);
					document.cstaffForm.submit.click();

					$("tr#" + marketSurveyID).remove();
					marketSurveyID = "";
			...
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/marketsurvey/ea_getListMarketSurvey.jspa?search="
						+ search;
				numback(url);
				break;
		}
	...
	$("#tosearch").click(function() {
		...
		$("#postSearchForm").attr(
				"action",
				basePath + "ea/marketsurvey/ea_toSearch.jspa?pageNumber="
						+ pNumber);
		...
		if (marketSurveyID == "") {
			$("#cstaffForm")
					...
					.attr(
							"action",
							basePath
									+ "/ea/marketsurvey/ea_addmarketSurvey.jspa?pageNumber="
									+ pNumber);
			document.cstaffForm.submit.click();
			document.cstaffForm.reset();
			token = 1;
