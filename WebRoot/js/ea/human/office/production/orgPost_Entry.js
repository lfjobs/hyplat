$(function() {
	...
	function action(com, grid) {
		switch (com) {
			case '设置人员':
				...
				var postN = $("#orgPostForm").find("#"+depPostID).find("span#postName").html();
			...
			case '查询':
				...
				break;
			case '设置每页显示条数' :
				var url = basePath+ "ea/departmentpost/ea_getOrgPostListByOrg.jspa?search="+ search+"&departmentPost.organizationID="+orgId+"&star=00";
				numback(url);
				break;
			case '职务人员':
				if(depPostID==""){
	...
	$("#toCommit").click(function() {
		...
		var url = basePath +"ea/departmentpost/sajax_ea_orgPostEntry.jspa?date="+new Date()+"&departmentPost.organizationID="+orgId+"&entity.categoryName="+$("select#staffType option:selected").text();
