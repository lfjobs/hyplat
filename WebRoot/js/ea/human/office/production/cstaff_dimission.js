$(function() {
	...
	function action(com, grid) {
		switch (com) {
			...
			case '查询' :
				...
				break;
			case '导出' :
				url = basePath + "ea/cosdimission/ea_showExcel.jspa?search="
						+ psearch;
				open(url);
				break;
			case '转在职' :
				...
				var r=confirm("确定将此离职员工转为在职员工？");
				if(r){
					var url = basePath + "ea/cosdimission/sajax_n_ea_dimissonToCos.jspa";
					$.ajax({
							url : encodeURI(url),
							type : "get",
							async : true,
							dataType : "json",
							data:{"staffID":personvalue,"d":Math.ceil(Math.random()*1000)},
							success : function cbf(data) {
								//var member = eval("(" + data + ")");
								var sname = data.staffName;
								var staffTypeList = data.staffTypeList;
								$("select#staffTypeList").empty();
								var htmlStr="<option value=''>请选择</option>";
				           		for (var i = 0; i < staffTypeList.length; i++) {
				           			htmlStr+="<option value='"+staffTypeList[i].codeID+"'>"+staffTypeList[i].codeValue+"</option>";
				         		}
				           		$("select#staffTypeList").html(htmlStr);
								
								$("#stname").text(sname);
								$("#dimToCosDept").jqmShow();
							},
			...
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/cosdimission/ea_getListCOSDimission.jspa?search="
						+ psearch;
				numback(url);
				break;
			case '工资管理' :
				document.location.href = basePath + "page/ea/main/human/office/production/cstaff_dimissionSalary_search.jsp";
				break;
		}
	}
	...
	$("#saveDimToCos").click(function(){//人员转在职后专岗保存
		
		var deptID = $("#organizationPost").val();
		if(deptID == ''){
			alert('岗位不能为空！');
			return;
		}
		var typeId=$("select#staffTypeList option:selected").val();
		var typeName=$("select#staffTypeList option:selected").text();
		if(typeId==""){
			alert("员工工种不能为空!");
			return;
		}
		var staName = $("#stname").text();
		var orgName = $("#oraganizationName").val();
		var deptName = $("#organizationPost :selected").text();
		var url = basePath + "ea/cosdimission/sajax_n_ea_saveDimToCosDept.jspa";
		var vdata={};
		vdata["staffID"]=personvalue;
		vdata["staName"]=staName;
		vdata["orgID"]=treeid;
		vdata["orgName"]=orgName;
		vdata["deptID"]=deptID;
		vdata["deptName"]=deptName;
		vdata["staffTypeId"]=typeId;
		vdata["staffTypeName"]=typeName;
		vdata["d"]=Math.ceil(Math.random()*100);
		$.ajax({
				url : encodeURI(url),
				type : "post",
				async : true,
				...
				success : function cbf(data) {
					...
					document.location.href = basePath
							+ "ea/cosdimission/ea_getListCOSDimission.jspa?pageNumber=" + ppageNumber;
				},
				error : function cbf(data) {
					retoken = 0;
					alert("数据连接失败！");
	...
	$("#searchDim").click(function() {
		$("#cstaffDimForm").attr(
				"action",
				basePath + "ea/cosdimission/ea_toSearch.jspa?pageNumber="
						+ ppageNumber);
