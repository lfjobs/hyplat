$(function() {
	...
	function action(com, grid) {
		switch (com) {
			case '添加' :
				...
				$("#sa" + select).find("input#postName").addClass("put3");
				$("#sa" + select).find("input#postSureNum").addClass("isNaN");
				$("#sa" + select).show();
				select++;
				break;
			case '修改' :
				...
				$p.find('input:gt(0)').each(function() {
					if($(this).attr("name") == 'postSureNum'){
						$(this).addClass("isNaN");
					}
					if($(this).attr("name") == 'postName'){
						$(this).addClass("put3");
					}
					$(this).attr("name",
							"depPostMap[" + select + "]." + this.name);
			...
			case '删除' :
				...
				if (confirm("是否删除？")) {
					$("#orgPostForm").attr("target", "hidden").attr(
							"action",
							basePath + "ea/departmentpost/ea_deleteOrgPost.jspa?orgName="+orgname+"&departmentPost.depPostID="+depPostID);
			...
			case '全部保存':
				...
				$('#orgPostForm')
						...
						.attr(
								"action",
								basePath
										+ "ea/departmentpost/ea_sevaeOrgPostMap.jspa?orgName="+orgname+"&departmentPost.leveloneOrgID="+orgId);
				document.orgPostForm.submit.click();
				token = 2;
				break;
			case '查询':
				...
				break;
			case '设置每页显示条数' :
				var url = basePath+ "ea/departmentpost/ea_getOrgPostListByOrg.jspa?search="+ search+"&departmentPost.organizationID="+orgId;
				numback(url);
				break;
		}
	}
    ...
    $("#tosearch").click(function() {
		...
		$f.attr("action", basePath
						...
						+ pNumber+"&departmentPost.organizationID="+orgId);
...
function re_load() {
	if (token)
		document.location.href = basePath
				...
				+ $("#pageNumber").attr("value")+"&departmentPost.organizationID="+orgId;
}
