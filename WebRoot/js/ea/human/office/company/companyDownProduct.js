$(document).ready(function() {
	...
	function action(com, grid) {
		switch (com) {
			...
			/*case '产品合格':
				if (productdesignID == "") {
					alert('请选择!一条产品再设置产品合格 ');
					
					return;
				}
				if(confirm("确定设置产品合格吗？")){
					var url=basePath+"/ea/companytestproduct/ea_updatePpcestuer.jspa?pper=2&ppID="+ productdesignID;
					
					$("#addressForm").attr("target","hidden").attr("action",url);
					token=2;
			...
			case '产品不合格':
				if (productdesignID == "") {
					alert('请选择!一条产品再设置产品不合格');
					
					return;
				}
				if(confirm("确定设置产品不合格吗？")){
					var url=basePath+"/ea/companytestproduct/ea_updatePpcestuer.jspa?pper=3&ppID="+ productdesignID;
			...
			case '回滚':
				...
				if(confirm("确定设置产品回滚吗？")){
					var url=basePath+"/ea/companytestproduct/ea_updatePpcestuer.jspa?pper=1&ppID="+ productdesignID;
			...
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/productdesign/ea_getListProductdesign.jspa?identifier="+identifier+"&ghua="+ghua+"&flexbutton="+flexbutton+"&fiveClear="+fiveClear;
				numback(url);
				break;
			case '打印预览' :
			...
			case  '下架':
				...
				window.location.href=basePath + "/ea/wfjcustomer/ea_toxia.jspa?type=0&chenpID="+ productdesignID+",";		
				  break;
			case '产品发布' :
				if (productdesignID== "") {
					alert("请选择！");
...
//				window.location.href=basePath + "ea/cofi/ea_delItem.jspa?id="+ cid;
				window.open(basePath
						+ "/ea/productdesign/ea_toAddOrEditProductDesign.jspa?showType=edit&productDesign.ppID="+productdesignID+"&flexbutton=publish");
//				window.location.href=basePath + "/ea/wfjcustomer/ea_toxia.jspa?type=1&chenpID="+ productdesignID+",";
				break;
				break;
		}
...
function re_load() {
	if (token)
	document.location.href = basePath+ "/ea/companytestproduct/ea_getCompanyDownProduct.jspa";
	}
