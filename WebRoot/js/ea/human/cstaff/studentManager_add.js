$(function() {
	...
	$(".JQuerySubmits").click(function(){ //编辑菜单保存
		...
		var url = basePath + "ea/studentManager/sajax_ea_saveStudemtmanagerlist.jspa?date="+ new Date();
		$.ajax({
				...
				success: function(data){
					if(showType == 'add'){
						window.location.href = basePath+"ea/studentManager/ea_getStudentManagerList.jspa?showType=add";
					}else{
						var staffid = $("#staffID").text();
						window.location.href = basePath+"ea/studentManager/ea_getStudentManagerList.jspa?cstaff.staffID="+staffid+"&showType=edit";
					}
				},
				error:function(data){
					alert('保存失败！');
	...
	if(opetype=='edit'){
		...
		switch(menuid){
			case 1:
				...
				$("input#ReadCardBtn").hide();
				$("tr#tools").show();
				$("input#staffIdentityCard").removeClass("IdentityCard");
				break;
			default:
				$("div#box"+menuid).show();
