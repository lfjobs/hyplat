$(document).ready(function() {
	...
	parent.document.getElementById("mainframe17").style.height = 180 + len * 27 + "px";
		...
		switch (com) {
			case '添加' :
				...
				$("#sa" + select).find(':input:gt(0)').each(function() {
					$(this).attr("name",
							"agreementmap[" + select + "]." + this.name);
				});
				$("#sa" + select).show();
				var heis = parent.document.getElementById("mainframe17").offsetHeight + 27 + "px";
				parent.document.getElementById("mainframe17").style.height = heis;
				select++;
				break;
			...
			case '删除' :
				...
				if (agreementID.substring(0, 2) == "sa") {
					$("#" + agreementID).remove();
					agreementID = '';
					var heis = parent.document.getElementById("mainframe17").offsetHeight - 27 + "px";
					parent.document.getElementById("mainframe17").style.height = heis;
				...
				var hUrl = $("tr#"+agreementID).find("input#attachUrl").val();
				$f = $('#agreementForm');
				$f
						.attr("target", "hidden")
						.attr(
								"action",
								basePath
										+ "ea/agreement/ea_delAgreement.jspa?agreement.staffID="
										+ staffID
										+ "&agreement.agreementID="
										+ agreementID
										+ "&agreement.attachUrl="
										+ hUrl);
				document.agreementForm.submit.click();
				$("tr#" + agreementID).remove();
				agreementID = "";
				token = 11;
				break;
			case '全部保存' :
...
//				}
				$('#agreementForm')
						...
						.attr(
								"action",
								basePath
										+ "ea/agreement/ea_saveAgreement.jspa?agreement.staffID="
										+ staffID);
				document.agreementForm.submit.click();
				token = 2;
				break;	
	...
	$("a.aUrl").click(function() {
		var uid = $(this).parent();
		var attachUrl = $.trim($(uid).find("input#attachUrl").attr("value"));
		var urlReturn = OpenWord(attachUrl, 2);
		$(uid).find("input#attachUrl").attr("value", urlReturn);
	});
