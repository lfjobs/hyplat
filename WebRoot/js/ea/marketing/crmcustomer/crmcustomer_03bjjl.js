$(document).ready(function() {
	...
	function action(com, grid) {
		switch (com) {
			...
			case '删除' :
				...
				if (confirm("是否删除？")) {
					$f = $('#cstaffForm');
					$f.find('#activitykey').val(crmCustOfferKey);
					var url = basePath + "ea/marketingCrmCustomer/ea_delCustBjjl.jspa?pageNumber="
							+ pNumber;
					$f.attr("target", "hidden").attr("action", url);
					document.cstaffForm.submit.click();
					$("tr#" + crmCustOfferKey).remove();
					crmCustOfferKey = "";
					token = 11;
				}
				break;
			...
			case '查询' :
				...
				break;
			case '导出' :
				url = basePath + "ea/marketingCrmCustomer/sajax_ea_showBjjlExcel.jspa?crmCustomer.customerkey=" + customerkey + "&search=" + search;
				open(url);
				break;
			case '导入' :
				...
				break;
			case '设置每页显示条数' :
				var url = basePath + "ea/marketingCrmCustomer/ea_getListBjjl.jspa?crmCustomer.customerkey=" + customerkey + "&search=" + search;
				numback(url);
				break;
		}
	}
	...
	$(".JQuerySubmit").click(function() {
		...
		$("#cstaffForm").attr("target", "hidden").attr("action",
				basePath + "ea/marketingCrmCustomer/ea_saveOrUpdateBjjl.jspa?pageNumber=" + pNumber);		
		if (crmCustOfferKey == "") {			
			document.cstaffForm.submit.click();
	...
	$("#searchStaff").click(function() {
		$("#postSearchForm").attr("action",
				basePath + "ea/marketingCrmCustomer/ea_toSearchBjjl.jspa?crmCustomer.customerkey=" + customerkey + "&pageNumber=" + pNumber);
...
function re_load() {
	...
	if (token) {
		document.location.href = basePath
				+ "ea/marketingCrmCustomer/ea_getListBjjl.jspa?pageNumber=" + pNumber
