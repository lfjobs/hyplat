	...
	function action(com, grid) {
		switch (com) {
			...
			case '设置产品价格':
				...
				var url=basePath+"/ea/productdesign/ea_getpackegejiage.jspa?productDesign.ppID="+ productdesignID;
			...
			case '上架':
				...
				.attr(
						"action",
						basePath
								+ "/ea/wfjcustomer/ea_toxia.jspa");
					$("#type").val("1");
					$("#ID").val(productdesignID);
					
					document.shangjia.submit();		
				break;
			case  '下架':
				if (productdesignID== "") {
					alert("请选择！");
				...
				.attr(
						"action",
						basePath
								+ "/ea/wfjcustomer/ea_toxia.jspa?type=0&chenpID="+ productdesignID+",");
				$("#type").val("1");
				$("#ID").val(productdesignID);
					document.shangjia.submit();
				  break;
...
function re_load() {
	if (token)
		document.location.href = basePath
				...
				+ $("#pageNumber").attr("value")+"&search="+search+"&sdate="+sdate+"&edate="+edate+"&identifier="+identifierf+"&fiveClear="+fiveClear;
}
