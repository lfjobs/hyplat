...
$(function(){
   ...
   var timer  =  setTimeout(function(){
    ...
    $("#print").click(function(){
      ...
      var ulp = basePath
            + "ea/sm/sajax_ea_printTicket.jspa";
        $.ajax({
            type : "GET",
            url : ulp,
            dataType : "json",
            ...
            success : function(data) {
                ...
                if(posNum==null||posNum=="") {
                    ...
                    if(isAndroid==true){
                        ...
                        Android.printTicketAd(staffcode,posNum,journalNum,year+"-"+month+"-"+day+" "+hour+":"+minute+":"+second,$(".totalnum").text(),$(".totalmoney").text(),(ssprice!=null&&ssprice!=""?ssprice:$(".totalmoney").text()),(zlprice!=null&&zlprice!=""?zlprice:0),zf,companyTel,companyAddr,JSON.stringify(array),companyName,"http://www.impf2010.com/ea/wfjshop/ea_getjspzc.jspa?sccid=");
                        $(".alert_suc_").hide();
                        //定时跳转到首页
...
function CreatePrintPage() {
   ...
   LODOP.SET_PRINT_STYLE("FontSize",8);
    ...
    if(zf!="转他人支付"){
        hPos+=rowHeight;
        LODOP.ADD_PRINT_TEXT(hPos,5,80,50,"实付:");
        LODOP.ADD_PRINT_TEXT(hPos,132,190,50,(ssprice!=null&&ssprice!=""?ssprice:$(".totalmoney").text()));
        hPos+=rowHeight;
        LODOP.ADD_PRINT_TEXT(hPos,5,80,50,"找零:");
        LODOP.ADD_PRINT_TEXT(hPos,132,190,50,(zlprice!=null&&zlprice!=""?zlprice:0));
    }

    hPos+=rowHeight;
    LODOP.ADD_PRINT_TEXT(hPos,5,80,50,"支付方式：");
    LODOP.ADD_PRINT_TEXT(hPos,120,90,50,zf);
    hPos+=rowHeight;
    LODOP.ADD_PRINT_LINE(hPos,5, hPos, 500,2,0);

    LODOP.SET_PRINT_STYLE("FontSize",7);
    LODOP.SET_PRINT_STYLE("Bold",0);
    LODOP.ADD_PRINT_TEXT(hPos+5,5,180,50,"电话:"+companyTel);
    hPos+=rowHeight;
    LODOP.ADD_PRINT_TEXT(hPos+5,5,178,50,companyAddr);
    hPos+=rowHeight;
    LODOP.ADD_PRINT_BARCODE(hPos+10,36,150,150,"QRCode","http://www.impf2010.com/ea/wfjshop/ea_getjspzc.jspa?sccid=");
    hPos+=rowHeight+300;
    LODOP.ADD_PRINT_LINE(hPos,5, hPos, 500,2,0);
...
function jisuan(){
    ...
    $(".tr").each(function(){
        ...
        if(barCode.substring(0,2)=="21"){
            if(fhform=="3"){
                 if(variableID=="KG"||variableID=="kg"||variableID=="千克") {
