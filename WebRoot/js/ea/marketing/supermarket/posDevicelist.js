...
$(function() {
	...
	$('.flexme11').flexigrid({
				...
				width : 'auto',
				minwidth : 30,
				title : "POS销售终端管理"+"&nbsp;&nbsp;&nbsp;&nbsp;设备号：<input type='text' id='ch' style='width:100px'/> &nbsp;投放区域：<input type='text' id='oraganization' style='width:100px'/> &nbsp;使用状态：<select id='sta' ><option value=''>全部</option><option value='0'>使用中</option><option value='1'>已停用</option><option value='2'>已收回</option></select><input type='button' value=' 查询 ' id='toSearch' class='input-button'/> ",
				minheight : 80,
				buttons : [{
							name : '添加',
							bclass : 'add',
	...
	function action(com, grid){
		switch (com) {
			...
			case '删除':
                ...
                if(confirm("确定删除？")) {
                    var posKey = $("tr#" + posID).find("#posKey").text();
                    $("#addForm #posKey").val(posKey);
                    $("#addForm").attr("target", "hidden").attr("action", basePath + "ea/pos/ea_deletePos.jspa");
		    ...
		    case '设置每页显示条数' :
			var url = basePath
						+ "ea/pos/ea_getPosList.jspa?search="
						+ search;
				numback(url);
				break;
		}
	...
	$("#save").click(function() {
		...
		$("#addForm").attr("target","hidden").attr(
				"action",
				basePath + "ea/pos/ea_addOrUpdate.jspa");
        token = 2;
		document.addForm.submit.click();
	});
    ...
    $("#clearID").click(function(){
        $("#addForm #accessCcomID").val("");
        $("#addForm #accessCcomName").val("");
    ...
    //选择应用
    $("#addForm #app").change(function(){
       var accessCcomID = $("#addForm #accessCcomID").val();
       if(accessCcomID==""){
           alert("请先选择默认公司网站");
           return false;
       }
    ...
    $("#toSearch").click(function() {
        ...
        $("#searchForm").find("#organizationName").val($("#oraganization").val());
        $("#searchForm").find("#state").val($("#sta").val());
        $("#searchForm").attr(
            "action",
            basePath + "ea/pos/ea_toSearch.jspa");
        token = 2;
   ...
   //服务区域数据开始---------------------------------------------------
    ...
    tree.setOnDblClickHandler(function(){
        $("#addForm #organizationID").attr("value",treeid);
        $("#addForm #organizationName").attr("value",treename);
        $("#addForm #orgName").attr("value",treename);
        $("#addForm #orgName").focus();
        $("#serdevice").hide();
    });
    function getPID() {
    ...
    //添加选择服务区域
    $("#addForm #orgName").click(function () {
...
function checkPosNum(posNum) {
    ...
    var bool = null;
    var url = basePath + "ea/pos/sajax_ea_checkRepPosNum.jspa?date="
        + new Date();
    $.ajax({
        url : url,
        type : "get",
        ...
        dataType : "json",
        data : {
            "posDevice.posNum" : $("#addTable #posNum").val(),
            "posDevice.posID":posID
        },
...
function getSiteList(){
    ...
    $.ajax({
        ...
        success : function(data) {
			...
			$(".site").append(a.join(""));
			  if(posID!=""){
	               $("table#addTable").find("#siteId").val($("tr#" + posID).find("#siteId").text());
                  siteId = $("tr#" + posID).find("#siteId").text();
	            }
...
//根据场地查询场地的设备
function getEquipList(siteId){
	$(".equip").html("");
    var url = basePath + "ea/carmanage/sajax_ea_getEquipList.jspa?date="
        + new Date();
    $.ajax({
        url : url,
        type : "get",
        ...
        success : function(data) {
            var member = eval("(" + data + ")");
            var list = member.equiplist;
        	var a = [];
			$(list).each(function(i, dom) {
				a.push("<option value='"+this[1]+"'>"+this[1]+"</option>");
			});
			$(".equip").append(a.join(""));
            if(posID!=""){
               $("table#addTable").find("#equipmentNumber").val($("tr#" + posID).find("#equipmentNumber").text());
            }
...
//获取货柜编号
function gethgcodeList(){
    var accessCcomID = $("#addForm #accessCcomID").val();
    $(".hg").html("");
    var url = basePath + "ea/sm/sajax_ea_getHgCodeList.jspa?date="
        + new Date();
    $.ajax({
        url : url,
        type : "get",
        ...
        success : function(data) {
            var member = eval("(" + data + ")");
            var list = member.hgcodelist;
            var a = [];
            $(list).each(function(i, dom) {
                a.push("<option value='"+this.depotCoding+"'>"+this.depotCoding+this.depotName+"</option>");
            });
            $(".hg").append(a.join(""));
            if(posID!=""){
                $("table#addTable").find("#hgbh").val($("tr#" + posID).find("#hgcode").text());
            }
