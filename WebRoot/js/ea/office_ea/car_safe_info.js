	...
	function action(com, grid) {
		switch (com) {
			...
			case '删除' :
				...
				$f = $('#carSafeinformationForm');
				$f.find(':input#carSafeID').val(carSafeID);
				if (confirm("是否删除？")) {
					$("#carSafeinformationForm").attr("target", "hidden").attr(
							"action",
							basePath + "ea/carsafeinformation/ea_deleteSafe.jspa?pageNumber="+ pNumber  + "&carsafeinformation.carSafeID="+ carSafeID);
					document.carSafeinformationForm.submit.click();
					token = 2;
//					$("tr#" + carSafeID).remove();
//					carSafeID = "";
			...
			case '查询':
				...
				break;
			case '设置每页显示条数' :
				var url = basePath+ "ea/carsafeinformation/ea_getCarsafeinformationList.jspa?search="+ search+"&carInformation.carID="
						+ carID+"&type="+type;
				numback(url);
				break;
			case '导出' :
				var url = basePath+ "ea/carsafeinformation/ea_showSafeExcel.jspa?";
				open(url);
				break;
		}
	}
	...
	$(".JQueryflexme tr[id]").click(function() {
		carSafeID = this.id;
		if (personurl) {
			$("#mainframe").attr("src", personurl + carSafeID);
		}
				...
				if ($("form .error").length) {
					...
					$("#addForm")
							...
							.attr(
									"action",
									basePath
											+ "ea/carsafeinformation/ea_saveSafeList.jspa?pageNumber="
	...
	$("input.JQuerySubmits").click(function() {
		...
		$("#updateForm")
				...
				.attr(
						"action",
						basePath
								+ "ea/carsafeinformation/ea_updateCarSafe.jspa?pageNumber="
								+ pNumber  + "&carsafeinformation.carSafeID="+ carSafeID);
	...
	$("#searchCar").click(function() {
		...
		$("#carSearchForm").attr(
				"action",
				basePath + "/ea/carsafeinformation/ea_toSearch.jspa?pageNumber="
						+ pNumber);
...
function re_load() {
	if (token)
		document.location.href = basePath
				+ "ea/carsafeinformation/ea_getCarsafeinformationList.jspa?pageNumber="
