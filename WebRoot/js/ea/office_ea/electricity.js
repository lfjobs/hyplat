$(document).ready(function() {
	...
	function action(com, grid) {
		switch (com) {
			...
			case '删除' :
				...
				if (confirm("确定继续？")) {
					$f.attr("target", "hidden").attr(
							"action",
							basePath + "ea/electricity/ea_del.jspa?pageNumber="
									+ pNumber + "&electricity.electricityID="
									+ addressID);
				...
				break;
			case '导出' :
				var url = basePath + "ea/electricity/ea_showExcel.jspa?search="
						+ search;
				open(url);
				break;
			// case '查询':
			...
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/electricity/ea_getListForPage.jspa?search="
						+ search;
				numback(url);
				break;
		}
	...
	$("input.JQuerySubmit").click(function() {// 保存
				...
				if (addressID == "") {
					$("#cstaffForm").attr("target", "hidden").attr(
							"action",
							basePath
									+ "ea/electricity/ea_save.jspa?pageNumber="
									+ pNumber);
				...
				$("#cstaffForm").attr("target", "hidden").attr(
						"action",
						basePath + "ea/electricity/ea_save.jspa?pageNumber="
								+ pNumber);
				document.cstaffForm.submit.click();
				token = 2;
			});
	...
	$("span.list_remove").click(function() {// 删除点击事件
				...
				$f.attr("action", basePath
								+ "ea/electricity/t_ea_del.jspa?pageNumber="
								+ pNumber);
				document.addressForm.submit.click();
			});
	...
	$("#tosearch").click(function() {
		...
		$("#postSearchForm").attr(
				"action",
				basePath + "ea/electricity/ea_toSearch.jspa?pageNumber="
						+ pNumber);
...
function re_load() {
	if (token)
		document.location.href = basePath
				+ "ea/electricity/ea_getListForPage.jspa?pageNumber=" + pNumber
