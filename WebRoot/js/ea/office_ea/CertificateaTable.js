	...
	function action(com, grid) {
		switch (com) {
			...
			case '删除' :
				...
				$f = $('#carCertificateaForm');
				$f.find(':input#certifiID').val(certifiID);
				if (confirm("是否删除？")) {
					$("#carCertificateaForm").attr("target", "hidden").attr(
							"action",
							basePath + "ea/certificateatable/ea_deleteatable.jspa?pageNumber="+ pNumber  + "&certificateatable.certifiID="+ certifiID);
					document.carCertificateaForm.submit.click();
					token = 2;
//					$("tr#" + certifiID).remove();
//					certifiID = "";
			...
			case '查询':
				...
				break;
			case '设置每页显示条数' :
				var url = basePath+ "ea/certificateatable/ea_getCertificateaTableList.jspa?search="+ search+"&carInformation.carID="
						+ carID+"&type="+type;
				numback(url);
				break;
			case '导出' :
				var url = basePath+ "ea/certificateatable/ea_showcateaExcel.jspa?";
				open(url);
				break;
		}
	}
	...
	$(".JQueryflexme tr[id]").click(function() {
		certifiID = this.id;
		if (personurl) {
			$("#mainframe").attr("src", personurl + certifiID);
		}
				...
				if ($("form .error").length) {
					...
					$("#addForm")
							...
							.attr(
									"action",
									basePath
											+ "ea/certificateatable/ea_savecateatList.jspa?pageNumber="
	...
	$("input.JQuerySubmits").click(function() {
		...
		$("#updateForm")
				...
				.attr(
						"action",
						basePath
								+ "ea/certificateatable/ea_updateCatea.jspa?pageNumber="
								+ pNumber  + "&certificateatable.certifiID="+ certifiID);
	...
	$("#searchCar").click(function() {
		...
		$("#carSearchForm").attr(
				"action",
				basePath + "/ea/certificateatable/ea_toSearch.jspa?pageNumber="
						+ pNumber);
...
function re_load() {
	if (token)
		document.location.href = basePath
				+ "ea/certificateatable/ea_getCertificateaTableList.jspa?pageNumber="
