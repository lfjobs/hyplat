$(function() {
	...
	function action(com, grid) {
		switch (com) {
			...
			case '删除' :
				...
				$f = $('#cstaffForm');
				$f.find(':input#taskNoticeID').val(taskNoticeID);
				if (confirm("是否删除？")) {
					$("#cstaffForm")
							.attr("target", "hidden")
							.attr(
									"action",
									basePath
											+ "ea/tasknotice/ea_delTaskNotice.jspa?pageNumber="
											+ ppageNumber + "&search="
											+ psearch);
					document.cstaffForm.submit.click();
					$("tr#" + taskNoticeID).remove();
					taskNoticeID = "";
					token = 11;
				}
				break;
			case '查询' :
				...
				break;
			case '导出' :
				url = basePath + "ea/tasknotice/ea_showExcel.jspa?search="
						+ psearch;
				open(url);
				break;
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/tasknotice/ea_getTaskNoticeList.jspa?search="
						+ psearch;
				numback(url);
				break;
		}
				...
				if (taskNoticeID == "") {
					$("#cstaffForm")
							...
							.attr(
									"action",
									basePath
											+ "ea/tasknotice/ea_saveTaskNotice.jspa?pageNumber="
											+ ppageNumber);
					document.cstaffForm.submit.click();
					document.cstaffForm.reset();
					token = 1;
	...
	$("#tosearch").click(function() {
		$("#postSearchForm").attr(
				"action",
				basePath + "ea/tasknotice/ea_toSearch.jspa?pageNumber="
						+ ppageNumber);
...
function re_load() {
	if (token)
		document.location.href = basePath
				+ "ea/tasknotice/ea_getTaskNoticeList.jspa?search=" + psearch
