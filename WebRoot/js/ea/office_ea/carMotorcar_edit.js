	...
	function action(com, grid) {
		switch (com) {
			...
			case '删除' :
				...
				$f = $('#motorcarForm');
				$f.find(':input#motorcarID').val(motorcarID);
				if (confirm("是否删除？")) {
					$("#motorcarForm").attr("target", "hidden").attr(
							"action",
							basePath + "ea/motorcar/ea_deleteCarmotorcar.jspa?pageNumber="+ pNumber  + "&motorcar.motorcarID="+ motorcarID);
					document.motorcarForm.submit.click();
					token = 2;
//					$("tr#" + motorcarID).remove();
//					motorcarID = "";
			...
			case '查询':
				...
				break;
			case '设置每页显示条数' :
				var url = basePath+ "ea/motorcar/ea_getMotorcarList.jspa?search="+ search+"&carInformation.carID="
						+ carID+"&type="+type;
				numback(url);
				break;
			case '导出' :
				var url = basePath+ "ea/motorcar/ea_showCarmotorcarExcels.jspa?";
				open(url);
				break;
		}
	}
	...
	$(".JQueryflexme tr[id]").click(function() {
		motorcarID = this.id;
		if (personurl) {
			$("#mainframe").attr("src", personurl + motorcarID);
		}
				...
				if ($("form .error").length) {
					...
					$("#addForm")
							...
							.attr(
									"action",
									basePath
											+ "ea/motorcar/ea_saveCarmotorcar.jspa?pageNumber="
	...
	$("input.JQuerySubmits").click(function() {
		...
		$("#updateForm")
				...
				.attr(
						"action",
						basePath
								+ "ea/motorcar/ea_updateCarmotorcar.jspa?pageNumber="
								+ pNumber + "&search=" + search+ "&motorcar.motorcarID="+ motorcarID);
	...
	$("#searchCar").click(function() {
		...
		$("#carSearchForm").attr(
				"action",
				basePath + "/ea/motorcar/ea_toSearch.jspa?pageNumber="
						+ pNumber);
...
function re_load() {
	if (token)
		document.location.href = basePath
				+ "ea/motorcar/ea_getMotorcarList.jspa?pageNumber="
