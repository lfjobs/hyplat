$(document).ready(function() {
	...
	$('.cabinet0').flexigrid({
		...
		title : "<img src='"
				+ basePath
				+ "images/ea/office/fileCabinet/009.gif' width='20' height='30'/>"
				+ fileFolderName,
		minheight : 330,
		buttons : [{
			name : '返回上级',
	...
	function action(com, grid) {
		switch (com) {
			...
			case '删除' :
				...
				if (confirm("确定继续？")) {
					window.location.href = basePath
							...
							+ "&fileFolderId=" + fileFolderId + "&sort=file"
			...
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/filecabinet/ea_getFileOfFolder.jspa?1=1";
				numback(url);
				break;
	...
	$("#submitUpload").click(function() {
		...
		$("#uploadFileForm #uploadInfoMode").val("upload");
		$("#uploadFileForm #uploadInfoFoId").val(fileFolderId);
...
function uploadFiles(str) {
	if (str == "upload") {
		...
		$("#hideAttachUpdate").show();
		$("#uploadFilesUpdate").text("继续添加");
		$("#hideAttachUpdate")
				.append("<div id='attachupdate_"
						+ k
						+ "'><input type='file' name='fileUpload.sourcePath' id='sourcePathUpdate_"
						+ k + "'/><a href='javascript:deleteAttach(" + k
						+ ")'>删除</a></div>");
		k++;
	}
...
function queryFileCabinet(str) {
	// $("option", $("#deptIDofSubscriber")).remove();
	var urlc = basePath
...
// 更新文件
function updateFile() {
	$("#uploadFilesUpdate").text("添加附件");
	$("#hideAttachUpdate").hide();
	queryFileCabinet("uploadFileUpdate");
...
function showDiv() {
	...
	setTimeout("closeDiv()", 20000);// 5秒后显示DIV
}
function closeDiv() {
