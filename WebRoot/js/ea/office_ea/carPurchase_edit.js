$(function() {
	...
	function action(com, grid) {
		switch (com) {
			...
			case '删除' :
				...
				$f = $('#purchaseForm');
				$f.find(':input#purchaseID').val(purchaseID);
				if (confirm("是否删除？")) {
					$("#purchaseForm").attr("target", "hidden").attr(
							"action",
							basePath + "ea/carpurchase/ea_deletpurchase.jspa?pageNumber="+ pNumber  + "&purchase.purchaseID="+ purchaseID);
					document.purchaseForm.submit.click();
					token = 2;
					//$("tr#" + purchaseID).remove();
					//purchaseID = "";
			...
			case '查询':
				...
				break;
			case '设置每页显示条数' :
				var url = basePath+ "ea/carpurchase/ea_getPurchaseList.jspa?search="+ search+"&carInformation.carID="
						+ carID+"&type="+type;
				numback(url);
				break;
			case '导出' :
				var url = basePath+ "ea/carpurchase/ea_showPurchaseExcel.jspa?";
				open(url);
				break;
		}
	}
	...
	$(".JQueryflexme tr[id]").click(function() {
		purchaseID = this.id;
		if (personurl) {
			$("#mainframe").attr("src", personurl + purchaseID);
		}
				...
				if ($("form .error").length) {
					...
					$("#addForm")
							...
							.attr(
									"action",
									basePath
											+ "ea/carpurchase/ea_savePurchaseList.jspa?pageNumber="
	...
	$("input.JQuerySubmits").click(function() {
		...
		$("#updateForm")
				...
				.attr(
						"action",
						basePath
								+ "ea/carpurchase/ea_updatePurchase.jspa?pageNumber="
								+ pNumber  + "&purchase.purchaseID="+ purchaseID);
	...
	$("#searchCar").click(function() {
		...
		$("#carSearchForm").attr(
				"action",
				basePath + "/ea/carpurchase/ea_toSearch.jspa?pageNumber="
						+ pNumber);
...
function re_load() {
	if (token)
		document.location.href = basePath
				+ "ea/carpurchase/ea_getPurchaseList.jspa?pageNumber="
