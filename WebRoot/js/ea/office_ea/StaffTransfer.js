$(function() {
	...
	function action(com, grid) {
		switch (com) {
			...
			case '删除' :
				...
				$f = $('#cstaffForm');
				$f.find(':input#transferID').val(transferID);
				if (confirm("是否确定删除？")) {
					$("#cstaffForm")
							.attr("target", "hidden")
							.attr(
									"action",
									basePath
											+ "ea/stafftransfer/ea_delstaffTransfer.jspa?pageNumber="
											+ ppageNumber + "&search="
											+ psearch);
					document.cstaffForm.submit.click();
					$("tr#" + transferID).remove();
					transferID = "";
					token = 11;
				}
				break;
			case '查询' :
				...
				break;
			case '导出' :
				url = basePath + "ea/stafftransfer/ea_showExcel.jspa?search="
						+ psearch;
				open(url);
				break;
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/stafftransfer/ea_getstaffTransferList.jspa?search="
						+ psearch;
				numback(url);
				break;
		}
				...
				if (transferID == "") {
					$("#cstaffForm")
							...
							.attr(
									"action",
									basePath
											+ "ea/stafftransfer/ea_savestaffTransfer.jspa?pageNumber="
											+ ppageNumber + "&search="
											+ psearch);
					document.cstaffForm.submit.click();
					document.cstaffForm.reset();
	...
	$("#tosearch").click(function() {
		$("#postSearchForm").attr(
				"action",
				basePath + "ea/stafftransfer/ea_toSearch.jspa?pageNumber="
						+ ppageNumber);
