$(document).ready(function() {
		...
		switch (com) {
			...
			case '删除' :
				...
				if (confirm("是否删除？")) {
					...
					var url = basePath
							+ "ea/propagate/ea_delPropagate.jspa?pageNumber="
							+ pNumber;
			...
			case '导出' :
				url = basePath
						+ "ea/propagate/ea_showPropagateExcel.jspa?search="
						+ search;
				open(url);
				break;
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/propagate/ea_getPropagateList.jspa?search="
						+ search;
				numback(url);
				break;
		}
	...
	$(".JQuerySubmit").click(function() {
		$("#cstaffForm").attr("target", "hidden").attr(
				"action",
				basePath + "ea/propagate/ea_savePropagate.jspa?pageNumber="
						+ pNumber);
		if (propagateID == "") {
			document.cstaffForm.submit.click();
	...
	$("#searchStaff").click(function() {
		$("#postSearchForm").attr(
				"action",
				basePath + "ea/propagate/ea_toSearch.jspa?pageNumber="
						+ pNumber);
...
function re_load() {
	if (token) {
		document.location.href = basePath
				+ "ea/propagate/ea_getPropagateList.jspa?pageNumber=" + pNumber
