	...
	function action(com, grid) {
		switch (com) {
			...
			case '删除' :
				...
				$f = $('#carCNGForm');
				$f.find(':input#cngID').val(cngID);
				if (confirm("是否删除？")) {
					$("#carCNGForm").attr("target", "hidden").attr(
							"action",
							basePath + "ea/carcng/ea_deleteCarcng.jspa?pageNumber="+ pNumber  + "&carCNG.cngID="+ cngID);
					document.carCNGForm.submit.click();
					token = 2;
//					$("tr#" + cngID).remove();
//					cngID = "";
			...
			case '查询':
				...
				break;
			case '设置每页显示条数' :
				var url = basePath+ "ea/carcng/ea_getCarCNGList.jspa?search="+ search+"&carInformation.carID="
						+ carID+"&type="+type;
				numback(url);
				break;
			case '导出' :
				var url = basePath+ "ea/carcng/ea_showCngExcel.jspa?";
				open(url);
				break;
		}
	}
	...
	$(".JQueryflexme tr[id]").click(function() {
		cngID = this.id;
		if (personurl) {
			$("#mainframe").attr("src", personurl + cngID);
		}
				...
				if ($("form .error").length) {
					...
					$("#addForm")
							...
							.attr(
									"action",
									basePath
											+ "ea/carcng/ea_savecarcngList.jspa?pageNumber="
	...
	$("input.JQuerySubmits").click(function() {
		...
		$("#updateForm")
				...
				.attr(
						"action",
						basePath
								+ "ea/carcng/ea_updateCarcng.jspa?pageNumber="
								+ pNumber  + "&carCNG.cngID="+ cngID);
	...
	$("#searchCar").click(function() {
		...
		$("#carSearchForm").attr(
				"action",
				basePath + "/ea/carcng/ea_toSearch.jspa?pageNumber="
						+ pNumber);
...
function re_load() {
	if (token)
		document.location.href = basePath
				+ "ea/carcng/ea_getCarCNGList.jspa?pageNumber="
