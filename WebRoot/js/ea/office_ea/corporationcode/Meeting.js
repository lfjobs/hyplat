$(document).ready(function() {
	...
	function action(com, grid) {
		switch (com) {
			...
			case '删除' :
				...
				if (confirm("是否删除？")) {
					$f = $('#cstaffForm');
					$f.find('#meetingID').val(meetingID);
					var url = basePath
							+ "ea/meeting/ea_delMeeting.jspa?pageNumber="
							+ pNumber;
			...
			case '查询' :
				...
				break;
			case '导出' :
				url = basePath + "ea/meeting/ea_showMeetingExcel.jspa?search="
						+ search;
				open(url);
				break;
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/meeting/ea_getMeetingList.jspa?search=" + search;
				numback(url);
				break;
		}
	}
	...
	$(".JQuerySubmit").click(function() {
		...
		if (meetingID == "") {
			$("#cstaffForm").attr("target", "hidden").attr(
					"action",
					basePath + "ea/meeting/ea_saveMeeting.jspa?pageNumber="
							+ pNumber);
			document.cstaffForm.submit.click();
	...
	$("#searchStaff").click(function() {
		$("#postSearchForm").attr("action",
				basePath + "ea/meeting/ea_toSearch.jspa?pageNumber=" + pNumber);
...
function re_load() {
	if (token) {
		document.location.href = basePath
				+ "ea/meeting/ea_getMeetingList.jspa?pageNumber=" + pNumber
 ...
 */
							...
							var ts3 = new TreeSelector($("#deptID")[0], data,
									-1);
							ts3.createTree();
							var ts = new TreeSelector(
									$t.find("select#mtOrganizationID")[0],
									data, -1);
							ts.createTree();
						},
