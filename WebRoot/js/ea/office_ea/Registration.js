$(document).ready(function() {
	...
	window.parent.document.getElementById("mainframe4").style.height = 180 + len * 27 + "px";
	 $(".jqmWindow").jqm({
...
//                .jqDrag('.drag');// 
    ...
    function action(com, grid){
           ...
           case '添加':
            		$("#sa").after($("#sa").clone(true).attr("id","sa"+select).addClass("check"));
            	      $("#sa"+select).find(':input:gt(0)').each(function(){
                        $(this).attr("name","registrationmap["+select+"]."+this.name);
                     });
            	    $("#sa"+select).show();
		...
		case '全部保存':
			  if(notoken)
			    {return;}
			     if(select==1)
			     {
			       ...
			       notoken = 1;
		       var re = 0;
		        $("input[name$='bankAccount']",$(".check")).each(function(i,tmp){
		          if(this.value == ""){
		          alert("开户账号不能为空");
		        ...
		        $("input[name$='bankName']",$(".check")).each(function(i,tmp){
		          if(this.value == ""){
		          alert("银行名称不能为空");
		          $(this).css("background-color","red");
		          re = 1;
			 ...
			 $('#staffappraisalForm').attr("target","hidden").attr("action",basePath+"ea/registration/ea_addRegistration.jspa?pageNumber="+ppageNumber);
			...
			case '删除':
			     if(registrationID=='')
				  ...
				  if(registrationID.substring(0,2) == "sa")
				  {
			    ...
			    if (confirm("是否删除？")) {
				$f.attr("target","hidden").attr("action",basePath+"ea/registration/ea_delRegistration.jspa?pageNumber="+ppageNumber+"&registration.registrationID="+registrationID);
				document.staffappraisalForm.submit.click();
				$("tr#"+registrationID).remove();
					registrationID="";
					token=11;
			...
			case '设置每页显示条数':
			    var url = basePath
						...
						+ ccompanyID + "&search=" + search;
				numback(url);
				break;
			case '导出':
				var url = basePath+"ea/registration/ea_showExcel.jspa?search="+search+"&registration.ccompanyID="+ccompanyID;
				open(url);
				break;
		   case '查询':
   ...
   //根据条件查询
    $("#tosearch").click(function(){
          ...
          $("#appraisalForm").attr("action", basePath+"ea/registration/ea_toSearch.jspa?pageNumber="+ppageNumber);
          document.appraisalForm.submit.click();
      });
      
	$("#openAccountDate").blur(function() {
				var start = $(this).val();
				if (start != '') {
					$("#start").attr("value", start);
				}
			});
	$("#cancellationDate").blur(function() {
				var end = $(this).val();
				if (end != '') {
					$("#end").attr("value", end);
				}
...
////////////////////////////////地址查询!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!==================================================BEGIN!
    ...
    function LiuZhongYaoDeShaGuaDiZhisearch(address){
        ...
        if (DistrictID == "") {
            var url = basePath+"ea/cstaff/sajax_n_ea_getCDistricts.jspa?districtPID=0"+"&date="+new Date();
             $.ajax({
						                        url: url,
						                        type: "get",
						                        async: true,
        ...
        var urldistrict = basePath+"ea/cstaff/sajax_n_ea_getPDetailsDistricts.jspa?districtPID="+DistrictID+"&date1="+new Date();
         $.ajax({
						                        url: urldistrict,
						                        type: "get",
						                        async: true,
	...
	$('td.JQueryaddresssearch select[number]').change(function(){
            ...
            $(searchrovince, $td).children("option:selected").attr("class");
            $($td).children('select:gt(' + number + ')').attr("disabled", false);
            searchdistrictPID = $(searchrovince, $td).children("option:selected").val();
            if(searchdistrictPID == '--请选择--'){
              retoken=0;
               return;
            }
            var url = basePath+"ea/cstaff/sajax_n_ea_getCDistricts.jspa?districtPID="+searchdistrictPID+"&date2="+new Date();
             $.ajax({
						                        url: encodeURI(url),
						                        type: "get",
						                        async: true,
...
function re_load(){
 	if(token)
	 document.location.href=basePath+"ea/registration/ea_getListRegistration.jspa?registration.ccompanyID="+ccompanyID+"&pageNumber="+ppageNumber+"&pageForm.pageNumber="+$("#pageNumber").attr("value");
 }