...
//				},
		...
		switch (com) {
			case '查看' :
				...
				window.location.href = basePath
						...
						+ docId + "&type=toFinishedView" + "&load=" + load
			...
			case '归档' :
				...
				var status = $("tr#" + docId).find("input#status").val();
				if (status == "G") {
					alert("已归档");
					return;
				}
			//	if (confirm("确定归档?")) {
					...
					$("#CommonForm")
							...
							.attr(
									"action",
									basePath
											+ "ea/documentinfo/ea_completeArchive.jspa?date="
											+ new Date());
			...
			case '放入回收站' :
				...
				//if (confirm("确定放入回收站？")) {
					putRecycleBin(docId+",", "read");
				//}
				break;
			case '审批跟踪登记表':
				document.location.href=basePath+"/ea/documentflow/ea_getReadDocList.jspa?finishType=readyes&track=00";
				break;
			case '查询' :
				showQueryContent("yesread");
				break;
			case '导出' :
				url = basePath + "/ea/documentflow/ea_showExcel.jspa?search="
						+ search+"&finishType=readyes&date="
						+ new Date();;
				open(url);
				break;
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/documentflow/ea_getReadDocList.jspa?finishType=readyes&search="+search+"&date="
						+ new Date();
				numback(url);
				break;
	...
	$("#tosearch").click(function() {
		$("#searchForm #finishType").val("readyes");
...
function checkshare() {
	var docId = $("input[name=radioGroup]:radio:checked").val();
	var shareType = $("input[name=range]:radio:checked").val();
	var url = basePath + "ea/documentinfo/sajax_ea_checkShare.jspa?date="
			+ new Date();
...
function re_load() {
	if (token) {
		document.location.href = basePath
				+ "ea/documentflow/ea_getReadDocList.jspa?finishType=readyes";
