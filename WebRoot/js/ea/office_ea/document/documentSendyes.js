				...
				function() {
					...
					function action(com, grid) {
						...
						case '放入回收站':
							...
							// if (confirm("确定放入回收站？")) {
							putRecycleBin(docId + ",", "publish");
							// }
							break;
						case '查询':
							showQueryContent("yespublish");
							break;
						case '审批跟踪登记表':
							document.location.href = basePath
									+ "/ea/documentflow/ea_getPublishDocList.jspa?finishType=publishyes&track=00";
							break;
						case '导出':
							url = basePath
									+ "/ea/documentflow/ea_showExcel.jspa?search="
									+ search + "&finishType=publishyes&date="
									+ new Date();
							;
							open(url);
							break;
						...
						case '设置每页显示条数':
							var url = basePath
									+ "ea/documentflow/ea_getPublishDocList.jspa?finishType=publishyes&search="
									...
									function() {
										...
										$("#searchForm")
												.attr(
														"action",
														basePath
																+ "ea/documentflow/ea_toSearchpublish.jspa?pageNumber="
																+ pNumber
																+ "&date="
																+ new Date());
...
function re_load() {
	if (token) {
		document.location.href = basePath
				+ "ea/documentflow/ea_getPublishDocList.jspa?finishType=publishyes";
