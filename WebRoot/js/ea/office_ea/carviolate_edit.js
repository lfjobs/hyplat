	...
	function action(com, grid) {
		switch (com) {
			...
			case '删除' :
				...
				$f = $('#carviolateForm');
				$f.find(':input#carviolateID').val(carviolateID);
				if (confirm("是否删除？")) {
					$("#carviolateForm").attr("target", "hidden").attr(
							"action",
							basePath + "ea/carviolate/ea_deleteViolate.jspa?pageNumber="+ pNumber  + "&carviolate.carviolateID="+ carviolateID);
					document.carviolateForm.submit.click();
					token = 2;
//					$("tr#" + carviolateID).remove();
//					//carviolateID = "";
			...
			case '查询':
				...
				break;
			case '设置每页显示条数' :
				var url = basePath+ "ea/carviolate/ea_getCarViolateList.jspa?search="+ search+"&carInformation.carID="
						+ carID+"&type="+type;
				numback(url);
				break;
			case '导出' :
				var url = basePath+ "ea/carviolate/ea_showViolateExcel.jspa?";
				open(url);
				break;
		}
	}
	...
	$(".JQueryflexme tr[id]").click(function() {
		carviolateID = this.id;
		if (personurl) {
			$("#mainframe").attr("src", personurl + carviolateID);
		}
				...
				if ($("form .error").length) {
					...
					$("#addForm")
							...
							.attr(
									"action",
									basePath
											+ "ea/carviolate/ea_saveCarviolateList.jspa?pageNumber="
	...
	$("input.JQuerySubmits").click(function() {
		...
		$("#updateForm")
				...
				.attr(
						"action",
						basePath
								+ "ea/carviolate/ea_updateCarviolate.jspa?pageNumber="
								+ pNumber + "&search=" + search+ "&carviolate.carviolateID="+ carviolateID);
	...
	$("#searchCar").click(function() {
		...
		$("#carSearchForm").attr(
				"action",
				basePath + "/ea/carviolate/ea_toSearch.jspa?pageNumber="
						+ pNumber);
...
function re_load() {
	if (token)
		document.location.href = basePath
				+ "ea/carviolate/ea_getCarViolateList.jspa?pageNumber="
