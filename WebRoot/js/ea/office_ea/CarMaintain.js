	...
	function action(com, grid) {
		switch (com) {
			...
			case '删除' :
				...
				$f = $('#carMaintainForm');
				$f.find(':input#maintainID').val(maintainID);
				if (confirm("是否删除？")) {
					$("#carMaintainForm").attr("target", "hidden").attr(
							"action",
							basePath + "ea/carmaintain/ea_deletMaintain.jspa?pageNumber="+ pNumber  + "&carMaintain.maintainID="+ maintainID);
					document.carMaintainForm.submit.click();
					token = 2;
//					$("tr#" + maintainID).remove();
//					maintainID = "";
			...
			case '查询':
				...
				break;
			case '设置每页显示条数' :
				var url = basePath+ "ea/carmaintain/ea_getListCarMaintain.jspa?search="+ search+"&carInformation.carID="
						+ carID+"&type="+type;
				numback(url);
				break;
			case '导出' :
				var url = basePath+ "ea/carmaintain/ea_showmaintainExcel.jspa?";
				open(url);
				break;
		}
	}
	...
	$(".JQueryflexme tr[id]").click(function() {
		maintainID = this.id;
		if (personurl) {
			$("#mainframe").attr("src", personurl + maintainID);
		}
				...
				if ($("form .error").length) {
					...
					$("#addForm")
							...
							.attr(
									"action",
									basePath
											+ "ea/carmaintain/ea_savemaintainList.jspa?pageNumber="
	...
	$("input.JQuerySubmits").click(function() {
		...
		$("#updateForm")
				...
				.attr(
						"action",
						basePath
								+ "ea/carmaintain/ea_updateMaintain.jspa?pageNumber="
								+ pNumber  + "&carMaintain.maintainID="+ maintainID);
	...
	$("#searchCar").click(function() {
		...
		$("#carSearchForm").attr(
				"action",
				basePath + "/ea/carmaintain/ea_toSearchCar.jspa?pageNumber="
						+ pNumber);
