	...
	function action(com, grid) {
		switch (com) {
			...
			case '查询' :
				...
				break;
			case '导出' :
				url = basePath + "ea/informbills/ea_showExcel.jspa?search="
						+ psearch;
				open(url);
				break;
			case '设置每页显示条数' :
				var url = basePath
						+ "ea/informbills/ea_getInformBillsList.jspa?search="
						+ psearch;
				numback(url);
				break;
		}
				...
				if (informID == "") {
					$("#cstaffForm")
							...
							.attr(
									"action",
									basePath
											+ "ea/informbills/ea_saveInformBills.jspa?pageNumber="
											+ ppageNumber + "&search="
											+ psearch);
					document.cstaffForm.submit.click();
					document.cstaffForm.reset();
...
function re_load() {
	if (token)
		document.location.href = basePath
				+ "ea/informbills/ea_getInformBillsList.jspa?search=" + psearch
