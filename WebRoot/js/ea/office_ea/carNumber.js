$(function() {
  ...
  if(carID!=undefined&&carID!=""){
	...
	function action(com, grid) {
		switch (com) {
			case '添加' :
				...
				break;
			case '导出' :
				var url = basePath+ "ea/carnum/ea_showExcelCarnumber.jspa?";
				open(url);
				break;
			case'修改':
				if(cnID==""){
				...
				if (confirm("是否删除？")) {
					...
					$f
							...
							.attr(
									"action",
									basePath
											+ "/ea/carnum/ea_deleteCarroad.jspa?pageNumber="
											+ pNumber + "&carNumber.cnID="
											+ cnID);
			...
			case '设置每页显示条数' :
				var url = basePath
						+ "/ea/carnum/ea_getListCarByCompanyID.jspa?carInformation.carID="
	...
	$(".carroad tr[id]").click(function() {
				$("input.JQuerypersonvalue", $(this))
						.attr("checked", "checked");
				cnID = this.id;
			    
			});
	$(".carroad tr[id]").dblclick(function() {
				$("input.JQuerypersonvalue", $(this))
						.attr("checked", "checked");
	...
	$("input.JQuerySubmit").click(function() {
		if (cnID == "") {
			$("#addForm").attr("target", "hidden").attr(
					"action",
					basePath + "ea/carnum/ea_saveCarNumList.jspa?pageNumber="
							+ pNumber);
...
function re_load() {
	if (token)
		document.location.href = basePath
				+ "/ea/carnum/ea_getListCarByCompanyID.jspa?pageNumber="
