...
JS.Connector = JS.extend(JS.Observable,
		{
			...
			constructor : function() {
				...
				this.addEvents([ 'beforeConnect', 'connect', 'beforeStop',
						'stop', 'message', 'revival' ]);
				if (JS.isIE7) {
			...
			doDrop : function(url, cId, conn, xhr) {
				...
				try {
					...
					var url = this.url + '?' + this.CMDTAG + '=drop&cid='
							+ this.cId + param;
			...
			dispatchServerEvent : function(msg) {
				this
						.fireEvent('message', msg.channel, msg.data, msg.time,
								this);
			},
			...
			startConnect : function(url) {
				if (this.running) {
					...
					connXhr.addListener('load', function(xhr) {
						...
						this.fireEvent('connect', data.cId, data.channels,
								data.ws, data.timeout, this);
						this.revivalConnect();
					}, this);
			...
			revivalConnect : function() {
				...
				function revival() {
					...
					var url = self.url + '?' + self.CMDTAG + '=revival&cid='
							+ self.cId + param;
			...
			start : function(url, param) {
				...
				var url = this.url + '?' + this.CMDTAG + '=conn&cv='
						+ this.version + param;
				this.running = true;
				this.currRetry = 0;
				var self = this;
			...
			stop : function(cause) {
				...
				if (this.fireEvent('beforeStop', cause, this.cId, this.url,
						this) === false) {
					return;
				}
				this.running = false;
