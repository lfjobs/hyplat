...
(function() {
	//需要将config.jqueryOverrideVal 设为tue才能使用jquery取值
	...
	a.extend(a.fn, {
		...
		ckeditor : function(b, c) {
			...
			this.filter('textarea, div, p').each(function() {
				var e = a(this), f = e.data('ckeditorInstance'), g = e
							...
							if (j.config.autoUpdateElementJquery
								...
								e.bind('destroy.ckeditor', function() {
											e.parents('form').unbind('submit',
													k);
											e.parents('form').unbind(
													'form-pre-serialize', k);
										});
							}
							j.on('destroy', function() {
