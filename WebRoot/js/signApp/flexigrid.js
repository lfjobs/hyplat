		...
		var g = {
			...
			addData : function(data) { // parse data
				...
				if (p.dataType == 'json') {
					$.each(data.rows, function(i, row) {
						    ...
						    if (p.checkbox) {
							    var cth = $('<th/>');
							    var cthch = $('<input type="checkbox" name="Selected" id="Selected" value="'
							        + $(tr).attr('id') + '"/>');
							    var objTr = $(tr);
			...
			populate : function() { // get latest data
				...
				var param = [{
				 ...
				 $(".pReload ,.pFirst ,.pPrev ,.pNext ,.pLast").unbind("click");
				if (p.params) {
				...
				//function binds(){
					$.ajax({
						...
						success: function(data){
							g.addData(data);
							function binds(){
							$(".pReload ,g.pDiv").bind("click", function(){
								g.populate();
							});
							$(".pFirst ,g.pDiv").bind("click", function(){
								g.changePage('first');
							});
							$(".pPrev ,g.pDiv").bind("click", function(){
								g.changePage('prev');
							});
							$(".pNext ,g.pDiv").bind("click", function(){
								g.changePage('next');
							});
							$(".pLast ,g.pDiv").bind("click", function(){
								g.changePage('last');
							});
							};
							setTimeout(binds, 2000);
						},
						error: function(data){
							try {
								if (p.onError) 
		...
		if ($('th', g.hDiv).length) {
			...
			if (p.checkbox) {
				$('tr', g.hDiv).each(function() {
					...
					var cthch = $('<input type="checkbox" id="Selected"/>');
