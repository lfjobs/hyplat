$(document).ready(function(){
   ...
   //     alert("温馨提示：发展让其您所属商城业主会员或客户粉丝下订单，您才能享受更多佣金金币");
        ...
        var pid = "";
        $(".ptbl").each(function () {
            var ppid = $(this).find(".ppid").val();//购物车产品id
            var num = $(this).find(".num").val();//添加数量
            var stardard = $(this).find(".stardard").text();//规格
            var price= $(this).find(".price").val();//金额
            var priceid= $(this).find(".priceid").val();//促销活动id
            var prt= $(this).find(".prt").val();///价格类型 0或者null零售订单  1：批发价格 2：VIP  3. 普通活动 4.特价活动
            var ccj= $(this).find(".ccj").val();//批发价
            var pscId= $(this).find(".pscId").val();//购物车id
            if (stardard == "") {
                stardard = "默认规格";
            }
            pid += ppid + "-" + num + "-" + stardard.trim() + "-"+ priceid + "-"+ prt + "-"+ price + "-"+ ccj + "-" + pscId ;
            pid += "zz";
        });
        $("#companyIds").val(companyId);
        $("#pids").val(pid);
        $("#leavemessages").val(leavemessage);
        var ulp = basePath + "/ea/wfjshop/sajax_ea_ShoppingMulti.jspa?";
...
function zf() {
	...
	if (zffs == null) {
		...
		if (zffs == '1') {			
			...
			par.push("WIDout_trade_no=" + ddid);//单号
			par.push("&WIDtotal_fee="+ $("#morre").val());//总价
			par.push("&WIDsubject=" + goodsname);//货物名称
			...
			$("#formsutm").attr("action", basePath + "ea/wfjshop/ea_zfgs.jspa?ddid=" + ddid + "&baseUrl=" + basePath+"&code=2&buyIsOkPage="+buyIsOkPage);
			$("#submit").click();
		} else if (zffs == '3') {
			var money = $("#morre").val();
            ...
            if(ss == "cg"){
                if (buyIsOkPage=="buyIsOkPage"){
                    document.location.href = basePath+"st/buyIsOkPage.jsp?ddid="+ddid+"&companyName="+companyName+"&staffName="+staffName+"&companyIdentifier="+companyIdentifier;
                	  ...
                	  document.location.href = basePath + "page/conacts/restaurant/paySuc.jsp?jump=phl";
                }
            }else  if(ss=="shibai"){
