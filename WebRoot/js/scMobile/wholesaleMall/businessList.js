 ...
 * 异步获取所有商家
 */
function ajaxBussList() {
    clearTimeout(t);
    pagenumber++;
    //ajax获取超市相关数据追加展示
    var url = basePath+"ea/wholesaleMall/sajax_ea_ajaxBussList.jspa";
    //获取搜索的超市名称
    var search = $("#supermarketName").val();
    $.ajax({
        url: encodeURI(url),
        type: "POST",
        async: true,
        ...
        success: function (data) {
            ...
            if (pageForm != null && pageForm.recordCount > 0) {
                ...
                // if(pagenumber==1){
                //     sup.push('<li>');
                //     /*sup.push('<a class="txt clearfix" style="display: block;" href="'+basePath+'/ea/wholesaleMall/ea_toWholesaleMall.jspa?ccompanyID='+qbsjCompanyID+'&companyName=全部商家">');*/
                //     sup.push('<a class="txt clearfix" style="display: block;" href="'+basePath+'/ea/wholesaleMall/ea_toWholesaleMall.jspa?ccompanyID=all&companyName=全部商家">');
                //     sup.push('<div>');
                ...
                for (var i in supList) {
                    sup.push('<li>');
                    sup.push('<a  class="txt clearfix" style="display: block;" href="'+basePath+'ea/wholesaleMall/ea_toWholesaleMall.jspa?ccompanyID=' + supList[i][0] + '&industryType=' + supList[i][4] + '&companyName=' + supList[i][1] + '">');
                    sup.push('<div>');
