...
function getOneMenuHtml(oList){
    ...
    if(showFlag){//分列表
       innerHtml += "<li onclick=\"toMenuClick('-1','-1');\">预算单</li>";
    }else{//总列表
       innerHtml += "<li class=\"active\" onclick=\"toMenuClick('-1','-1');\">预算单</li>";
    }
...
function toMenuClick(id,depName){
    ...
    //拼接链接
    var url = "ea/scBudget/ea_toPayBudgetList.jspa";
