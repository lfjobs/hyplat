...
(function($){
	$.addFlex = function(t,p)
	{
			...
			$('thead tr:first th:visible',this.hDiv).each
				(
			 	function ()
					...
					$('tbody#tbwid tr',this.hDiv).each(function(){ //控制文本宽度与列宽度相同
						/*************设置input****************/
						if($(this).find('td:eq('+n+') input').next().text() == '选择'){
							$(this).find('td:eq('+n+') input').css('width',cdpos-30).css('margin','-5px');
							
						}else if($(this).find('td:eq('+n+') input').hasClass("fgnsize")){
							$(this).find('td:eq('+n+') input').css('width',$(this).find('td:eq('+n+') input').attr("size"));
						}else{
							$(this).find('td:eq('+n+') input').css('width',cdpos+6).css('margin','-5px');
						}
						
						if(n>0){
							$(this).find('td:eq('+n+') input').css('border','4px solid #d5effc').css('background-color','#d5effc');
							
							$(this).find('td:eq('+n+') input').blur(function(){
								$(this).css('background-color','#d5effc').css('border','4px solid #d5effc');
							});
						} 
					});
		...
		*/
		...
		/*if(g && g.bDiv)
		    $(g.bDiv).css({ "height":( _height - _removeHeight - 60) + "px"});
        */
		if(!p.allDouble){  
            $(g.bDiv).css({"height": $(window).height() - 31 - 30 - 26 - 40 + "px"});
        	...
        	resizeDoubleRightFrame(g);
			
			$(window).resize(function(){
				setTimeout(resizeDoubleRightFrame(g),100);
			}); 
		}*/
		/****************/
	...
	function resizeDoubleRightFrame(g){
		var _mainFrame = window.parent.document.getElementById("mainFrame");
		var leftHeight = $(window.parent.document.getElementById("leftFrame")).height();
		var rightTopHeight = $(window.parent.document.getElementById("daohang")).height();
		var _height =  leftHeight - rightTopHeight;
		$(window.parent.document.getElementById("rightFrame")).height(_height);
		var _removeHeight = $(g.hDiv).height() + $(g.mDiv).height() + $(g.tDiv).height();
		if(_mainFrame && _mainFrame.height() > 0){
			$(g.bDiv).css({ height:$(window).height()/2 - 30 - _removeHeight + "px"});
			_mainFrame.height(_height / 2 - 30 );
		}else{					
			$(g.bDiv).css({ height:(_height - _removeHeight - 60) + "px"});
		}
	}
