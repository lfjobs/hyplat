...
function getList() {
    $(".div-chaxun").hide();
    $(".div-mianshi").hide();
    var url = "ea/cosincumbent/sajax_n_ea_getAuditionkb.jspa?";
        ...
        success: function (data) {
            if (data != null) {
                ...
                for (var i = 0; i < arry.list.length; i++) {
                    ...
                    htmlstr.push("</div>");
                    var status
                    if (arry.list[i].status == "21") {
                        status = "通过";
                    } else {
                        status = "未通过";
                    }
                    htmlstr.push("<div class='sn-p' title='" + arry.list[i].staffName + "' style='width:20%!important;'>" + arry.list[i].staffName + "</div>");
                    htmlstr.push("<div class='oask-p' title='" + arry.list[i].auditionDirection + "' style='width:20%!important;'>" + arry.list[i].auditionDirection + "</div>");
                    htmlstr.push("<div class='ln-p' title='" + arry.list[i].auditionPost + "' style='width:15%!important;'>" + arry.list[i].auditionPost + "</div>");
                    htmlstr.push("<div class='ln-p' title='" + status + "' style='width:15%!important;'>" + status + "</div>");
                    htmlstr.push("<div class='ln-p' title='" + arry.list[i].experience + "' style='width:15%!important;'>" + arry.list[i].experience + "</div>");
