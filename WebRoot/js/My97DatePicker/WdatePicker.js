...
function daymonth(el){
 new WdatePicker({dateFmt:'yyyy-MM'});
}

function cher_dm(el){
	 new WdatePicker({dateFmt:'yyyyMM'});
}

function cher_ydm(el){
	 new WdatePicker({dateFmt:'yyyyMMdd'});
}
...
function cher_time(el){
	 new WdatePicker({dateFmt:'yyyyMMddHHmmss'});
}

function maxdaymonth(el){ //月工资查询(每月3号凌晨对月工资进行保存)
 var date = new Date();
 var year = date.getFullYear();
 var months = date.getMonth();
 var endMonths = '';
 
 if(date.getDate() < 3){ //1、2日 （每月3号之前只能查上月之前的月工资）
 	if(months == 0){ //1月
 		year = year - 1;
 		months = 11;
 	}else if(months == 1){ //2月
 ...
 endMonths = year + '-' + months;
 new WdatePicker({dateFmt:'yyyy-MM', maxDate:endMonths});
}

function dayhour(el){
 new WdatePicker({dateFmt:'HH:mm:ss'});
}