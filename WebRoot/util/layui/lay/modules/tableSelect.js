...
//                return false;
            ...
            var colss=opt.table.cols[0];
            var collength=colss.length;
            var sumSize=30;
            for (var i = 0; i < collength; i++) {
            	var width=100;
            	if(colss[i].hasOwnProperty("width")&&colss[i]["width"]!=0){
            		width=parseInt(colss[i]["width"]);
            	}else if(colss[i].hasOwnProperty("minWidth")&&colss[i]["minWidth"]!=0){
            		width=parseInt(colss[i]["minWidth"]);
            	}
            	sumSize+=width;
            }
            var t = elem.offset().top + elem.outerHeight()+"px";
            var leftSet=elem.offset().left;
//            var winWidth=$(window).width();
//            if(winWidth-leftSet<sumSize){
//            	leftSet=winWidth-sumSize;
//            }
            var l = leftSet +"px";
    ...
    */
    ...
    tableSelect.prototype.reload = function (id,opt) {
    	table.reload(id,opt);
    }
